...
class Personnel extends CActiveRecord{
	...
	*/
	...
	public static function search($term, $limit=10){
        ...
        */
        $criteria->params = $params;
        $doctorTitle = self::getDoctorTitle();
        $drTitles = explode(',', $doctorTitle);

        $criteria->addInCondition('job_function_title', $drTitles,'AND');
		return self::model()->findAll($criteria);
	}

	/* Added by jeff for global_config/dynamic roles for doctors. */
	public function getDoctorTitle()
    {
        $config = new ConfigGlobal();

        /* Global config for Doctor roles  */
        $DoctorTitle = $config->findByAttributes(array(
            'type' => 'eclaims_doctor_title'
        ))->value;
