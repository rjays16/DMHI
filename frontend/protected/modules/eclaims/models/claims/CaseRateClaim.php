...
Yii::import('eclaims.models.MiscOps');
Yii::import('eclaims.models.MiscOpsDetails');
...
class CaseRateClaim extends Claim
{
    ...
    protected function setCf1Node()
    {
        ...
        $attrs['pMemberFirstName'] = $encoder->formatUTF8(strtoupper($member->member_fname));
        ...
        $attrs['pPatientFirstName'] = $encoder->formatUTF8(strtoupper($person->getNameFirst()));
    ...
    protected function setCf2Node()
    {
        ...
        if ($personIsDeath) {
            ...
            // Commented for Service Request DMHI - Bender - Feb 23,2021 3:55AM
         ...
         */
        $attrs['pReferralIHCPAccreCode'] = $encounter->getDispositionCode() === 'T' ? $encounter->referralDiagProcedure->accredit_nr : '';
        $attrs['pReferralReasons'] = $encounter->getDispositionCode() === 'T' ? $encounter->referralDiagProcedure->reason : '';
    ...
    protected function setCf2Special()
    {
        ...
        $this->setSpecialNcp();
        $misc = $this->getCheckupDates();

        //TODO
        if (strtoupper($misc['ops_code']) === self::MCP01) {
            $this->setSpecialMcp();
        }
    ...
    protected function setSpecialMcp()
    {
        ...
        $dates = $this->getCheckupDates();
        $attrs['pCheckUpDate1'] = $this->formatter->formatDate($dates['checkup_date1']);
        $attrs['pCheckUpDate2'] = $this->formatter->formatDate($dates['checkup_date2']);
        $attrs['pCheckUpDate3'] = $this->formatter->formatDate($dates['checkup_date3']);
        $attrs['pCheckUpDate4'] = $this->formatter->formatDate($dates['checkup_date4']);
    ...
    protected function setCf2Consumption()
    {
        ...
        $sql = $db->Prepare("SELECT
                  ...
                  (SELECT 
                    IFNULL(SUM(sai.`amount`), 0) 
                      FROM seg_additional_insurance `sai` 
                      WHERE sai.hcare_id = '" . self::HCARE_ID . "'
                      AND sai.`encounter_nr` = sbe.`encounter_nr`) AS total_phic_hci_coverage,
                  (SELECT 
                    IFNULL(SUM(sai.`amount`), 0) 
                      FROM seg_additional_insurance `sai` 
                      WHERE sai.hcare_id <> '" . self::HCARE_ID . "'
                      AND sai.`encounter_nr` = sbe.`encounter_nr`) AS total_other_insurance_coverage,
                  (SELECT 
                    SUM(sbp.dr_claim)
                  FROM
                    seg_billing_pf sbp 
                  WHERE sbp.hcare_id = '" . self::HCARE_ID . "' 
                    AND sbp.bill_nr = sbe.bill_nr) AS total_phic_doc_coverage,
                    sem.memcategory_id,
                    ser.total_meds,
                    ser.total_xlo
        ...
        if ($result = $db->Execute($sql, $enc)) {
            if ($result->RecordCount()) {
                ...
                $total_doc_coverage = $row['total_phic_doc_coverage'];
                $total_hci_charge = $row['total_hci_charge'];
                $total_hci_discount = $row['total_hci_discount'];
//                $total_hci_coverage = $row['total_services_coverage']; //comment by Juna, 2021
                $total_hci_coverage = $row['total_phic_hci_coverage']; //added by Juna, 2021
                $patient_name = $row['name'];
                $total_charge = $row['total_doc_charge'] + $row['total_hci_charge'];
                $total_coverage = $row['total_doc_coverage'] + $row['total_phic_hci_coverage'] + $row['total_other_insurance_coverage'] + $row['total_phic_doc_coverage'];
...
//                $pf_other = $row['pf_other'];
    ...
    protected function isNewBorn($enc)
    {
        ...
        if ($result = $db->Execute($strSQL, $enc)) {
            if ($result->RecordCount()) {
                while ($row = $result->FetchRow()) {
                    if ($row['package_id'] == self::NEWBORN) {
                        return true;
                    } else {
                        return false;
                    }
        ...
        if ($isWellBaby) {
            ...
            if ($rs) {
                if ($rs->RecordCount() > 0) {
                    ...
                    $sql_hearing = $db->Prepare("INSERT INTO seg_caserate_hearing_test (encounter_nr,is_availed) VALUES (?,1)");
                    $rs = $db->Execute($sql_hearing, $enc);
                    if ($rs) {
                        return 1;
                    } else {
                        return 2;
    ...
    protected function getDoctorInfo($enc)
    {
        ...
        $strSQL = $db->Prepare("SELECT sbe.`bill_nr`,
            sbe.`bill_dte`,
            fn_getpersonell_first(sbp.`dr_nr`) as pDoctorFirstName,
            fn_getpersonell_last(sbp.`dr_nr`) as pDoctorLastName,
     ...
     */
    public function setMembershipType($memType)
    {
    ...
    protected function getFilterCardNumber()
    {
        global $db;
        $encounterNr = $this->encounter_nr;
        $miscOps = MiscOps::model()->findByAttributes(array('encounter_nr' => $encounterNr));
        $miscOpsDetails = MiscOpsDetails::model()->findByAttributes(array('refno' => $miscOps->refno));
        return $miscOpsDetails->sticker_no;
    }

    protected function getCheckupDates()
    {
        global $db;
        $encounterNr = $this->encounter_nr;
        $miscOps = MiscOps::model()->findByAttributes(array('encounter_nr' => $encounterNr));
        return MiscOpsDetails::model()->findByAttributes(array('refno' => $miscOps->refno));
    }
