...
class EclaimsEncounterInsurance extends EncounterInsurance
{
    ...
    public function createEncounterInsurance(Encounter $encounter)
    {
        ...
        $insuranceProvider
            = InsuranceProvider::getProviderByShortFirmId(InsuranceProvider::INSURANCE_PHIC);

        $this->encounter_nr = $encounter->encounter_nr;


        $encounterInsurance
            = $this->getEncounterInsuranceByProvider($insuranceProvider);
        ...
        if (empty($encounterInsurance)) {
            ...
            global $db;
            $memCategory
                = $db->GetOne('SELECT memcategory_id FROM seg_memcategory WHERE memcategory_code='
                .$db->qstr($phicmember->member_type));
    ...
    /**
     * @param $encouter Encounter, selected reason, other reason, insurance
     *                  number. Saving audit trail of removing insurance from
     *                  eclaims to billing.
     *
     * @author Jeff Ponteras on 01-28-18.
     */
    public function removeReasonInsurance(
        $pid,
        $session_user,
        $reasonSelected,
        $reasonOthers,
        ...
        $insurance_nr
            = $db->GetOne('SELECT insurance_nr FROM seg_encounter_insurance_memberinfo WHERE encounter_nr='
            .$db->qstr($this->encounter_nr));

        if (!$insurance_nr || $insurance_nr == null || $insurance_nr == '0') {
            $insurance_nr
                = $db->GetOne('SELECT insurance_nr FROM seg_insurance_member_info WHERE pid='
                .$db->qstr($pid));
        }

        if ($reasonSelected == 'Others') {
            $text = "Deleted by ".$session_user." on ".date("F j, Y, g:i a")
                ." Other reason:".$reasonOthers."\nencounter_nr="
                .$db->qstr($this->encounter_nr).",pid=".$db->qstr($pid)
                .",reason=".$db->qstr($reasonSelected).",other_reason="
                .$db->qstr($reasonOthers).",insurance_nr="
     ...
     */
    public function InsertReasonInsurance(
        $pid,
        $session_user,
        Encounter $encounter
    ) {
        ...
        $text = "Created by ".$session_user." on ".date("F j, Y, g:i a")
            ."\nencounter_nr=".$db->qstr($this->encounter_nr).",pid="
            .$db->qstr($pid).",insurance_nr=".$db->qstr($insurance_nr)."\n\n";
        $text = $db->qstr($text);
        $enc = $db->qstr($this->encounter_nr);
    ...
    /**
     * @param $encouter Encounter.
     *                  Checking if insurance number is null before adding of
