...
?>
        ...
        <div class="span12">
            ...
            if ($countActiveEnc > 1) {
                ...
                $insuranceButton = array(
                    ...
                    'disabled' => empty($person->activeInsuranceEncounter) || $hasFinalBill || empty($person->phicMember2->insurance_nr),
                    'htmlOptions' => array(
            ...
            if ($countActiveEnc == 1 && !empty($person->activeInsuranceEncounter->encounterInsurance)) {
                $insuranceButton = CMap::mergeArray($insuranceButton, array(
                    ...
                    'htmlOptions' => array(
                        ...
                        'data-toggle' => 'modal',
                        'data-encounter' => $person->activeInsuranceEncounter->encounter_nr,

                        'data-target' => (!$hasFinalBill) ? '#riModal' : '#'
                    )
                ));
            }
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Reason for Removing of Insurance</h4>
                </div>
                <!-- Modal Content -->
                <div class="modal-body">
                    <form id="reason-form">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="riModalSelect" name="riModalSelect">
                                <option>Not compensable</option>
                                <option>Admitted from other hospital</option>
                                <option>Regular HD from other hospital</option>
                                <option>Correction/Wrong entry of issuance no.</option>
                                <option>Encoding of Pin/Insurance Number for POC/HSM</option>
                                <option>Not qualified for PHIC</option>
                                <option>Others</option>
                            </select>
                            <div>
                <textarea id="riModalTextArea" name="riModalTextArea" placeholder="type reason/s here. . ."
                          style="width: 515px;"></textarea>
                                <input type="hidden" id="pid" name="pid" value="<?php echo $_GET['pid']; ?>">
                                <input type="hidden" id="urlData" name="urlData" value="<?php echo $baseUrl; ?>">
                                <input type="hidden" id="action" name="action" value="remove">

                                <!-- jeff 02-04-18 for getting encounter from selected trashCan -->
                                <input type="hidden" id="get_enc" name="get_enc"
                                       value="<?php echo $person->encounter->encounter_nr; ?>">
                ...
                <div class="modal-footer">
                    <button type="submit" id="riModalSubmit" class="btn btn-default"
                            style="background-color: #00a50e; width: 120px; color: #fff; border-radius: 15px 50px; border:none; text-shadow: none;">
                        <i class="fa fa-check"></i> Submit
                    </button>
                    <button type="button" id="riModalClose" class="btn btn-default" data-dismiss="modal"
    ...
    <script type="text/javascript">
        ...
        $("#reason-form").submit(function (event) {
            ...
            data.push({name: 'encounter', value: button.data('encounter')});
