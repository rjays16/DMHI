...
//                        }
    ...
    public function actionResponseDetails()
    {
        ...
        $transmittals = EclaimsTransmittal::model()->findAll(
            array(
        ...
        foreach ($transmittals as $transmittal) {
            ...
            if ($transmittal->ext->is_mapped == 1 && $transmittal->ext->is_uploaded == 1) {
                ...
                $details = EclaimsTransmittalDetails::model()->findAll(
                    array(
                        'condition' => 'transmit_no=:transmitNumber',
                        'params' => array(':transmitNumber' => $transmitNumber),
                    )
    ...
    public function actionMap()
    {
        ...
        /*Auto check status of all claims under this transmittal number after mapping. */
        if ($call_status) {
        ...
        foreach ($claim_list as $key => $claim) {
            ...
            try {
                ...
                Yii::app()->user->setFlash(
                    'error',
                    '<strong>'.$e->getMessage(
                    ).':</strong> '.@$result['reason'].' <small> (Code:'.$e->statusCode.')</small>'
                );
            }
        }
