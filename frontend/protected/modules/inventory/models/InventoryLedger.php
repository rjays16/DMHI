...
class InventoryLedger extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('unit_cost, movement, prev_qty, prev_cost, pack_qty', 'numerical'),
			array('post_uid, tref_no, sku_id', 'length', 'max'=>36),
			array('tr_code', 'length', 'max'=>3),
			array('encounter_no', 'length', 'max'=>12),
			array('post_date, tr_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('post_uid, post_date, tref_no, tr_code, tr_date, sku_id, unit_cost, movement, prev_qty, prev_cost, pack_qty, encounter_no', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function postSkuItemMovement($enc_no, $refno, $trcode, $trdate, $skuid, $qty, $packqty, $ucost = 0)
    {
        try {
            ...
            $this->attributes = array(
                ...
                'movement' => doubleval($qty),
                'pack_qty' => doubleval($packqty),
