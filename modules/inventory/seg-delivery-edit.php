			...
			$data_items = array();
				foreach ($_POST["items"] as $i=>$v) {
                 ...
                 // Associated serial no.
                        ...
                        $saveok = $objdel->saveDeliveryDetail($_POST['rcv_area'], $data, $_POST["delivery_date"], $expiry, $objserial, $lotNo);
...
# Render form elements
		...
		if ($_GET['target'] == "Edit") {
			$supplier_id = $objdelivery->getSupplier($_GET['refno']);
		}
		...
		if($supplier_values){
			...
			foreach($supplier_values AS $key => $value){
				...
				$supplier .= "<option value=\"".$value['supplier_id']."\" $selctd>".$value['supp_name']."</option>\n";
			}
		}
		else
		...
		$smarty->assign('sPoNo','<input class="segInput" id="po_no" name="po_no" type="text" size="10" value="'.$po_no.'" style="font:bold 12px Arial"/>');
