...
function prepareAdd(id, unit, unit_name, isperpc, qty, expiry_dt, serial_no, lot_no, price_add) {
                ...
                rowSrc = "<tr>"+
                            ...
                            '</td>'+
                             '<td>'+
                                '<input type="button" name="nem[]" id="nem'+tmpid+'" value="'+packname+'" style="cursor:pointer; color:#000066; font-family:verdana; font-weight:bold; padding:0px 2px" '+
                                    'onclick="showAdditionalInfo(\''+tmpid+'\',\''+pack+'\',\''+packname+'\', 0,\''+escape(name)+'\')" '+
                            ...
                            '<td>'+
                                '<input type="button" name="nem[]" id="nem'+tmpid+'" value="'+piecename+'" style="cursor:pointer; color:#000066; font-family:verdana; font-weight:bold; padding:0px 2px" '+
                                    'onclick="showAdditionalInfo(\''+tmpid+'\',\''+piece+'\',\''+piecename+'\', 1,\''+escape(name)+'\')" '+
...
function showAdditionalInfo(id, unit, unit_name, isperpc,name) {  
    var seg_URL_APPEND = $F('seg_URL_APPEND');
    return overlib(OLiframeContent('seg-deliveryitem-params.php'+seg_URL_APPEND+'&id='+escape(id)+'&unit='+unit+'&name='+escape(unit_name)+'&perpc='+escape(isperpc)+'&itemname='+name, 460, 225, 'fRelatedInfo', 0, 'no'),
