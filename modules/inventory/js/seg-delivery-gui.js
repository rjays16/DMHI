...
function applyUPrice(e, id, srow) {
    ...
    if ( (characterCode == 13) || (isESCPressed(e)) ) {        
        ...
        if ( !(isNaN(parseInt(uprice))) && (parseInt(uprice)>=0) ) {
            ...
            $("rowtotal_"+id+srow).innerHTML = '<input id="total_'+id+srow+'" type="hidden" value="'+formatNumber(Number(qty) * Number(uprice), 2)+'">'+formatNumber(Number(qty) * Number(uprice), 2);
...
//                }
            // added by ken for supplying data in suppliers and area that assign FIS PO.,
                $('supplier').value = details.supp_id;
...
//                details.price_add = parseFloat(nprice);
                ...
                src = '<tr class="wardlistrow'+alt+'" id="row_'+details.id+srow+'">' +                        
                          ...
                          //'<input name="uprices[]" id="uprice_'+details.id+srow+'" type="hidden" value="'+details.price_add+'">'+ // commented by michelle 07-04-16
                          '<input name="uprices[]" id="uprice_'+details.id+srow+'" type="hidden" value="'+formatNumber(Number(details.price_add),3)+'">'+
                          '<input name="eqty[]" id="eqty_'+details.id+srow+'" type="hidden" value="'+details.qty+'">'+
                          '<input id="total_'+details.id+srow+'" type="hidden" value="'+formatNumber(Number(details.qty) * Number(details.price_add), 2)+'">'+
                          ...
                          //'<td width="10%" align="right"><input style="width:95%;display:none;text-align:right" type="text" id="edituprice_'+details.id+srow+'" value="'+formatNumber(Number(details.price_add),2)+'" onFocus="this.select();" onblur="cancelUPrice(\''+details.id+'\', '+srow+');" onkeyup="applyUPrice(event,\''+details.id+'\','+srow+');">'+ //commented by michelle 07-04-16
                          '<td width="10%" align="right"><input style="width:95%;display:none;text-align:right" type="text" id="edituprice_'+details.id+srow+'" value="'+formatNumber(Number(details.price_add),3)+'" onFocus="this.select();" onblur="cancelUPrice(\''+details.id+'\', '+srow+');" onkeyup="applyUPrice(event,\''+details.id+'\','+srow+');">'+
                            ...
                            //'<span style="cursor:pointer" id="rowuprice_'+details.id+srow+'" onclick="editUPrice(\''+details.id+'\','+srow+');">'+formatNumber(Number(details.price_add),2)+'</span></td>'+ // commented by michelle 07-04-16                     
                            '<span style="cursor:pointer" id="rowuprice_'+details.id+srow+'" onclick="editUPrice(\''+details.id+'\','+srow+');">'+formatNumber(Number(details.price_add),3)+'</span></td>'+                          
                          //'<td width="12%" align="right"><span id="rowtotal_'+details.id+srow+'">'+formatNumber(Number(details.qty) * Number(details.price_add), 2)+'</span></td>'+  // commented by michelle 07-04-16
                          '<td width="12%" align="right"><span id="rowtotal_'+details.id+srow+'">'+formatNumber(Number(details.qty) * Number(details.price_add), 3)+'</span></td>'+ 
                      '</tr>';                      
            }
            else {
                src = "<tr><td colspan=\"10\">Delivery is currently empty ...</td></tr>";
            }
            if(items.length != 0){
                    dBody.innerHTML += src; 
            }
...
function jsSetSupplierName(value){
    ...
    $("supplier_name").value = value;
        ...
        var url = 'seg-deliveryitem-params.php?id='+id+'&itemname='+name+'&unit=2&name=piece(s)&update=1&qty='+qty+'&price='+price+'&srow='+srow+'&order='+order;
                overlib(
                        OLiframeContent(url, 425, 250, 'fOrderTray', 0, 'no'),
...
function updateItem(id, qty, expiry_dt, serial_no, price_add, srow, lot_no){
    ...
    $('uprice_'+id+srow).value = formatNumber(Number(price_add), 2);
    $('rowuprice_'+id+srow).innerHTML = formatNumber(Number(price_add), 2);

    $('total_'+id+srow).value = formatNumber(Number(qty) * Number(price_add), 2);
    $('rowtotal_'+id+srow).innerHTML = formatNumber(Number(qty) * Number(price_add), 2);
