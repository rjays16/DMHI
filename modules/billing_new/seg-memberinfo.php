...
function getMembershipTypes($memtype, &$disableEmployerFields) {
    global $db;
    $select = "'input'";
    $html = '<select onkeydown="if(event.keyCode == 13)getfocus();jumpNext(this,'.$select.')" class="segInput required-verify required-save" id="membership-type" name="pMembershipType" >';
    $strselected = ($memtype === '') ? ' selected="selected"' : "";
    $html .= '<option  onkeydown="if(event.keyCode == 13)getfocus();jumpNext(this,'.$select.')" value=""'.$strselected.'>--Select membership type--</option>';
    ...
    if ($result !== false) {
        while ($row = $result->FetchRow()) {
            ...
            $html .= sprintf(
                '<option onkeydown="if(event.keyCode == 13)getfocus();jumpNext(this,'.$select.')" value="%s" data-employer-required="%s" %s>%s</option>',
...
# Mod by Jeff 04.23.18 for fetching of insurance member info by encounter.
// $info = $person->getMemberInsuranceInfo($pid, $provider);
$info = $person->getMemberInsuranceInfoByEncounter($encounter_nr, $provider);
if ($isPrincipal) {
...
<div class="modal-content">
    ...
    <form id="member-form" action="#">
        ...
        <input type="text" id="member-id" onkeydown="if(event.keyCode == 13)jumpNext(this,'input')" class="segInput required-verify required-save" name="pPIN" value="<?= htmlentities($id) ?>" maxlength="12"/>
        ...
        <select class="segInput required-verify required-save" name="pMemberGender"  id="member-gender" >
        ...
        <input type="text" id="member-birthdate" onkeyup="dtval(this,event)" onkeydown="if(event.keyCode == 13)jumpNext(this,'select')" class="segInput required-get required-verify required-save" name="pMemberBirthDate" value="<?= $bdate ?>" style="width:100px" size="10" maxlength="10" /><br/>
...
<?php } ?>
        ...
        <input type="text" id="member-pempno" maxlength="25" onkeydown="if(event.keyCode == 13)jumpNext(this,'input')" class="segInput required-verify required-save" name="pPEN" value="<?= htmlentities($empno) ?>" <?= $disableEmployerFields ? 'disabled="disabled"' : '' ?> />
        <label for="member-pempnm">Employer Name</label>
        <input type="text" id="member-pempnm" onkeydown="if(event.keyCode == 13)getfocus()" class="segInput required-verify required-save" name="pEmployerName" value="<?= htmlentities($empnm) ?>" <?= $disableEmployerFields ? 'disabled="disabled"' : '' ?> />
