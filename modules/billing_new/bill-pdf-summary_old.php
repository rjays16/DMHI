...
//edited by daryl
    ...
    function getTotals(){
        ...
        $pf_ex = $totals['total_pf'] - ($coverages['total_pf_coverage'] + $discounts['total_pf_discount'] + $other['pfdiscount']);
        ...
        $total_discount = ($discounts['total_pf_discount'] + $other['pfdiscount']) + ($discounts['total_serv_discount'] + $miscData['misc_discount'] + $other['hcidiscount']);
...
// echo $hci_if_SC['ifSC']."ssss";
        ...
        $output = array("total_charge"=>$total_charge,
                        ...
                        "pf_discount"=>$discounts['total_pf_discount'] + $other['pfdiscount'],
...
// $get_result = array();
        ...
        if ($this->IsDetailed){
            ...
            if(count($data)>0){
                ...
                $this->Cell(COL05_WIDTH, 4, "", "", 0, 'R');
                $this->Cell(COL06_WIDTH, 4, "", "", 0, 'R');
                $this->Cell(COL06_WIDTH-8, 4, number_format($hosp_discount+$pfee_discount, 2, '.', ','), "", 1, 'R');
            }else{
                 $this->Cell(COL_MID+2, 4, "", "", 0, '');
...
var $serv_total = 0;
    function Sub_Total(){
        ...
        $this->Cell(COL_MID, 4, " ", "", 0, '');
        $this->Cell(COL04_WIDTH, 4, $this->hdata['serv_discount'], "T", 0, 'R');
...
#added by daryl
   ...
   if ($hosp_excess < 0){
    ...
    $hosp_excess = $hosp_excess;
        ...
        $this->Cell(COL07_WIDTH, 4, number_format($hosp_excess/*+$this->mandatory_excess*/,2,'.',','), "T", 0, 'R');
    ...
    function Less($totalExcess){
        ...
        #function to get discount
            ...
            if(!$this->prev_bill_amt)
                $netcharges = ($this->total_total_excess - $this->total_HCI_PF /*+ $this->prev_bill_amt*/) - $this->deposit_amount - $this->copay_amount ;
            else
                $netcharges = ($this->total_total_excess - $this->total_HCI_PF + $this->prev_bill_amt) - $this->deposit_amount - $this->copay_amount ;
            if ($netcharges < 0){
                 $netcharges = 0;
