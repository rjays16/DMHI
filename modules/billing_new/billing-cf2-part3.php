...
class PhilhealthForm2Part3 extends TCPDF {
    ...
    function getBillingDetails(){
		...
		if ($result = $db->Execute($sql)){ 
			if ($result->RecordCount()) {
                ...
                $this->total_hci_discount_new = $row['total_hci_discount_new'];
    ...
    * @return date
    **/
	function getCalculateDate($bill_dte, $status)
	{
    	...
    	if($status == "add3days"){
    		$date_orig->add(new DateInterval('P3D'));
    	}
		
		$t = $date_orig->format("U"); //get timestamp
		// var_dump($date_orig); die();
...
function addProfessionalFees($doctors){
 		$prof_fee_label = '10. Accreditaion Number/Name of Accredited Health Care Professional/Date Signed and Professional Fees/Charges):';
 		$col_header = array('Accreditation Number/Name of Accredited Health Care Professional/Date Signed and Professional Fees/Charges:',
 			'Details');
		...
		foreach ($doctors as $dkey => $row) {
				...
				$datenow = $this->getCalculateDate($this->bill_dte, "add3days");
	...
	function addPart3(){
		...
		$part3_sub = 'NOTE: Member/Patient should sign only after the applicable charges have been filled-out';
		...
		$a_ln5 = 'The benefit of the member/patient was completely consumed prior to co-pay OR the benefit of the member/patient is not completely consumed BUT with purchase/expenses for drugs/medicines,supplies,diagnostics and others. ';
		...
		$b_ln1 = array('I hereby consent to the submission and examination of the patient\'s pertinent medical records for the  purpose of verifying the veracity of this claim to effect efficient processing of benefit payment.','I hereby hold PhilHealth or any of its officers, employees and/or representatives free from any and all legal liabilities relative to the herein-mentioned consent which I have voluntarily', 'and willingly given in connection with this claim for reimbursement before PhilHealth.');
		...
		$relationship_label = array('Relationship of the representative to the member/patient:','Spouse','Sibling','Child', 'Parent','Others, Specify');
		$reasons_label = array('Reasons for signing on behalf of the member/patient:', 'Patient is Incapacitated', 'Other Reasons:');
		...
		$this->SetFont($this->fontfamily_answer, $this->fontstyle_answer, $this->fontsize_answer_check);
		$isDiscounted = $this->getTotalAppliedDiscounts();
		if(($this->excess <= 0 || (($this->memcategory_id == HSM || $this->memcategory_id == SM) && $this->charity)) && ($this->total_outside <= 0)){
			...
			if($isDiscounted){
				$this->total_hci_discount = ($this->total_hci_discount_new * $isDiscounted); 
				$this->total_doc_discount = ($this->total_doc_charge * $isDiscounted);
			}
			$check_value = '';
			$check_value2 = '/';
		...
		if($total_hci_excess < 0){
			$total_hci_excess= "0.00";
		}
		...
		if($total_doc_excess < 0){
			$total_doc_excess = "0.00";
		}
		...
		$this->Ln($this->rheight2);
	 	$this->SetFont($this->fontfamily_label.'bd', $this->fontstyle_label_italicized, $this->fontsize_label3);
		...
		$this->SetX($xx3+5);
	 	$this->SetFont($this->fontfamily_sans.'b', $this->fontstyle_label_italicized, $this->fontsize_label3-1);
		$this->Multicell(190, $this->rheight2, $b_ln1[0] ,$this->withoutborder, $this->alignLeft);
		$this->Ln($this->rheight6-5.5);
		$this->SetFont($this->fontfamily_sans.'b', $this->fontstyle_label_italicized, $this->fontsize_label3-1);
		$this->SetX($xx3+5);$this->Multicell(190, $this->rheight2, $b_ln1[1].$b_ln1[2] ,$this->withoutborder, $this->alignLeft);
	...
	function addPart4(){
		...
		$this->Ln();
		$this->SetFont($this->fontfamily_sans."bi", $this->fontstyle_label_bold_italicized, $this->fontsize_label-0.8);
		...
		$add_days = 10;
		$date = $this->getCalculateDate($this->bill_dte, "add3days");
	...
	function pageBoarders(){
		...
		$this->Rect(($x*20)+48, 5, 1, $this->totwidth+119, 'DF', "",  array(FFF, FFF, FFF));	//right
	}
