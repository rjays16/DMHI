...
function addProfessionalFees(){
		...
		$x=$this->GetX();
		$y=$this->GetY()-$this->lineAdjustment*300;
 		...
 		if($result)
		{
			while($row=$result->FetchRow())
			{
				...
				$this->Cell($width-$this->GetStringWidth($doc_name)+20, $this->rheight*2, strtoupper($doc_name),$this->withoutborder, $this->continueline, $this->alignCenter);
				//signature sub text
				$this->SetFont($this->fontfamily_label, $this->fontstyle_label_normal, $this->fontsize_label5);
				...
				$this->SetXY($x,$y);
				$add_days = 10;
				$datenow = date('mdY',strtotime($this->bill_dte) + (24*3600*$add_days));
	...
	function addPart3(){
		...
		$this->sety($yy2);
		$this->setx($x1+38);
		$this->Cell(20, $this->rheight2+18," ",$this->withborder,'C');
		...
		$this->Cell(38, $this->rheight2+18, "",$this->withborder,'C');

		$this->sety($yy2);
		$this->setx($x1+96);
		//$this->Cell(22, $this->rheight2+18,$total_hci_coverage,$this->withborder,'C');
		$this->Cell(22, $this->rheight2+18,"",$this->withborder,'C');
		...
		$this->Cell(20, $this->rheight2+17,"",$this->withborder,'C');
		...
		$this->Cell(38,$height , "",$this->withborder,'C');


		$this->sety($yy2);
		$this->setx($x1+96);
		//$this->Cell(22, $this->rheight2+17,$total_doc_coverage,$this->withborder,'C');
		$this->Cell(22, $this->rheight2+17," ",$this->withborder,'C');
	...
	function addPart4(){
		...
		$add_days = 10;
		$date = date('mdY',strtotime($this->bill_dte) + (24*3600*$add_days));
	...
	function addName(){
		...
		$this->SetFont($this->fontfamily_answer, $this->fontstyle_label_italicized, $this->fontsize_answer_cert);
		$this->Cell($this->totwidth, $this->rheight, strtoupper($addname), $this->withoutborder, $this->nextline, $this->alignLeft);
