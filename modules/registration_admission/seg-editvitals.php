...
*/
 ...
 $smarty->assign('sToolbarTitle',"$title :: Edit");
...
<?php
    ...
    if($_POST["submitted"]){
        ...
        $vitals_obj->where = "vitalsign_no=".$db->qstr($_POST['vitalno']);
        $saveok=$vitals_obj->updateDataFromInternalArray($PNo,FALSE);
        
        $smarty->assign('sMsgTitle','Vital signs successfully saved!'); 
            //$PNo = $dataVital['or_no'];
            
        $fetchVits = $vitals_obj->fetchVitalsDetails($_POST['vitalno']);
        ...
        $fetchVits = $vitals_obj->fetchVitalsDetails($_GET['vitalno']);
    ...
    $smarty->assign('sVitCalendarInput','<span id="show_vitdate" class="jedInput" style="margin-left:0px; font-weight:bold; color:#0000c0; padding:0px 2px;width:80px; height:24px">'.($submitted ? date($fulltime_format,strtotime($_POST['vitdate'])) : $vitalRow['date']).'</span><input class="jedInput" name="vitdate" id="vitdate" type="hidden" value="'.($submitted ? date($dbtime_format,strtotime($_POST['vitdate'])) : $vitalRow['date']).'" style="font:bold 12px Arial">');
    ...
    $smarty->assign('sVitbp','<input class="segInput" id="vital_bp_sys" name="vital_bp_sys" type="text" size="5" style="padding-left:4px;font:bold 14px Arial;" value="'.number_format($vitalRow['systole'],2).'" onkeydown="return key_check(event, this.value)" onblur="convertNumberValue(this, this.value);"/><span> /  </span><input class="segInput" id="vital_bp_dias" name="vital_bp_dias" type="text" size="5" style="padding-left:4px;font:bold 14px Arial;" value="'.number_format($vitalRow['diastole'],2).'" onkeydown="return key_check(event, this.value)" onblur="convertNumberValue(this, this.value);"/>');
    $smarty->assign('sVitt','<input class="segInput" id="vital_t" name="vital_t" type="text" size="18" style="padding-left:4px;font:bold 14px Arial" value="'.number_format($vitalRow['temp'],2).'" onkeydown="return key_check(event, this.value)" onblur="convertNumberValue(this, this.value);"/>');
    $smarty->assign('sVitw','<input class="segInput" id="vital_w" name="vital_w" type="text" size="18" style="padding-left:4px;font:bold 14px Arial" value="'.number_format($vitalRow['weight'],2).'" onkeydown="return key_check(event, this.value)" onblur="convertNumberValue(this, this.value);"/>');
    $smarty->assign('sVitrr','<input class="segInput" id="vital_rr" name="vital_rr" type="text" size="18" style="padding-left:4px;font:bold 14px Arial" value="'.number_format($vitalRow['resp_rate'],2).'" onkeydown="return key_check(event, this.value)" onblur="convertNumberValue(this, this.value);"/>');
    $smarty->assign('sVitpr','<input class="segInput" id="vital_pr" name="vital_pr" type="text" size="18" style="padding-left:4px;font:bold 14px Arial" value="'.number_format($vitalRow['pulse_rate'],2).'" onkeydown="return key_check(event, this.value)" onblur="convertNumberValue(this, this.value);"/>');
...
-->
    ...
    <input type="hidden" name="vitalsign_no" id="vitalsign_no" value="<?= $_GET['vitalno'] ?>">
    <input type="hidden" name="vitalno" id="vitalno" value="<?= $_GET['vitalno'] ?>">
...
$smarty->assign('sContinueButton','<img type="image" name="btnSubmit" id="btnSubmit" src="'.$root_path.'images/btn_submitorder.gif" align="center" style="cursor:pointer" onclick="if (confirm(\'Update this vital sign?\')) document.inputform.submit()">');
