...
function prepareSelect(id) {
	...
	var comp_id = $('comp_id'+id).value; /*added by mai 07-15-2014*/
    ...
    var check_ExceedPhicValue = $('ExceedPhicValue'+id).value;
	...
	/** End omick **/
	if (var_company)
		window.parent.$(var_company).value = company;
    ...
    var ExceedPhic = window.parent.$('ExceedPhic');
    var ExceedPhicCheckbox = window.parent.$('PhicExceed');
    if(check_ExceedPhicValue){
    	if(dept_area){
		ExceedPhic.innerHTML = "Already Exceed Phic coverage";
    	ExceedPhicCheckbox.checked=true;
    	}else{
...
var showcompany = window.parent.$('company');
	if (showcompany) {
		if (company)
			showcompany.innerHTML = company;
		else
			showcompany.innerHTML = "unknown";
	}
	...
	var grant_type = window.parent.$('grant_type');
	grant_type = grant_type ? grant_type : chargetype;
	type_charge = type_charge ? type_charge : chargetype;
	
	if(window.parent.$('comp_id')){
		window.parent.$('comp_id').value = comp_id;
	}
	
	if(grant_type){
		// alert("ss");
		// window.parent.$('charge_comp_balance').value = $('charge_comp_balance'+id).value;
		// window.parent.$('charge_comp_max').value = $('charge_comp_max'+id).value;
		// window.parent.$('comp_name').innerHTML = company;

		for (var i=0; i<grant_type.length; i++){
		  if (grant_type.options[i].value == 'company' ){
		     	grant_type.remove(i);
		     	break;
		     }
  		}
		...
		if(company){
			...
			grant_type.add(option);
			window.parent.$("iscash0").checked = true;
			window.parent.$("iscash1").checked = false;
			if(grant_type != chargetype){
				window.parent.$('is_cash').value = 0;
			}
			grant_type.value = "company";
			type_charge.style.display='';  
		}
	}
	/*end added by mai*/
...
function addPerson(listID, details) {
	...
	details = Object.extend({
		...
		sex: '',
		addr: '',
		company: '', /*added by mai 07-07-2014*/
		charge_comp_balance: '', /*added by mai 07-08-2014*/
		charge_comp_max: '', /*added by mai 07-08-2014*/
		comp_id: '', /*added by mai 07-15-2014*/
		zip: '',
		status: '',
		nr: '',
		type: '',
	...
	var id=details.id,
			...
			company=details.company, /*added by mai 07-07-2014*/
			charge_comp_balance=details.charge_comp_balance, /*added by mai 07-08-2014*/
			charge_comp_max=details.charge_comp_max, /*added by mai 07-08-2014*/
			comp_id=details.comp_id, /*added by mai 07-08-2014*/
			nr=details.nr,
			type=details.type,
			...
			ExceedPhicValue = details.HasExceedPhic,
		...
		if (id) {
			...
			rowSrc = '<tr>'+
									'<td>'+
										...
										'<input type="hidden" id="company'+id+'" value="'+company+'">'+ /* added by mai 07-08-2014*/
										'<input type="hidden" id="charge_comp_balance'+id+'" value="'+charge_comp_balance+'">'+ /* added by mai 07-08-2014*/
										'<input type="hidden" id="charge_comp_max'+id+'" value="'+charge_comp_max+'">'+ /* added by mai 07-08-2014*/
										'<input type="hidden" id="comp_id'+id+'" value="'+comp_id+'">'+ /* added by mai 07-08-2014*/
										...
										'<input type="hidden" id="company'+id+'" value="'+company+'">'+
                                        ...
                                        '<input type="hidden" id="ExceedPhicValue'+id+'" value="'+ExceedPhicValue+'">'+
