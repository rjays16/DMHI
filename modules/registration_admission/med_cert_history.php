...
$thisfile=basename(__FILE__);
 // $pid = $_GET["pid"];
 $pid = $persObj->getpidfrmenc( $_GET["enc"]);
 if($pid)
 	$pid=$pid; 
 else
 	$pid = $_GET['pid'];
		 ...
		 if($person=$result->FetchRow())
		 {
				 ...
				 if (!empty($person["name_first"])) {
						if (!empty($patient_name))
								...
								// $patient_name = $patient_name." ".$person["suffix"];
						$patient_name = $patient_name.$person["name_first"]."&nbsp;".$person["suffix"];
				 }
...
function NewMedCert(enc){
		return overlib(OLiframeContent('med_cert_encounters.php?pid='+enc, 790, 380, 'historyList', 1, 'auto'),
										WIDTH, 380, TEXTPADDING, 0, BORDER, 0, STICKY, SCROLL, CLOSECLICK, MODAL, CLOSETEXT, '<img src=../../images/close.gif border=0 onClick="startAJAXSearch(\'search\', 0, '+enc+');">',
