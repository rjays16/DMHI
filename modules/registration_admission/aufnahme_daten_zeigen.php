...
// $isTempOrFBill = $encounter_obj->isTempOrFBill($encounter_nr);
// if($isTempOrFBill)
// $updatefile='';
// else
...
if (($encounter_type==3)||($encounter_type==4)){
	...
	if ($encounter_type!=2){
		...
		$list='title,name_first,name_last,suffix,name_2,name_3,name_middle,name_maiden,name_others,date_birth,
	...
	*/
	...
	function viewClinicalForm(formID){
		if (formID==1){
			...
			window.open("../../modules/registration_admission/certificates/show_cover_sheet_jasper.php?encounter_nr="+<?=$encounter_nr?>+"&showBrowser=1","viewClinicalForm","modal, width=600,height=400,menubar=no,resizable=yes,scrollbars=no");
		...
		return overlib(
          OLiframeContent('../../modules/registration_admission/manual_coverage.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&is_doctor=1&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>', 
          						...
          						WIDTH,450, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="">',
						         CAPTIONPADDING,2, CAPTION,'Manual Entry of Coverage',
						         MIDX,0, MIDY,0, 
						         STATUS,'Manual Entry of Coverage');			
	}


		function set_confinement(){
		return overlib(
          OLiframeContent('../../modules/registration_admission/set_confinement.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&is_doctor=1&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>', 
		  						300, 100, 'fGroupTray', 0, 'auto'),
          						WIDTH,300, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="">',
						         CAPTIONPADDING,2, CAPTION,'Set Confinement Type',
						         MIDX,0, MIDY,0, 
						         STATUS,'Confinement Type');			
	}
		...
		if (ptype=='ipd' || ptype=='er')
				return overlib(
					...
					// 					 STATUS,'Operating Room Request');
					    ...
					    OLiframeContent("../../index.php?r=or/orRequest/orForm/pid/"+pid+"/enc_no/"+enc+"", 
		  						800, 440, 'fGroupTray', 1, 'auto'),
          						WIDTH,800, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="">',
	...
	function cf4_gen() {
		...
		function removeCompany(){
			var remove = confirm("Are you sure you want to remove this company?");
			if(remove){
				var hasRequest = '<?=$comp_obj->checkRequest($encounter_nr);?>';
				if(hasRequest){
					alert("Unable to remove company. Request/s has been done from cost centers charged to this company.");
				}else{
					jQuery.get("seg-remove-company.php",
			        {
...
if ((($allow_ipd_user)&&($ptype=='ipd'))&&(($encounter_type==3)||($encounter_type==4))){
	...
	$smarty->assign('sDeptInput',$current_dept_name);
...
/*added by mai 07-07-2014*/
// if (($encounter_type == 2) || ($encounter_type == 3) || ($encounter_type == 1)) {
// 	$smarty->assign('segCompany',TRUE);
// 	$smarty->assign('segCompanyLabel',"Company");
// 	$comp_name = $comp_obj->getCompanyName($encounter_nr);
// 	if($comp_name){
// 		$smarty->assign('segCompanyName', $comp_name);
// 		$smarty->assign('removeCompany','<a href="javascript:removeCompany();"><img src="'.$root_path.'images/btn_delitem.gif"></img></a>');
/*		"'.$root_path.'modules/registration_admission/'.strtr('seg-remove-company.php'.URL_APPEND.'&encounter_nr='.$encounter_nr,' ',' ').'" onclick="return removeCompany();"*/
// 	}
// }
/*end added by mai*/
