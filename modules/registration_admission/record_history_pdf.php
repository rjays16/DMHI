<?php
   ...
   $pdf->Cell(0,4,'DB Record\'s History',$borderNo,$newLineYes,'C');
	...
	switch($table){
   	 case 'care_person':  $sql="SELECT name_last AS \"LastName\", name_first AS \"FirstName\", history, suffix AS \"Suffix\" FROM care_person WHERE pid='$pid'";
		 break;
		 case 'care_encounter':  $sql=$sql="SELECT p.name_last AS  \"LastName\", p.name_first AS  \"FirstName\", p.suffix AS  \"Suffix\", e.history
	...
	$name = ucwords(strtoupper($history['LastName'])).', '.ucwords(strtoupper($history['FirstName'])).' '.ucwords(strtoupper($history['Suffix']));
	$pdf->Ln($space*4);
