...
#$objResponse->addAlert("setAll: result[$text] = '$result[$text]'; result[$value] = '$result[$value]'");
	...
	function getcompany($pid=""){
		...
		$sql = "SELECT company_id as id, company_desc as typedesc from seg_type_company";

			if($result = $db->Execute($sql)){
				if($result->RecordCount()){
						$objResponse->addScriptCall("js_ClearOptions", "CompanyName");
						$objResponse->addScriptCall("js_AddOptions","CompanyName", "- Select Company -", 0);
						while($row = $result->FetchRow()){
								$objResponse->addScriptCall("js_AddOptions","CompanyName", $row['typedesc'], $row['id']);
						}
			
						$scompany = $person_obj->getcompanynamebyID($pid);
						if($scompany){
								$objResponse->addScriptCall("js_setOption", "CompanyName", $scompany);
						}else{
								//$objResponse->alert("No confinement type set as of indicated bill date!");
								$objResponse->addScriptCall("js_setOption", "CompanyName", "- Select Company -");
						}
				}else{
