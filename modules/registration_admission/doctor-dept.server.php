...
#--------------- CREATED BY VANESSA -----------------------
	...
	*/
	...
	function getConfineTypeOption($enc,$pid){
		global $db,$person;
		...
		$sql = "SELECT casetype_id as id, casetype_desc as typedesc from seg_type_case";
			...
			if($result = $db->Execute($sql)){
				if($result->RecordCount()){
						$objResponse->addScriptCall("js_ClearOptions", "confineTypeOption");
						$objResponse->addScriptCall("js_AddOptions","confineTypeOption", "- Select Confinement Type -", 0);
						while($row = $result->FetchRow()){
								$objResponse->addScriptCall("js_AddOptions","confineTypeOption", $row['typedesc'], $row['id']);
						}
						// $prevenc = trim($objBilling->getPrevEncounter($enc));
						$nConfinementId = $person->confinement_if_exist($pid, $enc);
						if($nConfinementId){
								$objResponse->addScriptCall("js_setOption", "confineTypeOption", $nConfinementId);
						}else{
								//$objResponse->alert("No confinement type set as of indicated bill date!");
								$objResponse->addScriptCall("js_setOption", "confineTypeOption", "- Select Confinement Type -");
						}
				}else{
...
function isTempOrFBill ($encounter_nr){
global $enc_obj;
$objResponse = new xajaxResponse();
$isTempOrFBill = $enc_obj->isTempOrFBill($encounter_nr);
if($isTempOrFBill['encounter_nr']){
$objResponse->addScriptCall("ajxClearOptions", 0);
$objResponse->addScriptCall("ajxAddOption",0,$isTempOrFBill['dr_name'],$isTempOrFBill['current_att_dr_nr'] )	;
$objResponse->addScriptCall("ajxClearOptions", 1);
$objResponse->addScriptCall("ajxAddOption",1,$isTempOrFBill['dept_name'],$isTempOrFBill['current_dept_nr'] )	;
