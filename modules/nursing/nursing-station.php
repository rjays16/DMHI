...
#			$smarty->assign('sBed',strtoupper(chr($j+96)));   # burn commented : September 27, 2007
			...
			if($is_patient&&($bed['encounter_nr']!="")){
				...
				if(isset($sfn)&&$sfn) $sNameBuffer = eregi_replace($sfn,'<span style="background:yellow">'.ucfirst($sln).'</span>',ucfirst($bed['name_first']).' '.ucfirst($bed['suffix']));
					else $sNameBuffer = ucfirst($bed['name_first']).' '.ucfirst($bed['suffix']);
	...
	*/
	...
	if($waitlist_count&&$edit){
		while($waitpatient=$waitlist->FetchRow()){
			...
			$TP_WLIST_BLOCK.='&nbsp;'.ucwords(strtolower($waitpatient['name_last'])).', '.ucwords(strtolower($waitpatient['name_first'])).' '.ucwords(strtolower($waitpatient['suffix'])).' '.formatDate2Local($waitpatient['date_birth'],$date_format).'</nobr></span></a><br>';
		}
	}else{
		$TP_WLIST_BLOCK='&nbsp;';
	}
