...
//added by daryl
    if ($patient['company']){
        ...
        $result_comp = $person_obj->getcompanyname($pid);
        $company = $result_comp;
    }

    if($pid){
           $company_name = $comp_obj->CheckHasCompanyPid($pid);
        }
        
        if($company_name){
             $smarty->assign('scompany_name',$company_name);#added by daryl
        }else{
             $smarty->assign('scompany_name',$company);#added by daryl
        }
    ...
    if ((($encounter_nr)||($pid))&&(!$refno)){
         ...
         $person_name = mb_strtoupper($patient['name_last']).", ".mb_strtoupper($patient['name_first']).' '.mb_strtoupper($patient['suffix']).$name_middle;
...
//added by daryl
    $get_age = specific_age($patient['date_birth']);
    $sage_ = $get_age;    
                ...
                if ($_POST['isrepeat']==1){
                    ...
                    if (($isCorrectInfo)||($allow_radiorepeat)){
                        if($refno = $radio_obj->saveRadioRefNoInfoFromArray($_POST)){
                            $rid = $radio_obj->createNewRID($_POST['pid'], $_POST['rid']);
                    ...
                    if($refno = $radio_obj->saveRadioRefNoInfoFromArray($_POST)){
                        ...
                        if ($pid == ""){
                            ...
                            $rid = $radio_obj->createNewRID($pid_result, $_POST['rid']);
                        }else{
                            $rid = $radio_obj->createNewRID($_POST['pid'], $_POST['rid']);
                        }              
                        if($rid == 0){
                            $smarty->assign('sysInfoMessage',"File Number is already taken.");
                        }
                        else{
                        #added by VAN 06-02-2011
                        if ($_POST['for_manual_payment']){
                         ...
                         $saveok1=$comp_obj->saveChargetoCompanyTransaction($_POST); /*added by mai 07-15-2014*/    
                        if($saveok1){
                            $smarty->assign('sysInfoMessage',"Radiological Request Service successfully created.");
                        }else{
                            $smarty->assign('sysErrorMessage',"Unable to add request.");
                        }
                        }
                    }else{
                        $smarty->assign('sysErrorMessage','<strong>Error:</strong> '.$radio_obj->getErrorMsg());
...
if ($ifwalk > 0){
 ...
 $onLoadJS='onLoad="preset();checkCash();setchargecomp();setphic();"';
...
<?php
    ...
    $smarty->assign('sRID','<input class="segInput" id="rid" name="rid" type="text" size="15" value="'.$rid.'" maxlength="7" style="font:bold 12px Arial; float:left;">');
    $smarty->assign('sRIDbtn', '<img name="getRID" id="getRID" align="middle" src="'.$root_path.'images/cashier_refresh.gif" border="0" style="float:left; padding:3px; cursor:pointer" onclick="getLatestRid();">');
 ...
 if ($person_name == ""){
    ...
    $smarty->assign('sOrderName','<input class="segInput" id="ordername" name="ordername" type="text" size="40" value="'.$person_name.'" style="font:bold 12px Arial;" readonly>');#edited by daryl
    ...
    $smarty->assign('sOrderAddress','<textarea class="segInput" id="orderaddress" name="orderaddress" cols="37" rows="2" style="font:bold 12px Arial" readonly>'.$orderaddress.'</textarea>');#edited by daryl

    $smarty->assign('sDiscountShow','<input type="checkbox" name="issc" id="issc" onclick="seniorCitizen();" '.(($_POST["issc"])?'checked="checked" ':'').' '.(($is_cash=="0")?'disabled="disabled"':'').'><label for="issc" style="font:bold 11px Tahoma;">Senior citizen</label>');
    ...
    $smarty->assign('sIsCash','<input class="jedInput" type="radio" name="iscash" id="iscash1" value="1" '.(($is_cash!="0")?'checked="checked" ':'').' onclick="if (warnClear()) { changePrice(); changeTransactionType(); return true;} else {return false;}" /><label for="iscash1" class="jedInput">Cash</label>');
    $smarty->assign('sIsCharge','<input class="jedInput" type="radio" name="iscash" id="iscash0" value="0" '.(($is_cash=="0")?'checked="checked" ':'').' onclick="if (warnClear()) { changePrice(); changeTransactionType(); return true;} else return false;" style="margin-left:10px" /><label class="jedInput" for="iscash0">Charge</label>');
  ...
  if ($person_name == ""){
       $person_name = $_POST["ordername"];;
        ...
        $person_name =  $person_name ;
              ...
              // $smarty->assign('sOrderName','<input class="segInput" id="ordername" name="ordername" type="text" size="30" style="font:" value="'.$person_name.'"  onfocus="autoSuggestWalkin(this)" autocomplete="off"/>');
                $smarty->assign('sOrderAddress','<textarea class="segInput" id="orderaddress" name="orderaddress" cols="37" rows="2" style="font:bold 12px Arial" readonly>'.$getname['orderaddress'].'</textarea>');#edited by daryl
        ...
        if ($getname['date_birth']!='0000-00-00'){
            ...
            $sex = 0;
                ...
                $smarty->assign('sPatientAge',$getname['age']);
                $smarty->assign('sPatientSex',$gender);
                $smarty->assign('sPatientBdate',$dob_walk);
    ...
    $smarty->assign('sPE','<input type="hidden" '.(($is_pe==1)?'checked="checked" ':'').' name="is_pe" id="is_pe" onchange="" '.(($is_personnel)?'':'disabled="disabled" ').' value="1" />');
    ...
    #$result = $enc_obj->getChargeType("WHERE id NOT IN ('paid','phs','charity','cmap','lingap')","ordering"); //commented by ken 1/17/2014
    $result = $enc_obj->getChargeType("WHERE id IN ('phic','others', 'company')","ordering"); /*added by mai*/
    ...
    if($encounter_nr){
        ...
        $trans_amount = $comp_obj->hasChargeToCompany($encounter_nr, 'RAD', $refno);
    ...
    while ($row=$result->FetchRow()) {
        if ($grant_type==$row['id'] || ($row['id']=="company" && $trans_amount)){
            $checked = "selected";
        }else{
            $checked = "";
        }

        if(!$comp_id && $row['id']!="company"){
                                ...
                                "<select class=\"jedInput\" name=\"grant_type\" id=\"grant_type\" ".$disabled." 
                                     onchange=\" if (warnClear()) { emptyTray(); changeChargeType(); return true;} else {return false;}\">
                                         $options
                                            ...
                                            <input type='hidden' id='comp_id' name='comp_id' value='".$comp_id."'/>"); /*edited by mai added hidden field remaining balance company*/
    ...
    $smarty->assign('sManualCheck','<input type="hidden" name="for_manual" id="for_manual" value="1" '.(($row_manual_count)?'checked="checked" ':'').' onClick="setManualPayment();" />
    ...
    $smarty->assign('sBtnCoverage','<input type="hidden" name="btn-coverage" id="btn-coverage" src="'.$root_path.'images/btn_coverage.gif" border="0" style="cursor:pointer;" onclick="return openCoverages();">');
   ...
   #end pol
    if($encounter_nr){
            $HasExceedPhic = $enc_obj->HasExceedPhic($encounter_nr);
            if($HasExceedPhic){
                $smarty->assign('sCheckIfExceed', '<input id="PhicExceed" name="PhicExceed" type="checkbox" value ="" onclick="IfPhicExceed();" checked>');
                $smarty->assign('sWarningPhicExceed', 'Already Exceed Phic coverage');
            }else{
                $smarty->assign('sCheckIfExceed', '<input id="PhicExceed" name="PhicExceed" type="checkbox" value ="" onclick="IfPhicExceed();">');
            }
        }else{
            $smarty->assign('sCheckIfExceed', '<input id="PhicExceed" name="PhicExceed" type="checkbox" value ="" onclick="IfPhicExceed();">');
        }
...
<!--added by celsy 08/24/10 -->
     <input type="hidden" id="mode" value="<?=$mode?>"> <!-- added by mai 07-15-2014 -->
