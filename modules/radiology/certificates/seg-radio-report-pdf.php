...
require_once $root_path.'classes/dompdf/dompdf_config.inc.php';
...
#added by daryl
include_once($root_path.'include/care_api_classes/class_radiology.php');
$radio_obj=new SegRadio;

if ($pid){
    $pid_walk_ = $radio_obj->selectifwalkin($pid);
    $pid_walk  = $pid_walk_->FetchRow();
    $pid_walkin = $pid_walk['ifwalk'] ;
...
if ($pid_walkin == 1){
    // $gender = $getwalkin['sex'];
    $segBdateAge = "unspecified";

    $getwalkin_ = $radio_obj->selectwalkin($pid);
    $getwalkin = $getwalkin_->Fetchrow();
    $name_first = $getwalkin['name_first'];
    $name_middle = $getwalkin['name_middle'];
    $name_last = $getwalkin['name_last'];
    $sAddress = $getwalkin['orderaddress'];

        if ($getwalkin['sex']=='f'){
        $gender = "female";
        }else if($getwalkin['sex']=='m'){
            $gender = "male";
        }else{
            $gender = "unspecified";
        }
...
// Hospital Logo
$pdf->Image($root_path.'gui/img/logos/dmhi_logo.jpg',35,2,15,15);
...
$pdf->Cell(25, 3 ,'File Number: ', "",0,'');
$pdf->SetFont($fontStyle,"B", $fontSizeLabel+1);
$pdf->Cell(10, 3 ,$rid, "",0,'');
...
$pdf->Cell(12, 3 , 'Sex :', "",0,'');
$pdf->SetFont($fontStyle,"", $fontSizeLabel);
$pdf->Cell(50, 3 ,strtoupper($gender), "",0,'');
...
foreach($batchNrArrayInfo as $batchNrInfo){
    ...
    $pdf->SetFont($fontStyle,"", $fontSizeLabel);
    #edited by VAN 07-11-08
    $pdf->Cell(50, 3 ,"Reference # ".$refno, "",0,'');
    ...
    $pdf->SetFont($fontStyle,"", $fontSizeLabel);
    //print_r($findings);
    $findings = str_replace("cellspacing=\"1\" cellpadding=\"1\" border=\"1\" ",
                            "cellspacing=\"0\" cellpadding=\"0\" border=\"1\" width=\"\" style=\"border-collapse: collapse;\" ",$findings);

    $findings = str_replace("<td",
                            "<td style=\"padding:5px;border: 1px solid black;\" ",$findings);
        // $pdf->WriteHTML('<strong>'."               $findings");
    ...
    $pdf->SetFont($fontStyle,"B", $fontSizeLabel);
    $radio_impression_final = str_replace("cellspacing=\"1\" cellpadding=\"1\" border=\"1\" ",
                            "cellspacing=\"0\" cellpadding=\"0\" border=\"1\" width=\"\" style=\"border-collapse: collapse;\" ",$radio_impression_final);

    $radio_impression_final = str_replace("<td",
                            "<td style=\"padding:5px;border: 1px solid black;\" ",$radio_impression_final);
    ...
    if($radio_obj->hasBatchNR($batch_nr,count($findings_array))){
        ...
        for($x=0;$x<=2;$x++){
            if($docs[$x] != ''){
                ...
                while($row_pr = $rs_pr->Fetchrow()){
                    ...
                    $pos =  mb_strtoupper(trim($row_pr['job_function_title'])); 
                    ...
                    $dr_arrys[] = array( 'dr_info' => mb_strtoupper($row_pr['dr_name']).", ".$row_pr['drtitle']."<br>".$row_pr['job_function_title'] );      
       ...
       $dr_arrys[] = array( 'dr_info' => mb_strtoupper(mb_convert_encoding($doctors_final, "ISO-8859-1", 'UTF-8')) ); 
    }
    ...
    $pdf->SetFont($fontStyle,"", $fontSizeLabel);
    $y = $pdf->getY();
    $pdf->MultiCell(0,5,mb_strtoupper($Fcell),0,'C',0);
    $pdf->MultiCell(0,5,mb_strtoupper($cellpos),0,'C',0);               
    $pdf->Ln(8);
    ...
    // $pdf->Ln(8);
    // $pdf->Cell(140, 3 , '', "", 0,'');
    // $pdf->Cell(0, 3 ,$rValue.date("F d, o")." REVO", "", 0,'');
    $html = '<html xmlns="http://www.w3.org/1999/xhtml">
            ...
            <body>
            <center>
            <font style="font: 12pt arial, sans-serif;font-weight:bold">'.$hospitalInfo['hosp_name'].'</font><br>
            <font style="font: 10pt arial, sans-serif;font-weight:bold">'.$hospitalInfo['hosp_addr1'].'</font><br>
            <font style="font: 12pt arial, sans-serif;font-weight:bold">Department of Radiological & Imaging Sciences</font>
            <img style="position:absolute;top:-2px;left:80px" src='.$root_path.'gui/img/logos/dmhi_logo.jpg width="60">
            </center>
            <table border=0 class="header_table" cellspacing="-1">
                <tr>
                    <td width=80><b>File Number : </b></td>
                    <td width=270><b>'.$rid.'</b></td>
                    <td width=70>HRN : </td>
                    <td>'.$pid.'</td>
                </tr>
                <tr>
                    <td>Patient : </td>
                    <td>'.strtoupper($name_last.', '.$name_first.' '.$suffix.' '.$name_middle).'</td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan=4 align=center><font style="font: 12pt arial, sans-serif;font-weight:bold">'.$report.'</font></td>
                </tr>
                <tr>
                    <td colspan=4>Clinical Indication/Impression : '.$clinical_info.'</td>
                </tr>
                <tr>
                    <td colspan=2 align=right>Reference # '.$refno.'</td>
                    <td colspan=2 align=right><b><i>'.$result.'</i></b></td>
                </tr>
                <tr>
                    <td colspan=4>'.strtoupper($service_name).'</td>
                </tr>
                <tr>
            ...
            <br>
            ';
    $html .= $findings.'<br><br>IMPRESSIONS :<br>'.$radio_impression_final;        
    $html .= '<br><br><br>
            <table border=0 class="footer_table" cellspacing="">
                <tr>';
                    foreach ($dr_arrys as $val_dr) {
    ...
    $html .=   '</td>
            ...
            <table border=0 class="footer_table" cellspacing="" class="footer_table">
            <br>
                <tr>
                    <td width=350> </td>
                    <td><font style="font-size:10px">Encoded by : '.strtoupper($findings_encoder).'</font></td>
                </tr>
                <tr>
                    <td></td>
                    <td><font style="font-size:10px">Date Encoded : '.strtoupper($findings_date_final).'</font></td>
                </tr>
            </table>
            </body>
            </html>';
    ...
    // echo $html;exit();  
    $filename = 'radiology_result';
    $dompdf = new DOMPDF();
    $pdf->pdf_create($html,$filename,'A4','portrait');
}
