			...
			<tr>
				<td style="font:bold 12px Arial; background-color:#e5e5e5; color: #2d2d2d" >
					<div style="padding:4px 2px; padding-left:10px; ">
						...
						<?php 
						//echo $_POST['dept_nr'];
							$all_section = &$radio_obj->getRadioServiceGroups("department_nr = '164'");
							//echo $dept;
							while($res=$all_section->FetchRow()){
								$sTemp1 = $sTemp1.'
									<option value="'.$res['group_code'].'" ';
									$sTemp1 = $sTemp1.'>'.$res['name'].'</option>';
							}
						?>
						<select name="sect_nr" id="sect_nr" onChange="document.getElementById('search').value=''; startAJAXSearch('search',0);">
							<!--<option value="0">Select a Department</option>-->
							<?php
								echo $sTemp1;
							?>
