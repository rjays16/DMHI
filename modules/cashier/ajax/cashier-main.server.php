...
function addPFOItem($src, $ref, $code, $qty, $amount, $doctor="", $or='') {
	...
	if (in_array(strtolower($src),array('other','pp','fb','doctor','com','sp'))) { //updated by mai 07/24/2014
		$info = $cClass->GetPFOItemInfo($src, $ref, $code, $or);
#		$objResponse->addAlert(print_r($info,TRUE));
#		return $objResponse;
		if ($info) {

			$item->src = strtolower(($src=='COM') ? 'fb' : $src); //updated by mai 07/24/2014
			$item->payerType = (($src =='COM') ? 'company' : 'patient'); //added by mai 07/24/2014
			$item->ref = $ref;
			$item->id = $code;
			$item->name = $info['name'];
			$item->desc = $info['desc'];
...
#		return $objResponse;
			...
			if($item->src == 'fb' || $item->src == 'com'){ //updated by mai 07/24/2014
...
function updateDocPayment($refno, $amount, $pid){
	$cc = new SegCashier;
	$objResponse = new xajaxResponse();

	#updated by janken 10/13/2014 for filtering the date only greater than august will record the prev balance 
	// $enc_frmdte = $cc->getFromDate($refno);
	$cc->updateDocPayment($refno);
	// if(strtotime($enc_frmdte) >= strtotime("2014-08-01 00:00:00")){
	// 	$amount = $amount * -1;
	// 	$cc->updatePrevBal($pid, $amount, $refno);
	// }
	return $objResponse;
}

function CheckOrDepositAmount($pid){
	$cc = new SegCashier;
	$objResponse = new xajaxResponse();
	
	$enc_nr = $cc->GetupdateEncNo($pid);
	if($enc_nr){
		$CheckORDeposit = $cc->GetOrDepositAmountDetails($pid, $enc_nr);
		if($CheckORDeposit){
			while($row = $CheckORDeposit->FetchRow()){
				$PromtOrDetails .= "Package Name (". $row['package_name'].") \nDeposit Amount (" .$row['amount'].")";
			}
			$objResponse->call("PromtORdeposit", $PromtOrDetails);
		}else{
			$objResponse->call("alertNoOrDeposit", "No OR deposit!");
		}
	}else{
		return false;
	}
