...
if ($_POST['cancelor']) {
	...
	if ($cancelOK) {
		$db->CompleteTrans();
		$smarty->assign('sysInfoMessage','Payment successfully cancelled! Please contact the Nursing Station to update the patient\'s bed');
	}
	else {
		$db->FailTrans();
		$db->CompleteTrans();
...
function editOR(orno,pid) {
	url = "seg-cashier-main.php<?= URL_APPEND ?>&clear_ck_sid=<?= $clear_ck_sid ?>&or="+orno+"&pid="+pid+"&from=recent";
	...
	while ($row=$result->FetchRow()) {
		...
		if ($row['cancel_date']) {
			...
			$edit = strtr($template, array(
				...
				'{image}' => 'cashier_edit.gif',
				'{clickFn}' => $allowEdit ?
					'editOR(\''.$row["or_no"].'\',\''.$row["pid"].'\');return false' :
					'return false;',
				'{extras}' => 'onmouseover="tooltip(\'Edit entry\')" onmouseout="nd()"'
			));
			$print = strtr($template, array(
