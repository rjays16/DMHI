...
#added by daryl
$sget_company = $cClass->get_companyname($_GET['pid']);
$company=$sget_company['company'];
#ended by daryl
...
if (isset($_POST["submitted"])) {
		...
		foreach ($requests as $req) {
			...
			if (is_array($items)) {
				foreach ($items as $i=>$item) {
                    $src_arr[] = ($_POST['payerType_'.$req][$i]=='company') ? 'com' : $srcDept;
					$ref_arr[] = $refNo;
					$svc_arr[] = $item;
	...
	if ($saveok) {
		...
		#added by janken 11/25/2014 to update the flagger in seg_pay for the transaction type
		$saveok = $cClass->updateTransType($ORNo, $_POST['search-walkin']);
	}
	...
	if($saveok){
		if($CheckIfOR){
			...
			$ORsaveOk = $cClass->UpdateDepositAmountDetails($dataOR);
			if($ORsaveOk){
				$saveok = true;
			}else{
				$saveok = false;
				$errorMsg = $cClass->sql."valve".$ORsaveOk;
			}
		}
...
if (!$submitted || !$saveok) {
	$onLoadJS="onload=\"selectPatientType();\"";
...
<?php
	...
	if ($checked_requests) {
		foreach ($checked_requests as $i=>$item) {
			if (in_array(strtolower($item['ref_source']),array('other','fb','pp', 'com', 'sp'))) { //updated by mai 07/24/2014 added com
?>
//edited by ken for splitting doctor bill 2/19/2014
	xajax.call('addPFOItem', {
		...
		parameters:['<?= $item['ref_source'] ?>','<?= $item['ref_no'] ?>','<?= $item['service_code'] ?>', '<?= $item['qty'] ?>', '<?= $item['amount_due'] ?>' , '<?= $item['doctor'] ?>', '<?= $_GET['or']?>']
	});
<?php
			}
		}
...
function changePayer(value){ //added by mai 07/29/2014
	if($('orname').value){
		var ans = confirm('Are you sure you want to change the payer type?');
		if(ans){
			// if(value=="company"){
			// 	$('partialPaymentBtn').show();
			// }else{
			// 	$('partialPaymentBtn').hide();
			// }

			clearEncounter();
			clearList('fb','0000000000');
		}
	}
...
function selectPatientType(){ //added by mai 08-18-2014
	//updated by janken 11/25/2014

	$('search-walkin').value = $('encounter_type_show').innerHTML;
	

	// var val = 'patient';
	// switch(($('encounter_type_show').innerHTML).toLowerCase()){
	// 	case 'walk-in':
	// 		val = 'walkin';
	// 		break;
	// 	case 'none':
	...
	// if($('orname').value){
	// 	$('search-walkin').value = val;
	// }else{
	// 	$('search-walkin').value = "patient";
	// }
}
...
function personSelect() {
	...
	//$('partialPaymentBtn').hide();
	switch($('search-walkin').value){ //added by mai 07-22-2014
		case "walkin":
		...
		case "company":
				searchScript = "seg-select-company.php";
			 	caption = "Select registered company";
			 	//$('partialPaymentBtn').show();
			 	break;
		case "insurance":
				searchScript = "seg-select-insurance.php";
				caption = "Select registered insurance";
			 	break;
		case "discount":
				searchScript = "seg-select-discount.php";
				caption = "Select registered discount";
			 	break;
		case "patient":
				searchScript = "seg-select-enc.php";
				caption = "Select registered person";
	...
	/*if ($('search-walkin').value == "walkin")
		searchScript = "seg-select-walkin.php";
	else
		searchScript = "seg-select-enc.php";*/ //commentted by mai 07-22-2014
...
if (isset($_POST["submitted"])) {
	if ($saveok) {
		...
		$draftPrintUrl = $root_path.'modules/cashier/reports/or_jasper.php?'.URL_APPEND."&clear_ck_sid=$clear_ck_sid&nr=$ORNo&userck=$userck";
        ...
        if($access->checkPermissionRaw('_a_1_cashiernewor')) {
        	$smarty->assign('ORprint_cashier',  '<button id="btn-launch-director" style="cursor:pointer" class="segButton"><img src="../../gui/img/common/default/printer_add.png" /> OR ( CASHIER )</button>');
        	$smarty->assign('ORprint_pharma',  '<button style="cursor:pointer" id="btn-print-draft" class="segButton"><img src="../../gui/img/common/default/printer.png" />OR ( PHARMACY )</button>');
			// $smarty->assign('sOnLoadJs',"onload=\"draftPrint('$directorUrl');\"");
        } else {
            $smarty->assign('ORprint_cashier', '<button id="btn-launch-director" disabled="disabled" class="segButton"><img src="../../gui/img/common/default/printer_add.png" />OR ( CASHIER )</button>');
            $smarty->assign('ORprint_pharma',  '<button style="cursor:pointer" id="btn-print-draft" class="segButton"><img src="../../gui/img/common/default/printer.png" />OR ( PHARMACY )</button>');
...
if ($_GET['or']) {
	...
	$get_company = $cClass->get_companyname($pay_info['pid']);
...
$smarty->assign('sBillingAdd','<img class="segSimulatedLink" src="'.$root_path.'images/btn_add_billing.gif" align="absmiddle" '.
	'onclick="openBilling()">');
$smarty->assign('sBillingDoctor','<img class="segSimulatedLink" id="doc_payment" src="'.$root_path.'images/btn_add_doc_payment.gif" style="display:none;" align="absmiddle" '.
	'onclick="docPaid()">');
...
$smarty->assign('sHospitalServiceClear','<img class="segSimulatedLink" src="'.$root_path.'images/his_clear_button.gif" align="absmiddle" onclick="if (confirm(\'Clear the list?\')) clearList(\'other\',\'0000000000\')"/>');
//poliam comment out to meet mediquest request
...
/* *************************************************************************************************
	 Service Payment
 ************************************************************************************************* */
$smarty->assign('PaymentAdd','<input class="segButton" type="button" value="Add Payment" onclick="openPayment()" />');
$smarty->assign('PaymentClear','<input class="segButton" type="button" value="Clear list" onclick="if (confirm(\'Clear the list?\')) clearList(\'sp\',\'0000000000\')" />');
$smarty->assign('PaymentList',"
<tr>
	<td colspan=\"10\">List is currently empty...</td>
</tr>");
...
if ($pay_info['encounter_type'])  $smarty->assign('sOrderEncTypeShow',$enc[$_POST['encounter_type']]);
	...
	#updated by janken 11/25/2014 to get the right dropdown
	if ($pay_info['trans_type'])
		$smarty->assign('sOrderEncTypeShow',$pay_info['trans_type']);
...
$smarty->assign('sORName','<input class="segInput" id="orname" name="orname" type="text" size="30" readonly="readonly" value="'.$or_name.'"/>');
$smarty->assign('sORAddress','<textarea class="segInput" id="oraddress" name="oraddress" cols="27" rows="2" readonly="readonly">'.$pay_info['or_address'].'</textarea>');
$smarty->assign('sOREncNr','<input id="encounter_nr" name="encounter_nr" type="hidden" value="'.$pay_info["encounter_nr"].'"/>');
$smarty->assign('sOREncID','<input id="pid" name="pid" type="hidden" value="'.(($_GET['pid']) ? $_GET['pid'] : $pay_info['pid']).'"/>');
if ($_GET['or'] && FALSE)
...
'<select class="segInput" id="search-walkin" name="search-walkin" onclick="changePayer(this.value)">
	<option value="patient">In-Patient/Out-Patient</option>
	<option value="company">Company</option>
	<option value="insurance">Insurance</option>
	<option value="discount">Discounts</option>
	<option value="walkin">Walk-in</option>
</select>
'.'<input type="hidden" id="from" value="'.$_GET['from'].'"'); //updated by mai 07/22/2014, changed checkbox to dropdown
...
$smarty->assign('sAmtTendered',$pay_info['amount_tendered']);
...
if ($tab_enabled) {
	...
	if ($tab_enabled['fb'] || $tab_enabled['com']) $active_tab = 'billing'; //updated by mai 07/24/2014, added com
        if ($tab_enabled['sp']) $active_tab = 'copayment';
}
...
?>
	...
	<input type="hidden" name="PPTYPE" id="PPTYPE">
<?php
