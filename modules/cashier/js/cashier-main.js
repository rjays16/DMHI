...
//added by ken 6/3/2014 for company payment
function openPayment() {
	var name = $('orname').value;
	details = new Object;
	details.id = 'SERV_PAYMENT';
	details.name = name;
	details.desc = 'Service Payment';
	details.qty = 1;
	price=0;
	while (isNaN(parseFloat(price)) || parseFloat(price)<=0) {
		price = prompt("Set amount for payment:")
		if (price === null) return false;
	}
	details.origprice = price;
	details.price = price;
...
function openBilling() {
	var id = $('pid').value;
	var payerType = $('search-walkin').value; //added by mai 07/22/2014
	...
	if (!id) {
		...
		return overlib(OLiframeContent('seg-cashier-main-billinglist.php?or='+theORNo+'&patient='+id+'&payerType='+payerType, 640, 380, 'fMiscFees', 0, 'auto'),
			WIDTH,600, TEXTPADDING,0, BORDER,0,
			STICKY, SCROLL, CLOSECLICK, MODAL,
			CLOSETEXT, '<img src=../../images/close_red.gif border=0 >',
				...
				if (_replace || !$(srcRefItem)) {
					...
					if(count == 1){
						...
						if (_dflag!=0) {
							...
							if (parseFloat(_limit) < 0) {
									rowSrc += '				<td align="right" id="tdpriceorig_'+srcRefItem+'">'+
									...
									'</td>'+
									'<td align="right" style="color:'+_adjColor+'" id="tdprice_'+srcRefItem+'">'+
									...
									rowSrc+='<td align="center">'+
										'<input id="qty_'+srcRefItem+'" name="qty_'+srcRef+'[]" srcDept="'+src+'" refNo="'+ref+'" itemID="'+temp_id+'" type="hidden" '+disabledTag+' value="'+_qty+'"/>'+(_qty>1 ? (_qty) : (src=="ph" ? "1" : "&nbsp;"))+
												'</td>';
									rowSrc+='<td align="right" id="tdtotalorig_'+srcRefItem+'">'+
									...
									'</td>'+
									'<td align="right" style="color:'+_adjColor+'" id="tdtotal_'+srcRefItem+'">'+
...
function addServiceToList(details) {
	details = Object.extend({
		payerType: '', //added by mai 07/24/2014
		src: '',
		ref: '',
		id:'',
		name: '',
	...
	if (list) {
		...
		if (details.id) {
			var _id = details.id,
                _origID = details.id, //added by jasper 05/29/2013
                	_payerType = details.payerType, //added by mai 07/23/2014
					_name = details.name,
					_desc = details.desc,
					_price = details.price,
					_qty = details.qty,
			...
			if (_replace || !$(srcRefItem)) {
				...
				rowSrc += '<input id="payerType_'+srcRefItem+'" name="payerType_'+srcRef+'[]" srcDept="'+src+'" refNo="'+ref+'" itemID="'+_id+'" type="hidden" value="'+_payerType+'"/>'; //added by mai 07/24/2014
...
//				}
				...
				if (_calculate) {
				 ...
				 $('PPTYPE').value = _id; //added by daryl
				return true;
			}
			else  { // Row already exists
				return false;
...
function docPaid(value){
	ref = src_row+'d'+src_id;

	img = '<img align="absmiddle" src="../../images/flag_paid.gif" alt="paid">';

	doc_pf = $('total_'+ref).value;
	pid = $('pid').value;
	$('tdtotal_'+ref).innerHTML = img;
	$('tdtotalorig_'+ref).innerHTML = img;
	$('tdpriceorig_'+ref).innerHTML = '';
	$('tdprice_'+ref).innerHTML = '';

	xajax_updateDocPayment(src_id,doc_pf,pid);
}
function PromtORdeposit(OrDetails) {
    var id = $('pid').value;
	if (!id) {
		alert('No patient ID selected...');
		return false;
	    ...
	    details = new Object;
	    details.id = 'OR';
	    details.name = 'OR:Hospital Fees';
	    details.desc = 'OR Charge';
	    details.qty = 1;
	    price=0;
	    while (isNaN(parseFloat(price)) || parseFloat(price)<=0) {
	        price = prompt(OrDetails)
	        if (price === null) return false;
	    }
	    details.origprice = price;
	    details.price = price;
...
function ShowORdeposit(pid){
	pid = $('pid').value;
	if(!pid){
		alert('no patient ID selected...');
		return false;
	}else{
		xajax_CheckOrDepositAmount(pid);
	}

}

function alertNoOrDeposit(msg){
	alert(msg);
}