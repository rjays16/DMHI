							...
							<td width="20%" align="center" nowrap="nowrap"><strong>Select Report Type</strong>
								<select class="segInput" name="selreport" id="selreport" onchange="selOnChange()"/>
									<optgroup label="Daily reports">
										...
										<option value="dcpr">Daily Cash Position Report</option>
...
?>
					...
					<!-- added by janken 1/19/2014 -->
					<!--  Daily endorsement -->
					<table name="selOptions" width="100%" border="0" cellpadding="2" cellspacing="0" segOption="dcpr" style="font:bold 12px Arial; display:none">
						<tr>
							<td width="30%" align="right">Select encoder</td>
							<td>
								<select class="segInput" name="param" id="dcpr_encoder" paramName="encoder" segOption="dcpr">
									<?= $cashier_user_options ?>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<input class="segInput" name="param" id="dcpr_date" type="text" size="12" value="<?= $current_date ?>" paramName="date" segOption="dcpr"/>
								<img src="<?= $root_path ?>gui/img/common/default/show-calendar.gif" id="tg_dcpr_date" align="absmiddle" style="cursor:pointer;"  />
								<script type="text/javascript">
									Calendar.setup ({
										inputField : "dcpr_date", ifFormat : "<?= $phpfd ?>", showsTime : false, button : "tg_dcpr_date", singleClick : true, step : 1
									});
								</script>
							</td>
						</tr>
