...
<script type="text/javascript">
	...
	function prepareAdd(id) {
		var check_comp = "<?= $_REQUEST['payerType']?>";
        ...
        details.payerType = $('payer_'+id).value; //added by mai 07/23/2014
...
include($root_path."include/care_api_classes/class_company.php"); //added by mai 07/22/2014
$comp_obj = new Company(); //added by mai

include($root_path."include/care_api_classes/class_person.php"); //added by mai 07/22/2014
$person_obj = new Person(); //added by mai
...
if ($_POST['submitted'] || $_REQUEST['patient']) {
	...
	if($_REQUEST['payerType']=="company"){
		$result = $comp_obj->getCompanyBilling($patient);
	}else{
	...
	if ($result) {			
		...
		while ($row=$result->FetchRow()) {
            ...
            if($_REQUEST['payerType']=="company"){//added mai

				if($row['payments']!=0 && $row['payments']!=(0-$row['amount_due']) && $row['amount_due'] > $row['payments']){
					$row['request_flag'] = 'partial';
				}else if((0-$row['amount_due']) == $row['payments']){
					$row['request_flag'] = 'paid';
				}else{
					$row['request_flag'] = NULL;
				}

				$row['amount_due'] = $row['amount_due'] + $row['payments'];
				$payer="company";
				$total = $row['amount_due'];
			}else{
				//edited some functions to split doctor and hospital bill -ken 2/18/2014 
				//hospital coverage
				$HCIcoverage = $cClass->getCoverageTotal($row['encounter_nr']);
				//professional coverage
				$prof_coverage = $cClass->getProfCoverageTotal($row['bill_nr']);
				//hospital discount 
				$hbDiscount = $cClass->getHospitalDiscount($row['bill_nr']);
				//profession discount
				$discountCS = $cClass->getDiscountSC($row['encounter_nr']);
				$discountOther = $cClass->GetOtherDiscount($row['bill_nr']);
				$PfDIscount = $discountOther + $discountCS;
				//company coverage hospital
				$hospitalCompany = $comp_obj->GetAllAmountCoverage($row['encounter_nr']);
				//company coverage professional
				$ProfCompany = $comp_obj->GetAllAmountCoverage($row['encounter_nr'], $arae="PF");
				//deposite
				$deposit = $cClass->getDepositTotal($row['encounter_nr'], $row['bill_dte']);
				
				//previous balance
				// $prev_balance = $cClass->getPrevBal($row['bill_nr']);
				
				$prof_discount = $cClass->getProfDiscountTotal($row['bill_nr']);
				$profDiscount = $cClass->getProfDiscount($row['bill_nr']);
				
				
				
				$hospital = $row['total'] - ($HCIcoverage + $hbDiscount + $hospitalCompany);  //+ $prev_balance;
				$doctor = $row['doctor'] - ($prof_coverage + $PfDIscount + $ProfCompany);
				
				$temp_hospital = $hospital - $deposit;
				$temp_doctor = $doctor - $deposit;
				if($temp_hospital < 0){
					$doctor = $temp_doctor;
				}
				else
...
?>
			<tr class="<?= $class ?>">
				...
				<?php if($_REQUEST['payerType']!="company"){ ?>  <!-- added by mai 07/22/2014 -->
				<td style="color:#008000" ><?= $row["encounter_nr"] ?></td>
					...
					<input type="hidden" id="doctor_paid_<?= $count ?>" value="<?= ($row['doctor_flag'] ? "1" : "") ?>"/>
				<?php } else{?>
					<td></td>
				<?php } ?>
				...
				<td>
                    ...
                    <input type="hidden" id="payer_<?= $count?>" value="<?=$payer?>"/> <!-- added by mai 07/23/2014 -->				
				...
				<td align="center">
					...
					<!-- added by ken 2/24/2014 for displaying partial paid and paid -->
					<?php if(($row["request_flag"] && $row["doctor_flag"] && $_REQUEST['payerType']!="company") || ($row["request_flag"] && $row["doctor_flag"] && $_REQUEST['payerType']=="company")){ ?>
						<?= ("<img src=\"".$root_path."images/flag_{$row['request_flag']}.gif\" align=\"absmiddle\" />") ?>
					<?php } else if($row["request_flag"] && !$row["doctor_flag"] || !$row["request_flag"] && $row["doctor_flag"]){ ?>
						<b>Partially Paid</b>
					<?php } else{ ?>
						<?= "" ?>
					<?php } ?>
