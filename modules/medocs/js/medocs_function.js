...
//added by daryl 08/30/2014
function editAltDesc_bill(id) {
	$("descmain_edit"+id).style.display = "";
	$("descmain"+id).style.display = "none";
	$("icdCodeDesc_"+id).focus();
}
...
//added by daryl 08/30/2014
function editAltDesc_ICP(id) {
	$("descmain_edit_ICP"+id).style.display = "";
	$("descmain_ICP"+id).style.display = "none";
	$("icpCodeDesc_"+id).focus();
}

function editAltDesc_bill_ICP(id) {
	$("descmain_edit_ICP"+id).style.display = "";
	$("descmain_ICP"+id).style.display = "none";
	$("icpCodeDesc_"+id).focus();
}
...
function editAltCode(id){
	$("codemain_edit"+id).style.display = "";
	$("codemain"+id).style.display = "none";
	$("icdCode_"+id).focus();
}//ended by ken 4/10/2014
...
//added by daryl
function cancelAltDesc_ICP(id) {
	$("descmain_edit_ICP"+id).style.display = "none";
	$("descmain_ICP"+id).style.display = "";
}
//ended by daryl

//added by ken 4/10/2014
function cancelAltCode(id){
	$("codemain_edit"+id).style.display = "none";
	$("codemain"+id).style.display = "";
}//ended by ken 4/10/2014
...
function applyAltDesc(e,id,code,user_id) {
	...
	if ( (characterCode == 13) || (isESCPressed(e)) ) {
		...
		if(altDesc !="")
		{
			...
			xajax_saveAltDiagDesc_seg(enc_nr, code, altDesc, user_id);
...
//added by daryl
//08/30/2014
function applyAltDesc_ICP(e,id,code,user_id,refno='') {
	var characterCode;
	var enc_nr   = $('encounter_nr').value;
	// var user_id  = $('create_id').value;
	...
	if ( (characterCode == 13) || (isESCPressed(e)) ) {
		var altDesc =$("icpCodeDesc_"+id).value;
		if(altDesc !="")
		{
			xajax_saveAltDiagDesc_ICP(enc_nr, code, altDesc, user_id);
			$("descmain_ICP"+id).innerHTML = '<a onClick="editAltDesc_ICP(\''+id+'\')" style="cursor:pointer" >'+altDesc+'</a>';
		}		
		$("descmain_edit_ICP"+id).style.display = "none";
		$("descmain_ICP"+id).style.display = "";
	}
...
function applyAltDesc_bill(e,id,code,user_id) {
	var characterCode;
	var enc_nr   = $('encounter_nr').value;
	// var user_id  = $('create_id').value;
	...
	if ( (characterCode == 13) || (isESCPressed(e)) ) {
		...
		if(altDesc !="")
		{
			xajax_saveAltDiagDesc_bill(enc_nr, code, altDesc, user_id);
			$("descmain"+id).innerHTML = '<a onClick="editAltDesc_bill(\''+id+'\')" style="cursor:pointer" >'+altDesc+'</a>';
		}		
		$("descmain_edit"+id).style.display = "none";
		$("descmain"+id).style.display = "";
	}
...
function applyAltDesc_bill_ICP(e,id,code,user_id,refno='') {
	var characterCode;
	var enc_nr   = $('encounter_nr').value;
	// var user_id  = $('create_id').value;
	...
	if ( (characterCode == 13) || (isESCPressed(e)) ) {
		...
		if(altDesc !="")
		{
			xajax_saveAltDiagDesc_bill_ICP(enc_nr, code, altDesc, user_id, refno);
			$("descmain_ICP"+id).innerHTML = '<a onClick="editAltDesc_bill_ICP(\''+id+'\')" style="cursor:pointer" >'+altDesc+'</a>';
		}		
		$("descmain_edit_ICP"+id).style.display = "none";
		$("descmain_ICP"+id).style.display = "";
	}
...
//added by ken based on sir Dommie code for description 4/10/2014
function applyAltCode(e,id,code,user_id) {
	var characterCode;
	var enc_nr   = $('encounter_nr').value;
	// var user_id  = $('create_id').value;
	...
	if ( (characterCode == 13) || (isESCPressed(e)) ) {
		var altCode =$("icdCode_"+id).value;
		if(altCode !="")
		{
			xajax_saveAltDiagCode(enc_nr, code, altCode, user_id);
			$("codemain"+id).innerHTML = '<a onClick="editAltCode(\''+id+'\')" style="cursor:pointer" >'+altCode+'</a>';
		}		
		$("codemain_edit"+id).style.display = "none";
		$("codemain"+id).style.display = "";
	}
...
//		date_p = dateFormat(txtAdmissionDate) + " " +  atime;   // burn commmented: July 16, 2007
	...
	//if (id1x!=-1 && !(isNaN(id1x))){
		...
		if (isAdded) {
			...
			//alert('target = '+target);
			// verify_data_saving(encounter,encounter_type,date_p,id1x,aDoctor_nr,aDepartment_nr,create_id,target,type)
...
//added by daryl
//08/31/2014
function verify_data_saving(encounter,encounter_type,aDate,code,doc_nr, dept_nr,create_id,target,type){
 var root_path = '../..';
		return overlib(
          OLiframeContent(root_path+'/modules/medocs/procedure_date.php?enc='+encounter+'&enc_type='+encounter_type+'&aDate='+aDate+'&code='+code+'&doc_nr='+doc_nr+'&dept_nr='+dept_nr+'&create_id='+create_id+'&target='+target+'&type='+type+'', 
		  						450, 250, 'fGroupTray', 0, 'auto'),
          						WIDTH,300, TEXTPADDING,0, BORDER,0, 
									STICKY, SCROLL, CLOSECLICK, MODAL, 
								CLOSETEXT, '<img src='+root_path+'/images/close.gif border=0 onClick="document.location.reload(true)">',
						         CAPTIONPADDING,2, CAPTION,'Procedure Date',
						         MIDX,0, MIDY,0, 
						         STATUS,'Procedure Date');	
...
function gui_addIcdCodeRow(encounter,code,desc,target,create_id,type, doc, dept, code2,bill='') {
...
//08/30/2014
		if(bill=='billing'){
			input_type= 'hidden';
			edit_code = '';
			seditAltDesc = 'editAltDesc_bill';
			sapplyAltDesc = 'applyAltDesc_bill';
			
		}else{
			input_type= 'button';
			edit_code = 'editAltCode';
			seditAltDesc = 'editAltDesc';
			sapplyAltDesc = 'applyAltDesc';
		}
		//COMMENTED BY VAN 10-15-2008
	//	if (isNaN(code)) {
		//edit by daryl
		//11/16/2013
			...
			newRowSrc = '<tr class="wardlistrow'+(lastRowNo%2)+'" style="font-weight:'+fontweight+'" id="icdCodeRow'+lastRowNo+'"  >'+
					...
					*/
			//added by ken 4/10/2014 to show the alternative code
		//edited by daryl
			if(code2 != '0'){
				newRowSrc = '<tr class="wardlistrow'+(lastRowNo%2)+'" style="font-weight:'+fontweight+'" id="icdCodeRow'+lastRowNo+'"  >'+
						'<td>'+
							'<span id="codemain_edit'+lastRowNo+'" style="display:none;">' +
								'<input style="width:95%;"  id="icdCode_'+lastRowNo+'" type="text" name="icdCode['+lastRowNo+']" value="'+code2+'" onFocus="this.select();" onblur="cancelAltCode(\''+lastRowNo+'\');" onkeyup="applyAltCode(event,\''+lastRowNo+'\',\''+code+'\',\''+create_id+'\');">'+
								'<input type="hidden" id="icdCodeID'+lastRowNo+'" name="icdCodeID['+lastRowNo+']" value="'+code+'" >'+
							'</span>'+
							'<span id="codemain'+lastRowNo+'">' +
								'<a onClick="'+edit_code+'(\''+lastRowNo+'\')" style="cursor:pointer" >'+code2+'</a>'+
							'</span>' +
						'</td>'+
						'<td>';
			}
				...
				newRowSrc = '<tr class="wardlistrow'+(lastRowNo%2)+'" style="font-weight:'+fontweight+'" id="icdCodeRow'+lastRowNo+'"  >'+
						'<td>'+
							'<span id="codemain_edit'+lastRowNo+'" style="display:none;">' +
								'<input style="width:95%;"  id="icdCode_'+lastRowNo+'" type="text" name="icdCode['+lastRowNo+']" value="'+code+'" onFocus="this.select();" onblur="cancelAltCode(\''+lastRowNo+'\');" onkeyup="applyAltCode(event,\''+lastRowNo+'\',\''+code+'\',\''+create_id+'\');">'+
								'<input type="hidden" id="icdCodeID'+lastRowNo+'" name="icdCodeID['+lastRowNo+']" value="'+code+'" >'+
							'</span>'+
							'<span id="codemain'+lastRowNo+'">' +
								'<a onClick="'+edit_code+'(\''+lastRowNo+'\')" style="cursor:pointer" >'+code+'</a>'+
							'</span>' +
						'</td>'+
						'<td>';
			}//ended by ken 4/10/2014
						//'<input type="hidden" id="icdCodeDesc'+lastRowNo+'" name="icdCodeDesc['+lastRowNo+']" value="'+desc+'" >'+
			newRowSrc += '<span id="descmain_edit'+lastRowNo+'" style="display:none;">' +
							'<input style="width:95%;"  id="icdCodeDesc_'+lastRowNo+'" type="text" name="icdCodeDesc['+lastRowNo+']" value="'+desc+'" onFocus="this.select();" onblur="cancelAltDesc(\''+lastRowNo+'\');" onkeyup="'+sapplyAltDesc+'(event,\''+lastRowNo+'\',\''+code+'\',\''+create_id+'\');">'+
						'</span>'+
						'<span id="descmain'+lastRowNo+'">' +
							'<a onClick="'+seditAltDesc+'(\''+lastRowNo+'\')" style="cursor:pointer" >'+desc+'</a>' +
						'</span>' +
					...
					'<td align="center">'+
						...
						'<input type="hidden" id="targetIcdCode'+lastRowNo+'" value="'+target+'">'+
						'<input type="hidden" id="typeIcdCode'+lastRowNo+'" name="typeIcdCode'+lastRowNo+'" value="'+type+'">'+
						'<input type="'+input_type+'" id="icdCodeRmv'+lastRowNo+'" value="x" onclick="xajax_rmvCode(\''+type+'\',\''+encounter+'\',\''+code+'\',\''+target+'\','+lastRowNo+',\''+create_id+'\');" style="width:25px">'
					'</td>'+
					'</tr>';		
	//COMMENTED BY VAN 10-15-2008
	/*
...
function gui_addIcpCodeRow(encounter,code,desc,target,create_id,type,op_date='',num_sessions='',rvu='',multiplier='',charge='',bill='',refno='') {
	var srcTable, srcRows, srcTableBody, newRowSrc, lastRowNo;
	var i;
	//if (highlight==null) highlight=false;
	if (srcTable=document.getElementById("icpCodeTable")) {
...
//08/30/2014
		if(bill=='billing'){
			input_type= 'hidden';
			edit_code = '';
			seditAltDesc = 'editAltDesc_bill_ICP';
			sapplyAltDesc = 'applyAltDesc_bill_ICP';
			
		}else{
			input_type= 'button';
			edit_code = '';
			seditAltDesc = 'editAltDesc_ICP';
			sapplyAltDesc = 'applyAltDesc_ICP';
		}
//alert(target);
		// if (isNaN(code)){
		//if (!(isNaN(code))){
				...
				// 	'</td>'+*/
				// 	'<td align="center">'+num_sessions+'</td>'+
				// 	'<td align="center">'+op_date+'</td>'+
				// 	'<td align="center">'+rvu+'</td>'+
				...
				// 		'<input type="'+input_type+'" id="icpCodeRmv'+lastRowNo+'" value="x" onclick="xajax_rmvCode(\''+type+'\',\''+encounter+'\',\''+code+'\',\''+target+'\','+lastRowNo+',\''+create_id+'\');" style="width:25px">'
				// 	'</td>'+
				// 	'</tr>';

		newRowSrc = '<tr class="wardlistrow'+(lastRowNo%2)+'" style="font-weight:'+fontweight+'" id="icpCodeRow'+lastRowNo+'"  >'+
						'<td>'+
							'<span id="codemain_edit'+lastRowNo+'" style="display:none;">' +
								'<input style="width:95%;"  id="icpCodeID'+lastRowNo+'" type="text" name="icpCodeID['+lastRowNo+']" value="'+code+'" onclick="getType(\''+type+'\',1,'+lastRowNo+')" >'+
								'<input type="hidden" id="icpCodeID'+lastRowNo+'" name="icpCodeID['+lastRowNo+']" value="'+code+'" >'+
							'</span>'+
							'<span id="codemain'+lastRowNo+'">' +
								'<a onClick="'+edit_code+'(\''+lastRowNo+'\')" style="cursor:pointer" >'+code+'</a>'+
							'</span>' +
	...
	newRowSrc += '<span id="descmain_edit_ICP'+lastRowNo+'" style="display:none;">' +
							'<input style="width:95%;"  id="icpCodeDesc_'+lastRowNo+'" type="text" name="icpCodeDesc['+lastRowNo+']" value="'+desc+'" onFocus="this.select();" onblur="cancelAltDesc_ICP(\''+lastRowNo+'\');" onkeyup="'+sapplyAltDesc+'(event,\''+lastRowNo+'\',\''+code+'\',\''+create_id+'\',\''+refno+'\');">'+
						'</span>'+
						'<span id="descmain_ICP'+lastRowNo+'">' +
							'<a onClick="'+seditAltDesc+'(\''+lastRowNo+'\')" style="cursor:pointer" >'+desc+'</a>' +
						'</span>' +
					...
					'<td align="center">'+
						'<input type="hidden" id="enIcpCode'+lastRowNo+'" value="'+encounter+'">'+
						'<input type="hidden" id="targetIcpCode'+lastRowNo+'" value="'+target+'">'+
						'<input type="hidden" id="typeIcpCode'+lastRowNo+'" name="typeIcdCode'+lastRowNo+'" value="'+type+'">'+
						'<input type="'+input_type+'" id="icpCodeRmv'+lastRowNo+'" value="x" onclick="xajax_rmvCode(\''+type+'\',\''+encounter+'\',\''+code+'\',\''+target+'\','+lastRowNo+',\''+create_id+'\');" style="width:25px">'
					'</td>'+
					'</tr>';	
...
function gui_clearIcpCodeRows(){
	...
	// 	}else return false;
	// }else return false;
	// document.getElementById("icpCodeTable").innerHTML = "";
	//  $j('#icpCode').val("");
 //    $j('#icpDesc').val("");
    		document.getElementById('icpCode').value='';
		document.getElementById('icpDesc').value='';
...
function checkTransferredEmpty(){
	var enc = $J('#encounter_nr').val();
	var obj = document.getElementsByName('disp_code');
	for (i=obj.length-1; i > -1; i--) {
		if (obj[i].checked) {
			if(obj[i].value == '8'){
				var nr = obj[i].value;
...
function checkTransferred(res){
	if (res == '1') {
		document.entryform.submit(); 
	}else{
		alert("Cannot Save, Empty Referral Information!");
		return false;
	}
}
