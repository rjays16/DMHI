...
# Save new diagnosis and procedure to care_notes and care_encounter_diagnosis
	...
	$patient_enc_remarks = $enc_obj->getPatientEncounterRemarks($encounter_nr);
	if($encounter_type==1){
	...
	#added by daryl
	#11/6/2014
		if(isset($_POST['aux_notes'])){
			$enc_obj->UpdateAdmittionDiagnosis($encounter_nr, $_POST['aux_notes']);
		}
	#ended by daryl
		//Result
		if(isset($_POST['result_code'])){
			...
			if(!$enc_obj->getIfDoaPatient($encounter_nr)){
	...
	//Save data for Inpatient
		...
		if(isset($_POST['disp_code'])){
			...
			if($patient_enc_disp['encounter_nr']==$encounter_nr){
				...
				if(isset($_POST['inp_remarks'])){

					$sremarks = $_POST['inp_remarks'];

			if($patient_enc_remarks['encounter_nr']==$encounter_nr){
				$remarks['remarks']=$_POST['inp_remarks'];
				$remarks['modify_id'] = $_SESSION['sess_user_name'];
				$remarks['modify_time']=date('Y-m-d H:i:s');
				// $remarks['create_id']= $_SESSION['sess_user_name'];
				// $remarks['create_time']=date('Y-m-d H:i:s');
				$enc_obj->setDataArray($remarks);

				if(!@$enc_obj->updateEncounterRemarks($patient_enc_remarks['encounter_nr'])) echo "<br>$LDDbNoSave";
			}else{
				$remarks['encounter_nr'] = $encounter_nr;
				$remarks['remarks']=$_POST['inp_remarks'];
				$remarks['modify_id'] = $_SESSION['sess_user_name'];
				$remarks['modify_time']=date('Y-m-d H:i:s');
				$remarks['create_id']= $_SESSION['sess_user_name'];
				...
				if(!@$enc_obj->saveEncounterRemarks($remarks)) echo "<br>$LDDbNoSave";
			}
	...
	// Save data for OPD patient
		...
		if ((isset($HTTP_POST_VARS['isdischarge']))&&($HTTP_POST_VARS['isdischarge'])){
			...
			//echo "patient is discharged";
				...
				$curl_obj->customerUpdateDischarge($encounter_nr,$enc_date,$HTTP_POST_VARS['date'],$HTTP_POST_VARS['time'], $pid);
			}
		}
   ...
   // $bill_nr =$enc_Info['bill_nr'];
	...
	if ($result_diagnosis = $objDRG->getDiagnosisCodes($encounter_nr,$encounter_type)){
		...
		while($temp=$result_diagnosis->FetchRow()){
			//added by ken 4/10/2014 to reflect the alternative code and description in medocs
			if($temp['code_alternative'])
				$temp['code'] = $temp['code_alternative'];
			if($temp['diagnosis_description'])
				$temp['diagnosis'] = $temp['diagnosis_description'];
	...
	//to reflect diagnosis encoded both medical records and billing
if ($result_diagnosis = $objDRG->get_DiagnosisCodes($encounter_nr)){
		$result_seg['diagnosis_principal']='';
		$result_seg['diagnosis_others']='';
		$rowsDiagnosis = $result_diagnosis->RecordCount();
		while($temp=$result_diagnosis->FetchRow()){
			//added by ken 4/10/2014 to reflect the alternative code and description in medocs
			if($temp['code_alt'])
				$temp['diag_code'] = $temp['code_alt'];
			if($temp['diagnosis_description'])
				$temp['diagnosis'] = $temp['diagnosis_description'];
			//ended by ken 4/10/2014
			if ($temp['type_nr'])
				$result_seg['diagnosis_principal'].= $temp['diag_code']." : ".$temp['diagnosis']." <br> \n";
			else
				$result_seg['diagnosis_others'].= $temp['diag_code']." : ".$temp['diagnosis']." <br> \n";
		}
	}
	//ended by daryl 09/03/2014
	...
	if ($result_therapy = $objDRG->getProcedureCodes($encounter_nr,$encounter_type)){
		...
		while($temp=$result_therapy->FetchRow()){
			if ($temp['type'])
				$result['therapy_principal'].= $temp['code']." : ".$temp['diag_desc']." <br> \n";
			else
				$result['therapy_others'].= $temp['code']." : ".$temp['diag_desc']." <br> \n";
		}
	}
