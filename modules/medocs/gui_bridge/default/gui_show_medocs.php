...
function viewCertMed(enc){
	//window.open("../../modules/registration_admission/certificates/cert_med_interface.php?encounter_nr="+<?=$encounter_nr?>+"&showBrowser=1","viewClinicalForm","width=800,height=700,menubar=no,resizable=yes,scrollbars=yes");
		return overlib(
					OLiframeContent("../../modules/registration_admission/med_cert_history.php?enc="+enc, 850, 440, "fOrderTray", 1, "auto"),
...
function ConfinementHistory(enc){
	//window.open("../../modules/registration_admission/certificates/cert_med_interface.php?encounter_nr="+<?=$encounter_nr?>+"&showBrowser=1","viewClinicalForm","width=800,height=700,menubar=no,resizable=yes,scrollbars=yes");
		return overlib(
					OLiframeContent("../../modules/registration_admission/confiment_history.php?encounter_nr="+enc, 850, 440, "fOrderTray", 1, "auto"),
																	WIDTH,440, TEXTPADDING,0, BORDER,0,
																		STICKY, SCROLL, CLOSECLICK, MODAL,
																		CLOSETEXT, "<img src=../../images/close.gif border=0 >",
...
$smarty->assign('LDFirstName',$LDFirstName);
$smarty->assign('LDSuffix',$LDSuffix);
...
$patient_enc_phic = $enc_obj->getPatientEncounterInsurance($encounter_nr);
#added by daryl 
$patient_enc_no = $enc_obj->getPatientEncounternoInsurance($encounter_nr);
#10/09/2014
#allow to return without PHIC
if($patient_enc_phic){
	$patient_enc = $patient_enc_phic;
}else{
	$patient_enc = $patient_enc_no;
}
#end by daryl
$patient_remarks = $enc_obj->getPatientEncounterRemarks($encounter_nr);
#echo "sql = ".$enc_obj->sql;
$test = $enc_obj->GetPaidBillDate($HTTP_SESSION_VARS['sess_pid'], $encounter_nr);
if($test['or_date'])
	$smarty->assign('PaidBill',date("m/d/Y h:iA",strtotime($test['or_date'])));
else
	$smarty->assign('PaidBill','');

$finalbill_datetime = $enc_obj->GetFinalBillDate($encounter_nr);
if($finalbill_datetime['bill_dte'])
	$smarty->assign('FinalBill',date("m/d/Y h:iA",strtotime($finalbill_datetime['bill_dte'])));
else
	$smarty->assign('FinalBill','');

//added by daryl
$admittion_datetime = $enc_obj->GetadmitDate($encounter_nr);
if($admittion_datetime['admit_date'])
	$smarty->assign('AdmitDateTime',date("m/d/Y h:iA",strtotime($admittion_datetime['admit_date'])));
else
	$smarty->assign('AdmitDateTime','');

		if ( $admittion_datetime['diagnosis'] ){
				$ssTemp =	'		<tr class="wardlistrow1" id="principal" name="principal">
				<td colspan="2" align="center">'.$admittion_datetime['diagnosis'].'</td>
			</tr>';
			}else{
				$ssTemp =	'		<tr class="wardlistrow1" id="principal" name="principal">
				<td colspan="2" align="center"><font color="red">No Admitting Diagnosis</font></td>
			</tr>
	';
			}
			$smarty->assign('sadmitting_details',$ssTemp);
		//ended by daryl
//ended by daryl
...
if($mode=='show'){
	...
	*/
	...
	if($rows){
		...
		//added by daryl
			$smarty->assign('admittinghead','Admitting Diagnosis:');
		//ended by daryl
        #------ notification
		...
		if ( (!empty($result_seg['diagnosis_principal']) && isset($result_seg['diagnosis_principal'])) ||
			...
			$smarty->assign('sDiagnosis',$result_seg['diagnosis_principal']);
...
';
		...
		$sTemp = '';
		if ( (!empty($result_seg['diagnosis_others']) && isset($result_seg['diagnosis_others'])) ||
				(!empty($result['therapy_others']) && isset($result['therapy_others'])) ){
			$smarty->assign('sRowClass','class="wardlistrow2" id="others" name="others"');

			$smarty->assign('sDiagnosis',$result_seg['diagnosis_others']);
...
# Create a new form for data entry###################
	...
	if($allow_medocs_user){
		//Admission patient from OPD || patient from ER
			...
			$smarty->assign('sremarks','<textarea name="inp_remarks" id="inp_remarks" cols="75" rows="3" wrap="physical" >'.trim($patient_remarks['remarks']).'</textarea>');
			...
			if(($allow_er_user)||($allow_ipd_user)){
				...
				while($result=$rowDisp->FetchRow()){
					if ($result['disp_desc']!='Admitted'){
						$sTmp = $sTmp.'<input name="disp_code" onClick="add_referral(this.value);" id="disp_code" type="radio" value="'.$result['disp_code'].'" ';
...
######################### END ###############################
	...
	$smarty->assign('sTailScripts','<script language="javascript">
                                        ...
                                        xajax_populateCode("'.$encounter_nr.'","'.$encounter_type.'","icp");
                                        xajax_populateNotification("'.$encounter_nr.'");
                                        xajax_populate_ICDRVS_Billing("'.$encounter_nr.'","'.$encounter_type.'","icd","billing");
                                        xajax_populate_ICDRVS_Billing("'.$encounter_nr.'","'.$encounter_type.'","icp","billing");
                                   </script>');
...
<input type="hidden" name="gender_seg" id="gender_seg" value="<?=$sex?>">
...
?>
	<div id="divSaveButton">
		<input type="<?php if($setHidden) echo "hidden"; else echo "image"; ?>" onclick="
			if (setFrmSubmt()){
			 	checkTransferredEmpty();
			}" title="Save and Discharge" <?php echo createLDImgSrc($root_path,'savedisc2.gif','0'); ?>>
	</div>
<?php } ?>

<script>
	function add_referral(referral_nr){
	if (referral_nr =='8') {
		var nr = referral_nr;
		var reason = reason;
		var enc = $J('#encounter_nr').val();
		var root_path = '<?php echo $root_path?>';
		var PageReferral = root_path+"modules/billing_new/billing-referral-new.php?enc="+enc+"&referral_nr="+nr;
	    var htmlReferral = '<iframe style="border: 0px; " src="' + PageReferral + '" width="470px" height="150px"></iframe>';
...
# if discharged do the ff codes
	if ( $enc_obj->Is_Discharged($encounter_nr) && ($mode=="show")){
		...
		#			$formToPrint = "<a href=\"".$root_path."modules/registration_admission/show_cover_sheet.php?encounter_nr=$encounter_nr\" target=_blank>".$segPrintIcon."Inpatient Clinical Cover Sheet</a>";
					...
					$formToPrint = "<a href=\"".$root_path."modules/registration_admission/certificates/show_cover_sheet_jasper.php?encounter_nr=$encounter_nr\" target=_blank>Inpatient Clinical Cover Sheet</a>";
...
#added by daryl
$smarty->assign('LBremarks',"Remarks");
$smarty->assign('ssremarks',$patient_remarks['remarks']);
