...
function populateSpecialLabServiceList($area='',$area_type='',$encounter_nr=0,$ref_source='LB',$is_cash=1,$discountid='',$discount=0,$is_senior=0,$is_walkin=1,$group_code,$source_req='LD',$isStat=0,$is_charge2comp=0,$compID='',$sElem,$searchkey,$page) {
		...
		//added by poliam 03/10/2014
		$t= time();
		$actualTime = date("H",$t);

		$actualDate = date("m/d", $t);
		
		//ended by poliam 03/10/2014
		#----------------
                ...
                if ($result['is_profile']){
                	...
                	if($prices){
                		...
                		if($encounter_nr){
                			...
                			if($actualTime >='17' || $actualTime <= '7')
                			$prices5pm = $srv->GetRoomPrice($result["service_code"], '99', $source='LB', $is_cash);
                			if($prices5pm){
                				while($rowprice=$prices5pm->FetchRow()) {
		                		$other_cash = $rowprice['cash_price'];
		                		$other_charge = $rowprice['charge_price'];
		                		$other_net = $rowprice['net_price'];
		                		}
                			}
                			//edited by poliam 05/19/2014
                			//holiday codes

                			$holidayDate = $srv->GetHolidays($actualDate);
                			if($holidayDate){
                				$holidayrate = 1.30;
                			}else{
                				$holidayrate = 1;
                			...
                			if($prices5pm){
                				$objResponse->addScriptCall("addProductToList","request-list",$result["service_code"],
														$name,$result["group_code"],$result["code_num"],$codenum,number_format($other_cash*$holidayrate, 2, '.', ''),
														number_format($other_charge*$holidayrate, 2, '.', ''), $result['is_socialized'],
                                                        $result['in_lis'],$result['oservice_code'],$result['ipdservice_code'],number_format($other_net*$holidayrate, 2, '.', ''), 
                                                        $available, $result['is_blood_product'],$result['is_package'],$result['is_profile'], $child_test);
                			}else{
                				$objResponse->addScriptCall("addProductToList","request-list",$result["service_code"],
														$name,$result["group_code"],$result["code_num"],$codenum,number_format($result["price_cash"]*$holidayrate, 2, '.', ''),
														number_format($result["price_charge"]*$holidayrate, 2, '.', ''), $result['is_socialized'],
                                                        $result['in_lis'],$result['oservice_code'],$result['ipdservice_code'],number_format($result['net_price']*$holidayrate, 2, '.', ''), 
                                                        $available, $result['is_blood_product'],$result['is_package'],$result['is_profile'], $child_test);
                			}
                		}
							...
							if($guiDetails["name_type"]=="D")
							 {
								...
								#added by VAN 07-15-2010
									...
									if ($area_type!='pw'){
										...
										if ($isStat){	
												$price_cash = $guiDetails["cash"] + ($guiDetails["cash"] * $stat_additional);
												$guiDetails["cash"] = round($price_cash);
												$price_charge = $guiDetails["charge"] + ($guiDetails["charge"] * $stat_additional);
												$guiDetails["charge"] = round($price_charge);
												$net_price = $guiDetails["net_price"] + ($guiDetails["net_price"] * $stat_additional);
												$guiDetails["net_price"] = round($net_price);
										}
									}
