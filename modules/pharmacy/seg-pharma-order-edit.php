...
if ($_REQUEST["view_from"]=='ssview') {
	$ss_view = TRUE;
		...
		$infoResult2 = $order_obj->getcompany_order($Ref);
		if ($infoResult2)	$infos = $infoResult2->FetchRow();
//added by daryl
	if ($infos['company']){
...
$smarty->assign('sWindowTitle',"Pharmacy::Edit request");
	...
	if($Ref){
       $company_name = $comp_obj->CheckHasCompanyrefno($Ref);
    }
...
//			'orderdate'=>$_POST['orderdate'],
			'is_cash'=>$_POST['iscash'],  //uncommented by janken 1/22/2014 to save the changing of transaction type
			'is_tpl'=>$_POST['is_tpl'],  //uncommented by janken 1/22/2014 to save the changing of transaction type
			'amount_due'=>$total,
...
$smarty->assign('scompany_name',$info['company']);#added by daryl
		...
		$smarty->assign('sPriority',($info['is_urgent']=="0") ? "Routine" : "Stat");
...
if ($_REQUEST['billing']) {
	...
	//updated by janken 1/22/2015 deleting the disabled function
	$smarty->assign('sIsCash','<input class="segInput" type="radio" name="iscash" id="iscash1" value="1" '.(($_POST["iscash"]!="0")?'checked="checked" ':'').'onclick="if (warnClear()) { changePrice(); changeTransactionType(); return true;} else {return false;}"  /><label class="segInput" for="iscash1" style="font:bold 12px Arial; color:#3e7bc6">Cash</label>');
	$smarty->assign('sIsCharge','<input class="segInput" type="radio" name="iscash" id="iscash0" value="0" '.(($_POST["iscash"]=="0")?'checked="checked" ':'').'onclick="if (warnClear()) { changePrice(); changeTransactionType(); return true;} else return false;"  /><label class="segInput" for="iscash0" style="font:bold 12px Arial; color:#c64c3e">Charge</label>');
...
'<select id="charge_type" name="charge_type" class="segInput" '.($_POST['iscash']==='0'?'':'style="display:none"').' disabled="disabled">
	...
	<option value="company" '.($_POST['charge_type']=='' && $trans_amount ?'selected="selected"':'').'>COMPANY</option>
                                            ...
                                            <input type='hidden' id='comp_id' name='comp_id' value='".$comp_id."'/>"); //updated by 07-18-2014
...
$smarty->assign('sNormalPriority','<input class="segInput" type="radio" name="priority" id="p0" value="0" '.(($info["is_urgent"]!="1")?'checked="checked" ':'').$readOnlyAll.'/><label class="segInput" for="p0">Routine</label>');
$smarty->assign('sUrgentPriority','<input class="segInput" type="radio" name="priority" id="p1" value="1" '.(($info["is_urgent"]=="1")?'checked="checked" ':'').$readOnlyAll.'/><label class="segInput" for="p1">Stat</label>');
...
$smarty->assign('sDiscountShow','<input type="checkbox" name="issc" id="issc" '.($issc?'checked="checked" ':'').' '.(($_POST["iscash"]=="0")?'disabled="disabled"':'').' onclick="seniorCitizen()" '.$readOnlyAll.'><label for="issc" class="segInput">Senior citizen</label>');
