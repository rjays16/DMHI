...
//        $db->debug = true;
		...
		foreach ($_POST['items'] as $i=>$v) {
			...
			$statusArray[] = $_POST['status'][$i];
                ...
                $cash = (float)str_replace(',','',$_POST['cash'][$i]);
                $charge = (float)str_replace(',','',$_POST['charge'][$i]);
...
$smarty->assign('sNormalPriority','<input class="segInput" type="radio" name="priority" id="p0" value="0" '.(($info['is_urgent']!="1")?'checked="checked" ':'').$readOnlyAll.'/> <label class="segInput" for="p0">Normal</label>');
$smarty->assign('sUrgentPriority','<input class="segInput" type="radio" name="priority" id="p1" value="1" '.(($info['is_urgent']=="1")?'checked="checked" ':'').$readOnlyAll.'/> <label class="segInput" for="p1">Urgent</label>');
...
//			}
        ...
        $price = $row['is_cash'] === 1 ? $row['pricecash'] : $row['pricecharge'];
		$rows .= "
