...
if (isset($_POST["submitted"])) {
    ...
    $_POST['grant']=$_POST['charge_type'];
    if($_POST['charge_type']=="company"){
        $_POST['charge_type']="";
    }
    /*end added by mai*/
...
//	}
	...
	if ($saveok) {
		...
		if($saveok1){ //added by mai 07-18-2014
		...
		$printfile = $root_path.'modules/pharmacy/seg-pharma-order.php'. URL_APPEND."&target=print&userck=$userck".'&cat=pharma&ref='.$data['refno'];
...
';
		...
		exit;
			...
			$smarty->assign('sysErrorMessage', "Unable to add medicine request");
		}
	}
	else {
...
$onLoadJS="onload='init(); preset();'"; //updated by mai 07/25/2014, added preset
...
$smarty->assign('sIsCash','<input class="segInput" type="radio" name="iscash" id="iscash1" value="1" '.(($_POST["iscash"]!="0")?'checked="checked" ':'').'onclick="if (warnClear()) { changePrice(); changeTransactionType(); return true;} else {return false;}" /><label for="iscash1" class="segInput" style="font:bold 12px Arial; color:#3e7bc6">Cash</label>');
$smarty->assign('sIsCharge','<input class="segInput" type="radio" name="iscash" id="iscash0" value="0" '.(($_POST["iscash"]=="0")?'checked="checked" ':'').'onclick="if (warnClear()) { changePrice(); changeTransactionType(); return true;} else return false;" style="margin-left:10px" /><label class="segInput" for="iscash0" style="font:bold 12px Arial; color:#c64c3e">Charge</label>');
...
<option value="CMAP">CMAP</option> -->
                                            ...
                                            <input type='hidden' id='comp_id' name='comp_id' value='".$comp_id."'/>"); //updated by mai 07-18-2014

// echo "pid".$_GET['pid'];
$result_comp = $person_obj->getcompanyname($_GET['pid']);

if($_GET['pid']){
       $company_name = $comp_obj->CheckHasCompanyPid($_GET['pid']);
    }
    if($company_name){
         $smarty->assign('scompany_name',$company_name);#added by daryl
    }else{
         $smarty->assign('scompany_name',$result_comp);#added by daryl
    }
...
if($_POST["encounter_nr"]){
	$HasExceedPhic = $enc_obj->HasExceedPhic($_POST["encounter_nr"]);
	if($HasExceedPhic){
...
$smarty->assign('sBtnCoverage','<input type="hidden" id="btn-coverage" src="'.$root_path.'images/btn_coverage.gif" border="0" onclick="return openCoverages();"/>');
$smarty->assign('sDiscountShow','<input type="checkbox" name="issc" id="issc" '.(($_POST["issc"])?'checked="checked" ':'').' onclick="seniorCitizen()"><label class="segInput" for="issc" style="font:bold 12px Arial;">Senior citizen</label>');
