		...
		if (dBody) {
			...
			if (details["FLAG"]=="1") {
				...
				'<tr id="p-'+id+'" '+((dRows.length%2>0)?' class="alt"':'')+'>' +
					...
					'<td align="right">'+(isNaN(parseFloat(details["price_charge"])) ? details["price_charge"] : _lgformatNumber(parseFloat(details["price_charge"]),2))+'</td>'+
...
$plst = &$listgen->createList('plst',array('Type','Item Code','Item name/Generic name','Sell Price', 'Charge Price', 'Senior', 'C1', 'C2', 'C3', ''),array(0,0,1,0,0,0,0,0,NULL),'populateProducts');
$plst->addMethod = 'addProductRow';
$plst->fetcherParams = array();
$plst->columnWidths = array("7%", "10%", "*", "11%", "9%", "7%", "7%", "7%", "8%");
