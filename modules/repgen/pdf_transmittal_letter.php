...
class RegGen_TransmittalLetter extends DMCRepGen {
	...
	//				$this->SetFontSize(14);
					...
					if ($length > $colwidths[1]) {
						...
						$ypos = $this->GetY();
						$this->SetXY($xpos, $ypos-4);
						$this->MultiCell($colwidths[1], $cell_height, strtoupper(trim($hosp["addr_no_street"])/*.", ".trim($hosp["brgy_name"]).", ".trim($hosp["mun_name"])*/), 0, "L"); //commented brgy name and municipality name because of redundancy. ken 4/15/2014
						$this->SetXY($xpos, $ypos);
