...
class RepGen_Radio_Services extends DMCRepGen {
    ...
    function RepGen_Radio_Services($from, $to) {
        ...
        $this->ColumnWidth = array(79,13,19,7,7,13,19,13,10,19,11,18,19);
        $this->Columns = 13;
    ...
    function Header() {
        ...
        $this->Line(138,24,138,28);
        $this->Line(183,24,183,28);
    ...
    function FetchData() {        
            ...
            while ($row=$result->FetchRow()) {
                ...
                if($result2)
                {
                    while ($row2=$result2->FetchRow()) {
                        ...
                        $sql = "SELECT 
                                  COUNT(ctrr.refno) AS inpatient 
                                FROM
                                  care_test_request_radio AS ctrr 
                                  INNER JOIN seg_radio_serv AS srs 
                                    ON ctrr.refno = srs.refno 
                                  LEFT JOIN care_encounter AS ce 
                                    ON srs.encounter_nr = ce.encounter_nr 
                                WHERE ctrr.service_code = '".$service_code."'
                                  AND DATE(ctrr.served_date) BETWEEN '".$this->from_date."'
                                  AND '".$this->to_date."' 
                                  AND ctrr.status NOT IN ('deleted') 
                                  AND srs.status NOT IN ('deleted') 
                                  AND ce.encounter_type IN (3, 4)";

                        $result3 = $this->Conn->Execute($sql);
                        if($result3) {
                            $row3 = $result3->FetchRow();
                            $inpatient = $row3["inpatient"];
                        } else {
                            $inpatient = 0;
                        }
                        ...
                        $sql = "SELECT 
                                  COUNT(ctrr.refno) AS outpatient 
                                FROM
                                  care_test_request_radio AS ctrr 
                                  INNER JOIN seg_radio_serv AS srs 
                                    ON ctrr.refno = srs.refno 
                                ...
                                WHERE ctrr.service_code = '".$service_code."'
                                  ...
                                  AND ctrr.status NOT IN ('deleted') 
                                  AND srs.status NOT IN ('deleted') 
                                  AND ce.encounter_type NOT IN (3, 4)";

                        $result3 = $this->Conn->Execute($sql);
                        if($result3) {
                            $row3 = $result3->FetchRow();
                            $outpatient = $row3["outpatient"];
                        }  else {
                            $outpatient = 0;
                        }

                        $sql = "SELECT 
                                  COUNT(ctrr.refno) AS walkin 
                                FROM
                                  care_test_request_radio AS ctrr 
                                  INNER JOIN seg_radio_serv AS srs 
                                    ON ctrr.refno = srs.refno 
                                ...
                                WHERE ctrr.service_code = '".$service_code."'
                                  ...
                                  AND '".$this->to_date."' 
                                  AND ctrr.status NOT IN ('deleted') 
                                  AND ctrr.request_flag IN ('paid') 
                                  AND srs.status NOT IN ('deleted') 
                                  AND (
                                    srs.encounter_nr IS NULL 
                                    OR srs.encounter_nr = ''
                                  )";
                        ...
                        if($walkin == 0)
                            ...
                            $temp_array[$ctr][3] = $walkin;

                        if($inpatient == 0 && $outpatient == 0 && $walkin == 0)
                            $temp_array[$ctr][4] = "z";
                        else
                            $temp_array[$ctr][4] = $inpatient + $outpatient + $walkin;
        ...
        $total = $in_total + $out_total + $walkin_total;
        $per_total = 0;

        for($z=0; $z<$ctr; $z++) {
            $prnt_total = $temp_array[$z][4];
            $prnt_inpatient = $temp_array[$z][1];
            $prnt_outpatient = $temp_array[$z][2];
            $prnt_walkin = $temp_array[$z][3];
            $temp_array[$z][5] = ($temp_array[$z][4] / $total) * 100;
            $per_total += $temp_array[$z][5];
            ...
            $this->Data[] = array($temp_array[$z][0], '', $prnt_inpatient, '', '', '', $prnt_outpatient, '', '', $prnt_walkin, '', $prnt_total, $prnt_pertotal);
        }
        $this->Data[] = array('TOTAL =>', '', number_format($in_total), '', '', '', number_format($out_total), '', '', number_format($walkin_total), '', number_format($total), $per_total."%");
    }
}
