...
//	var $healthperson_array;  // Array of class Health Personnel.
    ...
    //added by Marlon, 5/16/2019
    // var $newborn_package = false;
...
$this-> setXY($xsave+140, $ysave);
		...
		if($result = $db->Execute($strSQL,$this->encounter_nr)){
			if($result->RecordCount()){
				while ($row = $result->FetchRow()) {
					 ...
					 if($row['package_id']==NEWBORNPACKAGE) {
					 	return true;
					 	// $this->newborn_package = true;
					 }
    ...
    function setAllData($data){
        ...
        if ($row = $objInfo->getAllHospitalInfo()) {
            ...
            $this->hosp_address = strtoupper($row['addr_no_street']);
        } else {
        ...
        $timediff = $objBilling->getDateTimeDiff($this->encounter_nr);

        if(($timediff < 24) && ($deathdate)){
 			$dis_dates_ = $deathdate;
        }else{
        	$dis_dates_ = $encInfo['discharge_dt'];
        }

        $admits_date_ = (($this->encounter_type == OUT_PATIENT) || ($this->encounter_type == ER_PATIENT)) ? $encInfo['er_opd_datetime'] : $encInfo['admission_dt'];
		$this->admission_diagnosis = $admission_diag_;
		$countLines=substr_count($admission_diag_, "\n");
		if($countLines>0){
			$this->admission_diagnosis = implode(explode("\n",$admission_diag_), " ");
		}
		// $this->admission_diagnosis = ;
	...
	function addPart1and2(){
		...
		$this->SetFont($this->fontfamily_label."bd", $this->fontstyle_label_bold, $this->fontsize_label3);
		$this->Cell(50, $this->rheight7, $acc_no, $this->withoutborder, $this->continueline, $this->alignLeft);
		...
		$this->Line(($x+$this->rheight*2+2)+9, $y, $x+$ColumnWidth[1], $y);
		//put the name of facility here
		$this->SetFont($this->fontfamily_answer, $this->fontstyle_answer, $this->fontsize_answer+2);
		...
		$x = $this->GetX();
		$y = $this->GetY()+1+$this->rheight;
		$this->Line(($x-$this->rheight2*8)+3, $y, $x+$ColumnWidth[1], $y);
		//put the name of facility here
		$this->SetFont($this->fontfamily_answer, $this->fontstyle_answer, $this->fontsize_answer);
		...
		# Edited by James
	 	//name of patient
	 	$this->SetFont($this->fontfamily_label."bd", $this->fontstyle_label_bold, $this->fontsize_label3);
		$this->Cell($ColumnWidthName[0]+5, $this->rheight7, $name[0], $this->withoutborder, $this->continueline, $this->alignLeft);
		$x = $this->GetX();
		$y = $this->GetY()+.5;
		...
		$y = $this->GetY();
		$this->Line(($x-$ColumnWidth[0]-$this->rheight2)+2, $y+($this->rheight - $this->lineAdjustment)+1, $x+($ColumnWidth[0]*2)+$ColumnWidth[2]+$this->rheight7+20, $y+($this->rheight - $this->lineAdjustment)+1);
		//put name sub text here
	 	$this->Cell($this->blockwidth, $this->rheight2, '', $this->withoutborder, $this->nextline, $this->alignLeft);
		$this->SetFont($this->fontfamily_label, $this->fontsize_label_normal, $this->fontsize_label4);
		$this->Cell($ColumnWidthName[0]+6, $this->rheight2, '', $this->withoutborder, $this->continueline, $this->alignLeft);
...
//		$this->addSpecialConsiderations();
	...
	function addConfinementPeriod(){
        ...
        $this->Ln(6);
		$this->SetFont($this->fontfamily_label."bd", $this->fontstyle_label_bold, $this->fontsize_label3);
		$this->Cell($ColumnWidthConfinement[0]+5, $this->rheight, $confinement[0], $this->withoutborder, $this->continueline, $this->alignLeft);
 		$this->SetFont($this->fontfamily_label, $this->fontstyle_label_normal, $this->fontsize_label);
		...
		$this->SetFont($this->fontfamily_label, $this->fontstyle_label_normal, $this->fontsize_label);
		$this->Cell($ColumnWidthConfinement[0]+5, $this->rheight, '', $this->withoutborder, $this->continueline, $this->alignLeft);
...
//
					...
					$this->Cell(50, $this->rheight4*2, '5. Type of Accommodation:', $this->withoutborder, $this->continueline, $this->alignLeft);
					$this->Rect($x-15, $y+7, $this->boxwidth+1, $this->boxheight+1);
					...
					$this->Rect($x, $y+7, $this->boxwidth+1, $this->boxheight+1);
...
//        echo "<pre>".print_r($this->rvs_arr_specials,true)."</pre>";
        ...
        /*** MCP Package ***/
        $mcp_parameters = array('enc' => $this->encounter_nr,
            'checkup_date1' => $this->checkup_date1,
            'checkup_date2' => $this->checkup_date2,
            'checkup_date3' => $this->checkup_date3,
            'checkup_date4' => $this->checkup_date4

        );
        $mcp_date = $objBilling->addPrenatalDates($mcp_parameters);
        ...
        for($i=1;$i<=4;$i++){
            $ln = ($i == 4) ? 1 : 0;
            $this->captionLabel($i,$mcp_date[$i-1],$ln,3,40);
            $this->Indent(2);
        }
        ...
        // $this->checkLabel($nbLabels[1],true,'',0,false,true,39);
        // $this->checkLabel($nbLabels[2],true,'',0,false,true,28);
        $this->checkLabel($nbLabels[0],$isNewborn,'',0,false,true,28);
    ...
    function setCodesData($rows, $max_len, $doAlign = true) {
        ...
        foreach ($rows as $rkey => $row) {
            if(in_array($row['ops_code'],$spAddedCodes)/* && $row['special_case'] == 1*/){
                //$row['description'] = '';
                $spAddedCodes[] = $row['description'];//added by genz
            }else{
...
/*** GET ICD RVS ***/
$objBilling = new Billing();
$icd = $objBilling->getIcd($_GET['encounter_nr']);
$rvs = $objBilling->getRvs($_GET['encounter_nr']);
/*** RECONSTRUCT ICD RVS ***/
$objICD_ICP = new ICD_ICP;
$objICD_ICP->formatSpecialProcedures($rvs);
$icdData = $objICD_ICP->setCodesData($icd,35,true);//updated by genz
$rvsData = $objICD_ICP->setCodesData($rvs,26,true);//updated by genz
