...
*/
	...
	function Header() {
			...
			$row['hosp_addr1']   = "Mc Arthur Highway Lizada Toril Davao City";
				...
				$this->Image($root_path.'gui/img/logos/dmhi_logo.jpg',10,8,10);
				$this->SetXY(0,5);
				...
				if($patient!=NULL)
					 ...
					 $patient = $lab_results->get_patient_walkin($this->refno, $this->group_id);
					 extract($patient);
	...
	function LabResult_body(){
				...
				if($this->service_code)
						$sql = "SELECT 
									...
									FROM
									  ...
									  LEFT JOIN seg_lab_result AS r
									  	ON r.`param_id` = rpa.`param_id`
									WHERE r.`service_code` = '$this->service_code' AND
									(
									    NOT (
									      ISNULL(rp.`SI_lo_normal`) 
									      AND ISNULL(rp.`SI_hi_normal`)
									    )
									)";
				...
				$sql = "SELECT rd.`pathologist_pid`, rd.`is_confidential`, rd.`med_tech_pid`, res.*, rp.*, 
							...
							WHERE rd.`refno` = '$this->refno' 
								AND rd.`service_code` = '$this->service_code'
				...
				if($result!=NULL)
				{
						...
						while($result!=NULL && $value = $result->FetchRow()){
								...
								$unit = $value["unit"];
								if($value["form_name"]!="" && $group_name != $value["form_name"]){
										$group_name = $value["form_name"];
										$this->SetXY($tb_x,$tb_y);
										$this->Cell(50,4,strtoupper($value["form_name"]),0,0,'L');
								...
								elseif($value["is_numeric"]=="1"){
										...
										if($fld_value!="")
										{
												if($unit==""){
														...
														$this->SetXY($tb_x+75,$tb_y);
														$this->Cell(25,5,$fld_value .' '. $value['SI_unit'],0,0,'C');
												}
										}
