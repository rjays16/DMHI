...
function js_addDiscount(id, desc, discount, area_used, bill_codedareas, bill_areas, flash, billfinal) {
/*
 * GUI function for dynamically appending a row to the discount table
 */
	var destRows, destTableBody, newRowDest, lastRowNo;
	var i, cntr;
	var finalbillcheckbox;
	...
	if (destTable=document.getElementById("discountTable")) {
		...
		if (id) {
			...
			select_area = '<select id="area_used'+lastRowNo+'" name="area_used'+lastRowNo+'" style="display:none;">'+
				...
				if(billfinal == '1'){
					finalbillcheckbox = '<td align="center"><input  style="width:95%;display:none;" type="checkbox" name ="IsBillFinal'+lastRowNo+'" disabled="disabled">YES</td>';
				}else{
					finalbillcheckbox = '<td align="center"><input style="width:95%;display:none;" type="checkbox" name ="IsBillFinal'+lastRowNo+'" disabled="disabled"></td>';
				}
...
function js_prepareAdd() {
	...
	xajax_newDiscount(id.value, desc.value, disc.value, area.value, areas_id.value, areas_desc, userid, IsBillFinal);
