...
//		$sql="SELECT * FROM seg_discount ORDER BY discountdesc";		
		...
		while($result=$ergebnis->FetchRow()) {
			...
			$objResponse->addScriptCall("js_addDiscount", $result['discountid'], $result['discountdesc'], $result['discount']-0,$result['area_used'], $result['billareas_applied'], $areas_desc, '',$result['is_after']);
		}
	...
	function newDiscount($id, $desc, $discount, $area, $areas_id, $areas_desc, $encoder, $afterBill) {
		global $db;
		
		$dsc_obj=new SegDiscount;

		$result=$dsc_obj->CreateDiscount($id, $desc, $discount, $area, $areas_id, $encoder, $afterBill);		
		$objResponse = new xajaxResponse();
		if ($result) {
			$objResponse->addScriptCall("js_addDiscount", $id, $desc, $discount, $area, $areas_id, $areas_desc, TRUE, $afterBill);
