...
$is_setup = $_REQUEST['service_setup'];
...
switch(strtolower($area))
{
	case 'ld': 
		$aread = 'LD';
		$result = $objFis->GetLabService($name, $offset, $maxRows, $accountTransaction, $is_setup); 
		break;

	case 'rd': 
		$aread = 'RD';
		$result = $objFis->GetRadioService($name, $offset, $maxRows, $accountTransaction, $is_setup); 
		break;

	case 'ph': 
		$aread = 'PH';
		$result = $objFis->GetPharmaItems($name, $offset, $maxRows, $accountTransaction, $is_setup); 
		break;

	case 'ot': 
		$aread = 'OT';
		$result = $objFis->GetMiscItems($name, $offset, $maxRows, $accountTransaction, $is_setup); 
		break;
...
if($resultTransaction[0] !== 0){
	switch (strtolower($resultTransaction[2])) {
		case 'ins':
			$aread = "insurance";
			$result = $objFis->Getinsurances($name, $offset, $maxRows, $accountTransaction, $is_setup);
			break;
		case 'com':
			$aread = "company";
			$result = $objFis->GetCompany($name, $offset, $maxRows, $accountTransaction, $is_setup);
			break;
		case 'dc':
			$aread = "discount";
			$result = $objFis->GetDiscount($name, $offset, $maxRows, $accountTransaction, $is_setup);
			break;
		case 'dp':
			$aread = "deposit";
