<?php
    ...
    $up_48hours = "SELECT COUNT(e.pid) AS up_48hours  
      ...
      $stillbirths = "SELECT SUM(t.stillbirths) FROM (SELECT
            ...
            AND e.encounter_type IN (1,2,3,4)
            AND sr.result_code IN (4,8,9,10)
            AND DATE(e.discharge_date) BETWEEN ".$db->qstr($from_date_format)." AND ".$db->qstr($to_date_format)." ) AS t
            ";


        $neonatal = "SELECT SUM(t.neonatal) FROM (SELECT
        ...
        $maternal = "SELECT SUM(t.maternal) FROM (SELECT
