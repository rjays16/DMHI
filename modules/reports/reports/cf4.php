...
//	 var_dump($person_info['date_birth']);die;
...
//                    $params->put('discharge_diagnosis',$row['description']."<br>");
                }else{
                    if ($rate_type != 1){
//                        $params->put('discharge_diagnosis',$desc."<br>".$row['description']."<br>");
                        $diagnosis = $desc .'<br>'. $row['description'];

                    }else{
//                        $params->put('discharge_diagnosis',$row['description']."<br>".$desc);
                        $diagnosis = $row['description'] .'<br>'.$desc;
                    }
                }
            }
    }else{
...
$other_diag = '';
$other = $cf4->getOtherDiagnosis($code);
$misc = $cf4->getMisc();
...
$params->put('discharge_diagnosis', strtoupper($diagnosis). '<br>'. strtoupper($other_diag));
...
while ($row = $dt_res->FetchRow()) {
    ...
    $params->put('age', $row['discharge_date'] != null? $person->getFullAge($person_info['date_birth'], $row['discharge_date']) : $person->getFullAge($person_info['date_birth'], $row['encounter_date']));
}
