<?php
    ...
    $sql = "SELECT t.*,$age_bracket_sum,SUM(t.total) AS total FROM (SELECT c.description as descr, 
            ed.code AS subcode,  
              (SELECT IF(INSTR(ed.code,'.'), 
                SUBSTRING(ed.code, 1, 3), 
                    IF(INSTR(ed.code,'/'), 
            ...
            $age_bracket,SUM(CASE WHEN 1 THEN 1 ELSE 0 END) AS total,
            ...
            t.tab_code AS tab_index
            FROM seg_encounter_diagnosis AS ed 
            INNER JOIN care_encounter AS e ON e.encounter_nr=ed.encounter_nr 
            INNER JOIN care_icd10_en AS c ON c.diagnosis_code=ed.code
            INNER JOIN care_person AS p ON p.pid=e.pid 
                ...
                (SELECT IF(INSTR(ed.code,'.'), 
                    SUBSTRING(ed.code, 1, 3), 
                        ...
                        SUBSTRING(ed.code, 1, 5), 
                            ...
                            SUBSTRING(ed.code, 1, 3), 
                                IF(INSTR(ed.code,'-'), 
                                SUBSTRING(ed.code, 1, 3),ed.code)))))) ) AS t
