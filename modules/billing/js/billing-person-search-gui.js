...
function prepareSelect(id, billdate, dob, billnr, ddate, f_ddate, phic_nr) {
		var nr = $('nr'+id).value;
		...
		window.parent.$('birthdate').value = dob;
		if (nr) {
...
//		var bnoshow = ((typeof dschrg_dt == 'undefined') || (typeof dschrg_tm == 'undefined')) ? 0 : 1;
		...
		if (list) {
				...
				if (id) {
						...
						if(bnoshow){ 
							encfxn =  " prepareSelect('"+id+"', '"+billdate+"', '"+dob+"', '"+billnr+"','"+ddate+"','"+f_ddate+"','"+phic_nr+"');";
						}else{
							encfxn =  " prepareSelect('"+id+"','', '"+dob+"', '','"+ddate+"','"+f_ddate+"','"+phic_nr+"');";
						}
