		...
		if (list) {
				...
				if (dBody) {
					...
					if (details) {
								...
								var insurance_nr = $_GET('hcare_id');
								var from_transmittal_hist = $_GET('from');
								
						if (from_transmittal_hist == 'billing-transmittal-hist') {
							if (insurance_nr == 18) {
								srcRow = '<tr class="wardlistrow'+alt+'" id="row_'+details.enc_nr+'">'+
									 ...
									 '<td align="right" width="10%">'+formatNumber(Number(n_claim), 2)+'</td>'+
												...
												//added by daryl
												 '<td align="center" width="2%"><img class="segSimulatedLink" id="btnprint" style="cursor:pointer" src="'+root_path+'/images/cashier_edit_3.gif" border=0 '+
															'onclick="promptForAlterName(\''+details.enc_nr+'\')"/></td>'+			
									 '</tr>';
							} else {
								srcRow = '<tr class="wardlistrow'+alt+'" id="row_'+details.enc_nr+'">'+
													 '<input type="hidden" name="cases[]" id="case_'+details.enc_nr+'" value="'+details.enc_nr+'" />'+
...
function printInsuranceForm(enc_nr, form_no, newform) {
		...
		if (form_no == 1){
			...
			urlholder = rpath+'modules/repgen/pdf_philhealth_cf2_new_jasper.php'+seg_URL_APPEND+'&encounter_nr='+enc_nr+'&id='+hcareid;
		}
...
//    detailed = (shiftKeyPressed(e)) ? '1' : '0';
		urlholder = rpath+'modules/repgen/jasper_transmittal_letter.php'+seg_URL_APPEND+'&nr='+transmit_no+'&detailed='+detailed+'&class='+class_id+'&trdte='+(getDateFromFormat(trdate, 'yyyy-MM-dd HH:mm:ss')/1000)+'&caseType='+caseType+'&hcare_id='+insurance_nr;
...
function initFormsPrompt() {
		...
		// InstantiatinitFormsPrompte the Dialog
...
function initAlterName(){
	...
	// Instantiate the Dialog
		YAHOO.encounter.container.prompt_altername = new YAHOO.widget.Dialog("prompt_altername",
																																			 { width : "390px",
																																				fixedcenter : true,
																																				visible : false,
																																				constraintoviewport : true,
		...
		YAHOO.encounter.container.prompt_altername.validate = function() {
				var data  = this.getData();
				var alter_fname   = data.alter_fname;
				var alter_mname   = data.alter_mname;
				var alter_lname   = data.alter_lname;
				var alter_exname   = data.alter_exname;
				var enc_nrs = data.encounter_nrs;
				altername_validate(alter_fname,alter_mname,alter_lname,alter_exname,enc_nrs);
				return true;
		};
//    YAHOO.util.Event.addListener("btnPrint", "click", showCategoryPrompt);
}

function altername_validate(alter_fname,alter_mname,alter_lname,alter_exname,enc_nrs){
xajax_savealtername(alter_fname,alter_mname,alter_lname,alter_exname,enc_nrs);
}

function set_altername(alter_fname,alter_mname,alter_lname,alter_exname,enc_nrs){
		$('alter_fname').value = alter_fname;
		$('alter_mname').value = alter_mname;
		$('alter_lname').value = alter_lname;
		$('alter_exname').value = alter_exname;
}

//added by ddaryl 05/12/14
function promptForAlterName(enc_nr){

		$('encounter_nrs').value = enc_nr;
		// $('newform').value = newform;

		$('prompt_altername').style.display = "";
		// $('forms_list').style.visibility = "";

		YAHOO.encounter.container.prompt_altername.render();
		YAHOO.encounter.container.prompt_altername.show();

		// xajax_setFormsForSelection();
		xajax_setAlterName(enc_nr);
}
...
function promptForFormsPrinting(enc_nr, newform) {
		...
		// printInsuranceForm(enc_nr,3, newform);
...
//added by daryl
//09/15/2014
function show_diag_box(){
		var root_path = $('root_path').value;
		var encounter_nr = $('memcateg_enc').value;


	    	var pids="transmittal";
                var billDate = "";
                var frombilling = 1;

                var pageDiagnosis = root_path+"modules/billing_new/billing-diagnosis-procedures.php?pid="+pids+"&encounter_nr="+encounter_nr+"&frombilling="+frombilling+"&billDate="+billDate;
...
//			alert("after diagnosis_nr, code, description, doctor =  "+diagnosis_nr+" , "+code+" , "+description+" , "+doctor);
			...
			stmp = '<img style="cursor:pointer" title="Remove!" src="../../images/cashier_delete.gif" border="0" onclick="rmvcode('+diagnosis_nr+', \''+create_id+'\', \''+code+'\');"/>';
//			stmp = '&nbsp;';
...
function rmvcode(diagnosis_nr,create_id,code){
		var enc_nr   = $('memcateg_enc').value;
		xajax_getbillcaserate(enc_nr,diagnosis_nr,create_id,code);
}

function process_rmvcode(enc_nr,diagnosis_nr,create_id,code,result){
if (code == result)
	alert("Cannot be Deleted! Package has been used!");

else
	xajax_rmvCode(diagnosis_nr,create_id);
...
function saveXml(){
    showXmlLoading();
	xajax_downloadXmlFile($('transmit_no').value,$('memcat').value);
}
