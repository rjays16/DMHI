		...
		if ($result = $db->Execute($strSQL)) {
			if ($result->RecordCount()) {
				...
				while ($row = $result->FetchRow()) {
					...
					$objResponse->addScriptCall("addClaim", $row["insurance_nr"], $row["categ_id"], $row["categ_desc_phic"], $row["confine_period"], $row["encounter_nr"], $spatient, $row["this_coverage"], 0, $row["new_form"]);
				}
			}
		}
		else
