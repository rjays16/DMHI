<?php
		...
		//added by Nick, 3/27/2014
		require_once($root_path.'include/care_api_classes/eTransmittal/class_eTransmittalXml.php');
		function downloadXmlFile($transmit_no,$memcat){
			...
			$xml = $objXml->Generate();
			if(!$xml){
				$objResponse->call('saveXmlFile',$objXml->xmlToString(),0);
			}else{
				$objResponse->call('saveXmlFile',$objXml->xmlToString(),1);
			}
...
//				$s_encrs = implode("', '", $_SESSION['cases']);
						...
						if ($result = $db->Execute($strSQL)) {
								if ($result->RecordCount()) {
										...
										while ($row = $result->FetchRow()) {
												$spatient = $objbill->concatname((is_null($row["name_last"]) ? "" : $row["name_last"]), (is_null($row["name_first"]) ? "" : $row["name_first"].' '.$row["suffix"]), (is_null($row["name_middle"]) ? "" : $row["name_middle"]));
		...
		function setFormsForSelection() {
				...
				$objResponse->call("js_AddOptions", "forms_list", "New Form", 2, '0');
				$objResponse->call("js_AddOptions", "forms_list", "Old Form", 3, '0');
				return $objResponse;
		}
...
#added by daryl
			function setAlterName($enc) {
				global $db;
				$objResponse = new xajaxResponse();
		$sql = "SELECT * from seg_cf2_altername where encounter_nr =" . $db->qstr($enc);

		if ($result = $db->Execute($sql)) {
						if ($result->RecordCount()) {
							while ($row = $result->FetchRow()) {
							$objResponse->call("set_altername",$row['name_first'],$row['name_middle'],$row['name_last'],$row['suffix'],$enc);
							}
						}else{
							$strSQL = "SELECT cp.`name_first` fname, cp.`name_middle` mname, cp.`name_last` lname, cp.`suffix` suffix
						...
						if ($result = $db->Execute($strSQL)) {
							if ($result->RecordCount()) {
									...
									$objResponse->call("set_altername",$row['fname'],$row['mname'],$row['lname'],$row['suffix'],$enc);								
									}
							}
						} 
					}
...
#added by daryl
		...
		function savealtername($alter_fname,$alter_mname,$alter_lname,$alter_exname,$enc_nrs){
				global $db;
				$objResponse = new xajaxResponse();

					$sql = "SELECT encounter_nr from seg_cf2_altername where encounter_nr =" . $db->qstr($enc_nrs);

					if ($result = $db->Execute($sql)) {
						if ($result->RecordCount()) {
							
									$strSQL = "UPDATE seg_cf2_altername AS a 
													SET a.`name_first` = '".utf8_decode($alter_fname)."',
														a.`name_middle` = '".utf8_decode($alter_mname)."',
														a.`name_last` = '".utf8_decode($alter_lname)."',
														a.`suffix` = '".utf8_decode($alter_exname)."'
													WHERE  a.`encounter_nr` = '".$enc_nrs."'";
										$bSuccess = $db->Execute($strSQL);
								...
								$strSQL = "INSERT INTO seg_cf2_altername (encounter_nr, name_first, name_middle, name_last, suffix) ".
											"   values ('".utf8_decode($enc_nrs)."', '".utf8_decode($alter_fname)."', '".utf8_decode($alter_mname)."', '".utf8_decode($alter_lname)."', '".utf8_decode($alter_exname)."')";
...
#added by daryl
		...
		function getbillcaserate($enc_nr,$diagnosis_nr,$create_id,$code){
		$cdObj=new Medocs;
		$objResponse = new xajaxResponse();

		$result=$cdObj->if_bill_caserate($enc_nr);

		$objResponse->call("process_rmvcode",$enc_nr,$diagnosis_nr,$create_id,$code,$result);								
