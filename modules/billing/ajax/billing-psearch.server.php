...
//                $rowEnc=$objEnc->getEncounterInfo($result["encounter_nr"]);
                ...
                $p_name = $person->concatname(($result["name_last"]),  $result["name_first"]." ".$result["suffix"], $result["name_middle"]);
...
#------------until here only-------------------------------------------------------------------------------------fgdp----------------
                ...
                $dob = strftime("%b %d, %Y", strtotime($dob));
...
#                    $admission_dt = @formatDate2Local($rowEnc["admission_dt"], $date_format);
                    ...
                    $case_date = $enc_obj->getParentEncInfo($result["encounter_nr"]);
                    $is_final_bill = $enc_obj->hasFinalBilling($result["encounter_nr"]);

                    if ($result["encounter_type"] == ER_ADMISSION_PATIENT || $result["encounter_type"] == OPD_ADMISSION_PATIENT) {
                        if ($is_final_bill) 
                            $enc_dt = strftime("%b %d, %Y %I:%M%p", strtotime($is_final_bill['bill_frmdte']));
                        else
                            $enc_dt = strftime("%b %d, %Y %I:%M%p", strtotime($case_date["admission_dt"]));
                    } else
                        $enc_dt = strftime("%b %d, %Y %I:%M%p", strtotime($case_date["encounter_date"]));
                } else {
                    $enc_dt = '';
                }
