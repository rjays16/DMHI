...
<script>
		...
		YAHOO.util.Event.onDOMReady(initAlterName);
...
$smarty->assign('sDischargeDate', '<input type="text" id="dischrgdate" name="dischrgdate" size="25" value="" disabled="disabled" readonly="readonly"/>');
...
if ($curtransmit_no != '') {
	$from_transmittal_history = $_GET['from'];

	if (strcmp($from_transmittal_history, "billing-transmittal-hist") == 0) {
		if ($nhcare_id == PHIC_ID) {
			...
			$show_save = '<img id="btnSave" style="cursor:pointer" src="'.$root_path.'/images/btn_save.gif" border=0 onclick="if (validate()) document.transmittal_form.submit()" >';
			$smarty->assign('sTransmittalPrintHeader', "<th width=\"2%\">&nbsp;</th>");
		} else {
			$smarty->assign('sBtnDelete', '&nbsp;<img id="btnDelete" style="cursor:pointer" src="'.$root_path.'/images/btn_delete.gif" border=0 onclick="if (confirm(\'Delete this transmittal?\')) xajax_delTransmittal(\''.$curtransmit_no.'\')">');
			$smarty->assign('sBtnPrint2', '<img id="btnPrintTransmittal" style="cursor:pointer" src="'.$root_path.'/images/btn_printpdf.gif" border=0 >&nbsp;');
			$smarty->assign('sShowButtons', '');
			$smarty->assign('sNoShowButtons', 'style="display:none"');
		}
	} else {
...
$smarty->assign('sBtnSave2', $show_save);
