...
function deleteItem(param_id, form_id){
	...
	if (table && rmvRow) {
		...
		$('rowID'+param_id).remove();
		table.deleteRow(rmvRow.rowIndex);
	}

	var items = dBody.getElementsByTagName('tr');
	if (items.length == 0){ //if all rows are deleted
...
function printResult(){
	...
	window.open("reports/lab_result_report.php?pid="+pid+"&refno="+refno+"&group_id="+group_id+"&service_code="+service_code,"viewPatientResult","left=150, top=100, width=620,height=440,menubar=no,resizable=yes,scrollbars=yes");
}
