	...
	if (list) {
		...
		if (dBody) {
			...
			if (view_mode==1)
				...
				//details.date_request = new Date(details.date_request.replace( /(\d{2})-(\d{2})-(\d{4})/, "$2/$1/$3") );
				
				

				if(details.date_request == undefined){
					datetime = '<input type="text" name="dp'+id+'" id="dp'+id+'" disabled class="testcalendar" value="">';
					icon = '<input type="checkbox" name="dp_icon'+id+'" id="dp_icon'+id+'" onClick=calendar(\''+id+'\');>';
				}
				else{
					datetime = '<input type="text" name="dp'+id+'" id="dp'+id+'" class="testcalendar" value="'+details.date_request+'">';
					icon = '<input type="checkbox" name="dp_icon'+id+'" id="dp_icon'+id+'" onClick=calendar(\''+id+'\');>';
				}
				
				// icon = '<img src="../../images/show-calendar.gif" id="dp_icon'+id+'" class="segSimulatedLink" align="absmiddle" style="margin-left:2px;cursor:pointer" 
				//  />';
			...
			src =
					...
					'<td colspan="2" width="2%" id="datetime'+id+'" align="center">'+datetime+'</td>'+
					'<td width="2%" id="icon'+id+'" align="center">'+icon+'</td>'+
...
function initialRequestList(serv_code,grp_code,name,c_info,r_doc,r_doc_name,n_house,cash,charge,hasPaid,sservice,
                           ...
                           in_lis, oservice_code, ipdservice_code, is_serial, changed_icon, is_package, is_profile, child_test, date_request) {
	...
	var isrepeat = '<?=$_GET["repeat"];?>';
        ...
        details.date_request = date_request;
...
//added by janken 1/21/2015 for changing the price in lab
function changePrice(){
	...
	for (var i=0;i<items.length;i++) {
		...
		if(!$("iscash1").checked){
			trans = 'charge';
			$('withsampleID'+id).disabled = false;
		}
		else{
			trans = 'cash';
			$('withsampleID'+id).disabled = true;
			$('withsampleID'+id).checked = 0; 
		}
...
function changePrices(value, id){
	$('rowPrcCash'+id).value = value;
	$('rowPrcCharge'+id).value = value;
	$('rowPrcNet'+id).value = value;
	$('prc'+id).innerHTML = value;
	$('rowPrc'+id).value = value;
