...
*/
    ...
    if ($encounter_nr){
        ...
        // echo "<script> alert('encounter'); </script>";
    }else if($pid){
...
//ednded by daryl
    ...
    if ((($encounter_nr)||($pid))&&(!$refno)){
         ...
         $patient_name = mb_strtoupper(trim($patient['name_last'])).' '.mb_strtoupper($patient['suffix']).", ".mb_strtoupper(trim($patient['name_first']))." ".mb_strtoupper(trim($patient['name_middle']));
 ...
 $_POST['request_flag'] = $_POST['grant_type'];
    ...
    // for LIS header and details
        ...
        if ($_POST["items"]!=NULL){
             ...
             $_POST['arrayDateRequest'] = $arrayDateRequest;
...
//                              '</script>';
                            ...
                            $smarty->assign('sysInfoMessage',"Laboratory Request Service successfully updated.");
                                ...
                                $smarty->assign('sysErrorMessage', 'Unable to update charge to company.');
                            }
                        }else{
 ...
 $onLoadJS='onLoad="preset();checkCash();setchargecomp()"';
...
<?php
    ...
    if($encounter_nr){
       $company_name = $comp_obj->CheckHasCompany($encounter_nr);
    }
    if($company_name){
...
$iflabwalk = $labwalk['ifwalk'];
    ...
    $result = $enc_obj->getChargeType("WHERE id NOT IN ('paid','phs','charity','cmap','lingap') AND id in ('phic', 'company') ","ordering");
    ...
    if($encounter_nr){
        ...
        $trans_amount = $comp_obj->hasChargeToCompany($encounter_nr, 'LAB', $refno);
    ...
    $smarty->assign('sChargeTyp',
                                "<select class=\"jedInput\" name=\"grant_type\" id=\"grant_type\" ".$disabled." 
                                     onchange=\" if (warnClear()) { emptyTray(); checkCharge(this.value); changeChargeType(); return true;} else {return false;}\">
                                         $options
                                 </select><input type='hidden' id='charge_comp_balance'/><input type='hidden' id='charge_comp_max'/>
