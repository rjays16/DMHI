...
function compare_time($a, $b) {
    if ($a['serv_dt'].$a['serv_tm'] == $b['serv_dt'].$b['serv_tm']) return 0;
    return ($a['serv_dt'].$a['serv_tm'] > $b['serv_dt'].$b['serv_tm']) ? -1 : 1;
}
//end by daryl
...
$all_results = array();
                ...
                usort($all_results, 'compare_time');                                 
                        ...
                        foreach($all_results as $result)
                       {
                                ...
                                if ($result["pid"]!=" ")
                                        $name = ucwords(strtolower(trim($result["name_last"]))).", ".ucwords(strtolower(trim($result["name_first"])))." ".ucwords(strtolower(trim($result["suffix"])))." ".ucwords(strtolower(trim($result["name_middle"])));
                                else
                                ...
                                if ($mod){
                                        ...
                                        $labRequestDetail = "";
                                        $sex_ = $result["sex"];
                                        $labRequestDetails = $lab_obj->GetDetailLabRequest($result["refno"]);
                                         
                                        while($row2 = $labRequestDetails->FetchRow()){
                                            if($row2['date_request'] == '1970-01-01 08:00:00'){
                                                $date_request2 = "(no date/time inputed)";
                                            }else{
                                                $date_request2 = date('F d, Y h:ia', strtotime($row2['date_request']));
                                            }
                                            $labRequestDetail .= "service: ".$row2['name']. " Time: ".$date_request2."<br />"; 
                                        }


                                        $objResponse->addScriptCall("jsRequest",$sub_dept_nr,$my_count,trim($result["refno"]),$name,$requestDate,$urgency,$or_no, $labstatus, $paid, $repeat,trim($result['encounter_nr']), trim($result["pid"]),floor($age),$sex_,$location, $enctype, $result["is_cash"],$with_res,$result["is_repeat"], $labRequestDetail);
                                         }
                          }
...
$all_undone_results = array();
                ...
                if (($ergebnis) || ($ergebnis_walk)){
                     ...
                     usort($all_undone_results, 'compare_time');                                 
                        ...
                        $objResponse->addScriptCall("jsRequest",$sub_dept_nr,$my_count,trim($result["refno"]),$name,$requestDate,$urgency,$result2['or_no'], $service, $service_code, $repeat, trim($result["pid"]),floor($age),$sex_,$location, $enctype, $with_res, $result["group_id"], $result["with_result"],$result["is_repeat"],$result["in_lis"], $result["lis_order_no"]);
    ...
    function deleteRequest($refno){
        ...
        if (($row==0)&&(!$hasfinal_bill)){
            ...
            if ($status) {
                ...
                if ($comp_obj->deleteChargetoCompanyTransaction($refno, 'LAB')){ //delete ledger charged to company
                    //success
                }else{
                    $objResponse->addAlert("Unable to delete the request from company ledger.");
                }
                /*end added by mai*/

                #added by janken 10/24/2014 deleting lab FIS trans
                if(ENABLE_FIS){
                    if($curl_obj->inpatientDeleteBillEntry($refno)){

                    }else{
                        $objResponse->addAlert("Unable to delete the request from fis ledger.");
                    }
                }
    ...
    function populateLabResultList($done, $sElem,$searchkey,$page,$include_firstname,$mod, $encounter_nr='', $is_doctor=0,$pid='',$ref_source='LB') {
            ...
            if ($ergebnis) {
                ...
                while($result=$ergebnis->FetchRow()) {
                        ...
                        $objResponse->addScriptCall("addResultList","RequestList",$result['name'],$result['service_code'],date('Y-m-d',strtotime($result['service_date'])), $result['group_id'], $result['refno']);
