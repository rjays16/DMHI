    ...
    if ($saveok) {
        foreach($_POST["items"] as $i=>$v) {
            ...
            $strSQL = "SELECT
                          ...
                          SUM(
                            CASE
                              WHEN rd.is_unitperpc
                              THEN rd.item_qty
                              ELSE (100 * rd.item_qty)
                            END
                          ) AS rqty,
            ...
            if ($result = $db->Execute($strSQL)) {
                if ($result->RecordCount()) {
                    ...
                    while (($row = $result->FetchRow()) && ($allqty > 0)) {
                        ...
                        $servedqty += $balqty;
                        $fldArray = array('request_refno'=>"'{$row['refno']}'", 'issue_refno'=>"'{$lastnrthis}'", 'item_code'=>"'$v'", 'served_qty'=>"{$balqty}");
                ...
                if ($saveok) {
                    ...
                    if (!empty($saved_items)) {                                                                        
                        foreach($saved_items as $k=>$row) {
                            $fldArray = array('refno'=>"'{$lastnrthis}'", 'item_code'=>"'$v'", 'item_qty'=>"{$servedqty}", 'unit_id'=>"{$_POST['unitid'][$i]}",
...
//        $db->failTrans();
        ...
        if ($saveok) {
            ...
            $smarty->assign('sPrintButton',"<img class=\"segSimulatedLink\" src=\"".$root_path."images/btn_printpdf.gif\" border=\"0\" align=\"absmiddle\" onclick=\"window.open('".$root_path."modules/supply_office/pdf_reqiss_slip.php?refno=".$_REQUEST['refno']."',null,'height=600,width=870,status=yes,toolbar=no,menubar=no,location=no,resizable=yes');\"/>");
...
//            $result = $objdept->getAreasInADept($depscomma);
    ...
    $smarty->assign('sAuthorizedId','<input id="authorizing_id" name="authorizing_id" size="35" readonly="readonly" type="text" value="'.$_POST['authorizing_id'].'"/>');
