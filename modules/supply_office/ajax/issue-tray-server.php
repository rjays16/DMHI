<?php
    ...
    * Gets the quantity not yet served.
     ...
     * @created 07.04.2012
    */    
    function getPendingQty($itemcode, $src, $dest, $qtyperpk) {
      global $db;
      $strSQL = "SELECT 
                  a.item_qty,
                  a.is_unitperpc
      ...
      return $rqty - getRequestQty($itemcode);
    }

    function getRequestQty($itemCode) {
        global $db;
        $sql = "SELECT
                      SUM(served_qty) served_qty
                FROM
                  seg_requests_served  WHERE item_code = ?";
        $result = $db->GetRow($sql, array($itemCode));
        if($result) {
        ...
        if ($area && $areadest) {
            ...
            if ($ergebnis) {
                ...
                while($result=$ergebnis->FetchRow()) {
                    ...
                    $details->pending = getPendingQty($result["bestellnum"], $area, $areadest, $details->qtyperpack);
