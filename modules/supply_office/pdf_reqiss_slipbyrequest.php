...
class RepGen_req_iss_slipbyreq extends DMCRepGen {
    ...
    function RepGen_req_iss_slipbyreq($refno='') {
        global $db;
    ...
    function Header() {
        ...
        //$this->SetXY(13,32);
        $this->MultiCell($total_w, 3,$row['hosp_name'], '', 'C','');
        $this->Ln(1);
        $this->SetFont('Arial','',11);
        $this->MultiCell($total_w, 3,$row['hosp_addr1'],'', 'C','');
    ...
    function BeforeData() {
        ...
        $header = $req_obj->getRequestHeader($this->refno);
        
        $this->area = $objarea->getAreaName($header['area_code_dest']); 
        ...
        $this->SetXY(13,35);
        $this->Cell(53.5,5,'The '.$this->area.' In-charge',0,0,'L');
        $this->SetXY(13,39);
        $this->Cell(53.5,5,'BPH, '.$row['hosp_addr1'],0,0,'L'); 
        $this->SetXY(13,48);
    ...
    function FetchData() {
        ...
        $reqdetails = $req_obj->getRequestDetails($this->refno);
        //echo $req_obj->sql;
        if($reqdetails){
            ...
            while($rowreqdet = $reqdetails->FetchRow()){
                ...
                $this->_count = $count;
                $itemextended = $prod_obj->getExtendedProductInfo($rowreqdet['item_code']);
                $iteminfo = $prod_obj->getProductInfo($rowreqdet['item_code']);
                $skuInventory = new SkuInventory();
                $avgCost = $skuInventory->getItemAvgCost($rowreqdet['item_code']);
                $qtyPerPack = $itemextended['qty_per_pack'] ? $itemextended['qty_per_pack'] : 1;
                if($count > 1) {
                ...
                '',//$row['served_qty'],
                //number_format($itemextended['avg_cost'],2),//$rowissdet['avg_cost'],
                '',
                '',//number_format($comqty*$itemextended['avg_cost'],2),//$row['served_qty'] * $rowissdet['avg_cost']
    ...
    function AfterData() {
        ...
        if (!$this->CM) {
            if ($this->_count<1) {
                ...
                $this->SetFont('Arial','B',9);
                $this->SetTextColor(0);
                $this->Cell($this->ColumnWidth[0]+$this->ColumnWidth[1]+$this->ColumnWidth[2]+$this->ColumnWidth[3]+$this->ColumnWidth[4]+$this->ColumnWidth[5], $this->RowHeight, "TOTAL", 1, 0, 'R', 1);
                //$this->Cell($this->ColumnWidth[6], $this->RowHeight, number_format($this->totalcost,2), 1, 0, 'R', 1);
                $this->Cell($this->ColumnWidth[6], $this->RowHeight, '', 1, 0, 'R', 1);
            }
            ...
            $this->RowNoBorder(array('','REQUESTED BY:                                                                                                 '.$request['name_first'].' '.$request['name_last'].'','','RECEIVED BY:                                                                    _________________________________                 Name and Signature',''));
            $this->Ln(10);
            $this->SetWidths(array(1,100,79,80,10));
            
            $this->RowNoBorder(array('','APPROVED BY:                                                                    _________________________________________  Hospital Administrator or Authorized Representative','','ISSUED BY:                                                                    _________________________________                 Name and Signature',''));
        }
    }
...
$rep = new RepGen_req_iss_slipbyreq($_GET['refno']);
$rep->AliasNbPages();
$rep->FetchData();
$rep->Report();
?>