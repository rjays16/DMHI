...
class RepGen_req_iss_slip extends DMCRepGen {
    ...
    function RepGen_req_iss_slip($refno='') {
        global $db;
    ...
    function BeforeData() {
        ...
        $this->SetXY(13,39);
        $this->Cell(53.5,5,'BPHM, '.strtoupper($this->hosp_addr),0,0,'L'); 
        $this->SetXY(13,48);
            ...
            while($row = $reqserveddetails->FetchRow()){
                ...
                if($reqdetails){
                    ...
                    $this->Data[]=array(
                        ...
                        number_format($row['served_qty'],0,'.',','),
                        number_format($rowissdet['avg_cost'], 2, '.', ','),
                        number_format($row['served_qty'] * $rowissdet['avg_cost'], 2, '.', ',')                                                      
                    );     
                }
            }
        }        
    ...
    function AfterData() {
        ...
        if (!$this->CM) {
            ...
            $this->RowNoBorder(array('','REQUESTED BY:                                                                                                 '.$request['name_first'].' '.$request['name_last'].'','','RECEIVED BY:                                                                                                             '.$acknowledge['name_first'].' '.$acknowledge['name_last'].'',''));
            $this->Ln(10);
