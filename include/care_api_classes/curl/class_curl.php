...
/* Created by: Kenneth Jan Malubay
   	For passing data from HIS to FIS.,*/
...
define('DEBIT_ADJUSTMENT',16); // unused insurance coverage
define('PF',12);
define('ACC',7);
define('ARD',1);
define('ARA',2);
		...
		foreach ($data as $key => $value) {
			if($value['is_cash'] != '1'){
				...
				foreach($account_codes AS $codes_key => $codes_value){
					$credit_amount[$codes_value['income_account']] += $value['price_charge'];
					$debit_amount[$codes_value['cash_account']] += $value['price_charge'];
				}
					}
					else
						return true;
		...
		foreach ($data as $key => $value){
			if($value['is_cash'] != '1'){
				$account_codes = $this->getAccountCodes($value['service_code'], 'LD', CHARGE_ITEM);
					...
					if($account_codes){
				foreach($account_codes AS $codes_key => $codes_value){
					$credit_amount[$codes_value['income_account']] += floatval($value['price_charge']);
					$debit_amount[$codes_value['cash_account']] += floatval($value['price_charge']);
				}
					}
					else
						return true;
		...
		foreach($items AS $key => $value){
			if($value['serve_status'] == 'S'){
				...
				foreach($account_codes AS $codes_key => $codes_value){
					...
					$debit_amount[$codes_value['cogs_account']] += $avg_cost * $value['quantity'];
					$credit_amount[$codes_value['inventory_account']] += $avg_cost * $value['quantity'];
				}
			}
						else
							return true;
	...
	function inpatientMiscRequest($refno, $frombill=false, $bill_nr=null, $bill_date=null, $encounter=null, $from_date=null, $pid=null){
		...
		if($items_msc){
			foreach($items_msc AS $row){
				...
				if($account_codes){
					foreach($account_codes AS $codes_key => $codes_value){
						$credit_amount[$codes_value['income_account']] += $row['total_chrg'];
						$debit_amount[$codes_value['cash_account']] += $row['total_chrg'];
					}
				}
			}//End Foreach
		}//End If
		...
		foreach($data AS $key => $value){
			$account_codes = $this->getAccountCodes($value['service_code'], 'OT', CHARGE_ITEM);
				...
				if($account_codes){
			foreach($account_codes AS $codes_key => $codes_value){
				$credit_amount[$codes_value['income_account']] += $value['chrg_amnt'] * $value['quantity'];
				$debit_amount[$codes_value['cash_account']] += $value['chrg_amnt'] * $value['quantity'];
			}
				}
				else
					return true;
	...
	function inpatientChargeItems($encounter, $bill_date, $pid, $bill_nr, $from_date, $or_amount){
		...
		if($items){
			foreach($items AS $row){
				...
				if($account_codes){
				foreach($account_codes AS $codes_key => $codes_value){
					$credit_pha[$codes_value['income_account']] += $row['itemcharge'];
					$debit_pha[$codes_value['cash_account']] += $row['itemcharge'];
					$debit_pha[$codes_value['cogs_account']] += $avg_cost * $row['qty'];
					$credit_pha[$codes_value['inventory_account']] += $avg_cost * $row['qty'];
				}
				}
			}//End Foreach
		}//End If
		...
		if($coverage){
			foreach ($coverage as $row) {
				...
				foreach($account_codes AS $value){
					$credits[$row['firm_id']] = array('creditid' => $value['credit_id']);
					$debits[$row['firm_id']] = array('debitid' => $value['debit_id']);
				}
				}
				else
					return true;
			...
			foreach ($coverage as $row) {
				...
				if($account_codes){
					foreach($account_codes AS $values){
						$debit_amount[$values['debit_id']] += ($row['payment'] - $row['tax_wheld']);
						$debit_amount[$values['tax_account']] += $row['tax_wheld'];
						$credit_amount[$values['credit_id']] += $row['total_claim'];

						if($row['total_claim'] < $row['payment']){ //income
							$credit_amount[$values['income_account']] += ($row['payment'] - $row['total_claim']);
						}else if($row['total_claim'] > $row['payment']){ //loss
							$debit_amount[$values['cash_account']] += ($row['total_claim'] - $row['payment']);
						}
					}
				}
				else
...
function getHmoPf($ref_no, $encounter_nr, $dr_nr){	
	...
	function postHmoPf($encounter_nr, $ref_no, $hcare_id, $dr_nr){
		...
		if($doc_data){
			foreach($doc_data AS $value){
				...
				$pf[$value['dr_nr'].'_'.$value['role_area']] = ($value['dr_pay'] - $value['tax_wheld']);
				
				$account_codes = $this->getAccountCodes($hcare_id, '', PF_PAYMENT);
				
				if($account_codes){
					foreach($account_codes AS $values){
						$debit_amount[$values['debit_id']] += ($value['dr_pay'] - $value['tax_wheld']);
						//$debit_amount[$values['tax_account']] += $value['tax_wheld'];
						$credit_amount[$values['credit_id']] += $value['dr_pay'] - $value['tax_wheld'];
					}
				}
	...
	function inpatientPF($bill_date, $pid, $bill_nr, $encounter){
		$items = array();
		$debits = array();
		$credits = array();
		$pf = array();
		...
		$doc_data = $this->getDocAmount($bill_nr);
		if($doc_data){
			foreach($doc_data AS $value){
				$this->checkDoctor($value['dr_nr']);

				$pf[$value['dr_nr'].'_'.$value['role_area']] += $value['dr_claim'];
		
				$account_codes = $this->getAccountCodes(PF, '', PF);
	
				if($account_codes){
				foreach($account_codes AS $value_codes){
					$debits[] = array('debitid'=>$value_codes['debit_id'], 'amount'=>$value['dr_claim']);
					$credits[] = array('creditid'=>$value_codes['credit_id']);
				}
		...
		$disc_pf_data = $this->getDocDiscAmount($bill_nr);
		if($disc_pf_data){
			foreach($disc_pf_data AS $value){
				$this->checkDoctor($value['dr_nr']);

				$pf[$value['dr_nr'].'_'.$value['bill_areas']] += $value['amnt_discount'];
		
				$account_codes = $this->getAccountCodes(PF, '', PF);
	
				foreach($account_codes AS $value_codes){
					$debits[] = array('debitid'=>$value_codes['debit_id'], 'amount'=>$value['amnt_discount']);
					$credits[] = array('creditid'=>$value_codes['credit_id']);
				}
    ...
    function inpatientPF2($bill_date, $pid, $bill_nr, $data, $encounter, $or_no){
		$items = array();
		$debits = array();
		$credits = array();
		$pf = array();
		...
		foreach($data AS $value){
			...
			$account_codes = $this->getAccountCodes(PF, '', PF);
				...
				foreach($account_codes AS $value_codes){
					$debits[] = array('debitid'=>$value_codes['debit_id'], 'amount'=>$value['dr_charge']);
					$credits[] = array('creditid'=>$value_codes['credit_id']);
				}
	...
	* Modified By Jarel 10/16/2014
	*/
	function inpatientDiscount($bill_date, $pid, $bill_nr, $encounter){
		$items = array();
		$debits = array();
		$credits = array();
		$debits_disc = array();
		...
		$discount = $this->getDiscountData($encounter);
		if($discounts){
		foreach($discount AS $value_disc){
			$this->checkInsurance($value_disc['discountid'], $value_disc['discountdesc'], 'NONE');
			$account_codes = $this->getAccountCodes($value_disc['discountid'], '', DISCOUNT_CHARGE);

			foreach($account_codes AS $value){
				$credit_disc[$value['credit_id']] += $value_disc['hcidiscount'];
				$debit_disc[$value['debit_id']] += $value_disc['hcidiscount'];
			}
		}

		$disc_pf_data = $this->getDocDiscAmount($bill_nr);
		foreach($disc_pf_data AS $value_disc){
			$account_codes = $this->getAccountCodes($value_disc['discountid'], '', DISCOUNT_CHARGE);

			foreach($account_codes AS $value){
				$credit_disc[$value['credit_id']] += $value_disc['amnt_discount'];
				$debit_disc[$value['debit_id']] += $value_disc['amnt_discount'];
			}
		}

		if(!empty($credit_disc) && !empty($debit_disc)){

			foreach($debit_disc AS $key => $value){
				$debits[] = array('debitid'=>$key, 'amount'=>$value);
			}

			foreach($credit_disc AS $key => $value){
				$credits[] = array('creditid'=>$key, 'amount'=>$value);
			}
		...
		if(!empty($debit_disc) && !empty($credit_disc) || $result['code']==FIS_TRUE ){
			return true;
		}else if(empty($debit_disc) && empty($credit_disc)){
			return true;
		}else{
			return false;
		}
	...
	function inpatientPfAmount($data){
		...
		if($data){
		$doc_data = $this->getBillingDoctors($data['encounter_nr'], $data['bill_nr']);
		...
		if($doc_data){
			foreach($doc_data AS $key => $value){
				...
				$account_codes = $this->getAccountCodes(PF_PAYMENT, '', PF_PAYMENT);
					...
					foreach($account_codes AS $acc_value){
						// $debit_amount[$values['debit_id']] = ($value['dr_pay'] - $value['tax_wheld']);
						// $credit_amount[$values['credit_id']] = $value['dr_pay'] - $value['tax_wheld'];
						// $sl_debits[] = array('debitid'=>$acc_value['debit_id'], 'amount'=>$value['dr_charge'] - $value['claim_deduction'], 'drno'=>$value['dr_nr']);
						$sl_credits[] = array('creditid'=>$acc_value['credit_id'], 'amount'=>$value['dr_charge'] - $value['claim_deduction'], 'drno'=>$value['dr_nr']);
					}
		...
		//=================================================
			...
			if($account_codes){
				foreach($account_codes AS $value){
			$debits[] = array('debitid'=>$value['debit_id'], 'amount'=>$data['amount']); // For GL TRANS
			$credits[] = array('creditid'=>$value['credit_id'], 'amount'=>$data['amount']); // For GL TRANS
				}
			}
			else
				return true;

		$accounts = array('sl_debits'=>$sl_debits, 'sl_credits'=>$sl_credits, 'debits'=>$debits, 'credits'=>$credits);
		...
		$url = "/modules/api/customer/paypfbill";
	...
	function returnPharmaItem($items, $info){
		...
		foreach($items AS $key => $value){
			...
			$account_codes = $this->getAccountCodes($value['bestellnum'], 'PH', CHARGE_ITEM);
				...
				if($amount && $avg_cost && $account_codes){
			foreach($account_codes AS $codes_key => $codes_value){
				$debit_amount[$codes_value['income_account']] += $amount * $value['quantity'];
				$credit_amount[$codes_value['cash_account']] += $amount * $value['quantity'];
				$credit_amount[$codes_value['cogs_account']] += $avg_cost * $value['quantity'];
				$debit_amount[$codes_value['inventory_account']] += $avg_cost * $value['quantity'];
			}
		}
				else
					return true;
				...
				if($row['ref_source'] == 'LD'){
					...
					foreach($account_codes AS $codes_value){
						...
						$nonvatsales = $amount - $vat;
							...
							$debit[$codes_value['income_account']] += $amount;
					...
					foreach($account_codes AS $codes_value){
						...
						$debit[$codes_value['tax_account']] += $vat;
								...
								// or OUTPUT Taxes
								if($codes_value['tax_account'] != '0.0' || $codes_value['tax_account'] != '0'){
									/*
									i.e.
									...
									*/
									...
									$debit[$codes_value['income_account']] += $nontaxsales;
									$debit[$codes_value['tax_account']] += $outputtax;
								}
								else{
									$debit[$codes_value['income_account']] += $amount;
								}
	...
	function cashTransactions($or_no){
		...
		foreach($items AS $key => $value){
			...
			if($value['ref_source'] == 'PH'){
				...
				foreach($account_codes AS $codes_key => $codes_value){
					...
					$credit_pha[$codes_value['income_account']] += $nonvatsales;
									$credit_pha[$codes_value['tax_account']] += $vat;
								}
								else{
									$credit_pha[$codes_value['income_account']] += $value['amount_due'];
								}
							}
							else{
								...
								if($codes_value['tax_account'] != '0.0' || $codes_value['tax_account'] != '0'){
									...
									*/
									...
									$credit_pha[$codes_value['income_account']] += $nontaxsales;
									$credit_pha[$codes_value['tax_account']] += $outputtax;
								}
								else{
									$credit_pha[$codes_value['income_account']] += $value['amount_due'];
								}
					...
					$debit_pha[$codes_value['cash_account']] += $value['amount_due'];
					$debit_pha[$codes_value['cogs_account']] += $avg_cost * $value['qty'];
					$credit_pha[$codes_value['inventory_account']] += $avg_cost * $value['qty'];
						}
				}
			}
				...
				foreach($account_codes AS $codes_key => $codes_value){
					...
					$credit_lab[$codes_value['income_account']] += $nonvatsales;
					$credit_lab[$codes_value['tax_account']] += $vat;
				}
							else
								$credit_lab[$codes_value['income_account']] += $amount;

								$debit_lab[$codes_value['cash_account']] += $amount;
						}
					}
			}
				...
				foreach($account_codes AS $codes_key => $codes_value){
					...
					$credit_rad[$codes_value['income_account']] += $nonvatsales;
					$credit_rad[$codes_value['tax_account']] += $vat;
				}
							else
								$credit_rad[$codes_value['income_account']] += $amount;

								$debit_rad[$codes_value['cash_account']] += $amount;
						}
					}
			}
				...
				foreach($account_codes AS $codes_key => $codes_value){
					...
					$credit_msc[$codes_value['income_account']] += $nonvatsales;
					$credit_msc[$codes_value['tax_account']] += $vat;
				}
							else
								$credit_msc[$codes_value['income_account']] += $amount;

								$debit_msc[$codes_value['cash_account']] += $amount;
						}
					}
			}
				...
				if($value['trans_type'] == 'insurance'){
					...
					if($account_codes){
				foreach($account_codes AS $codes_key => $codes_value){
					$credit_serv[$codes_value['credit_id']] += $value['amount_due'];
					$debit_serv[$codes_value['debit_id']] += $value['amount_due'];
				}
			}
				}
				...
				/*
				$pf_payments = $this->getPFpayment($or_no); // Should be omitted
				if(!empty($pf_payments))
					$amount += $pf_payments['amount_due'];*/

				$pf_payment_from_fb = $this->getPFPaymentFromFB($value['service_code']);
				if(!empty($pf_payment_from_fb)){
					// $amount -= $pf_payment_from_fb;
					$pf_amount_from_fb = $pf_payment_from_fb;
			...
			$billDetails = $cClass->GetBillingDetails($bill_nr);
			$enc = $billDetails['encounter_nr'];
		...
		#-----Bill Payment saving to FIS (DOCTOR'S FEE)-----

		if($pf_amount_from_fb>0)
			$this->getPFforFIS($billDetails['bill_dte'], $pid, $bill_nr, $enc, $billDetails['bill_frmdte'], $or_no);
	...
	function postCashierDiscount($or_no, $pid, $post_dte){
		...
		// $sc_discount = $this->getCashierSeniorDiscount($or_no);
		...
		if($ar_discount){
			if($ar_discount > 0){
				...
				if($account_codes){
					...
					$credits[$values['credit_id']] += floatval($ar_discount);
					$debits[$values['debit_id']] += floatval($ar_discount);
				}
			}
		...
		// if($sc_discount > 0){
		// 	$account_codes = $this->getAccountCodes(senior_discount, '', DISCOUNT_PAY);

		// 	if($account_codes){
		// 		foreach ($account_codes as $keys => $values) {
		// 			$credits[$values['credit_id']] += floatval($sc_discount);
		// 			$debits[$values['debit_id']] += floatval($sc_discount);
		// 		}
		// 	}
		// 			}

		if($ar_discount/* || $sc_discount*/){
	...
	function getCashierSeniorDiscount($or_no){
		global $db; $discount = 0;

		$sql = "SELECT amount_due, is_senior FROM seg_pay_request WHERE or_no = ".$db->qstr($or_no);
		
		$result = $db->Execute($sql);

		if($result){
			while($row = $result->FetchRow()){
				$discount += (($row['amount_due']/(1-$row['is_senior'])) * $row['is_senior']);
			}	
		}
	...
	function companyCharge($bill_nr){
		...
		foreach($info AS $key => $value){
			...
			$account_codes = $this->getAccountCodes($value['comp_id'], '', COMPANY_CHARGE);
				...
				if($account_codes){
			foreach ($account_codes as $keys => $values) {
				$credit[] = array('creditid'=>$values['credit_id'], 'amount'=>floatval($value['bill_amount']));
				$debit[] = array('debitid'=>$values['debit_id'], 'amount'=>floatval($value['bill_amount']));
			}
				}
...
// added by Margaret 06/26/14
	...
	*/
	...
	function getPFPaymentFromFB($BillNr){
		// include_once($root_path."include/care_api_classes/class_cashier.php");

		$cClass = new SegCashier();
		global $db;
		$rowDetails = $cClass->GetBillingDetails($BillNr);
		$rowCoverage = $cClass->GetBillingCoverage($BillNr);
		$rowDiscount = $cClass->GetBillingDiscount($BillNr);
		$rowComputedDiscount = $cClass->GetBillingComputedDiscount($BillNr);
		
		$total = (float) $rowDetails['doc'];
        //edited by jasper 05/13/2013
		//$discount = (float) $rowDiscount;
		//$discounted = $total - ($total * $discount);
		$coverage = $rowCoverage['doc'];
		//$excess = $discounted - $coverage;
        $ComputedDiscount = $rowComputedDiscount['doc'];
        // $totCompDiscount += $ComputedDiscount ;
        // $excess = $total - $coverage - $ComputedDiscount;

        $total_pf_amount = $total - $coverage - $ComputedDiscount;

		if($total_pf_amount)
			return $total_pf_amount;
		else
			return 0;
	}
	...
	function getCompany($comp_id){
		...
		$this->sql = "SELECT comp_name, comp_full_name, comp_add FROM seg_company WHERE comp_id = ".$db->qstr($comp_id);
	...
	function getPFforFIS($billdate, $pid, $bill_nr, $enc, $frm_dte, $or_no){
		global $root_path;
		include_once($root_path.'include/care_api_classes/billing/class_billing_new.php');
		include_once($root_path.'include/care_api_classes/billing/class_billareas.php');
		include_once($root_path.'include/care_api_classes/billing/class_bill_info.php');
	    ...
	    if(is_array($hsp_pfs_benefits) && (count($hsp_pfs_benefits) > 0)) {
		    ...
		    return $this->inpatientPF2($billdate, $pid, $bill_nr, $data, $enc, $or_no);
		}
	}
