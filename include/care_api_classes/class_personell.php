		...
		*/
		...
		global $db;
				...
				$this->sql ="SELECT
												 ...
												 IF (other_title,other_title,if(job_function_title='Doctor','MD',if(job_function_title='Radiologic Technologist','MD,DPBR',''))) as drtitle, p.doctor_role as drRole, p.doctor_level,
												 CONCAT(ifnull(name_first,''),' ',IF (LENGTH(CONCAT(SUBSTR(name_middle,1,1),'.'))=2,CONCAT(SUBSTR(name_middle,1,1),'. '),' '),ifnull(name_last,'')) AS dr_name, p.`other_title`,
        ...
        global $db;
                ...
                $this->sql ="SELECT
                                                 IF (other_title,other_title,if(job_function_title='Doctor','MD',if(job_function_title='Consulting doctor','MD,FPCR',if(job_function_title='Radiologic Technologist','MD,DPBR','')))) as drtitle, p.doctor_role as drRole, p.doctor_level,
		...
		*/
        ...
        function getRadTech(){
                ...
                $this->sql = "SELECT nr, pr.pid, fn_get_personellname_lastfirstmi(pr.nr) as name,
                				p.`name_first`, p.`name_last`, pr.`other_title`
                                                FROM care_personell AS pr
                                                INNER JOIN care_person AS p ON pr.pid=p.pid
