	...
	*/
	...
	function saveResultDispFromArray($data,$flag=0){
		...
		*/
		...
		//added by daryl
		//08/30/2014
				function saveAltDiagDesc_seg($enc_nr, $code, $desc, $user_id) {
				global $db;
			...
			function saveAltDiagDesc_bill($enc_nr, $code, $desc, $user_id) {
				global $db;
			...
			function saveAltDiagDesc_bill_ICP($enc_nr, $code, $desc, $user_id, $refno) {
				global $db;
			...
			function saveAltDiagDesc_bill_ICP2($enc_nr, $code, $desc, $user_id, $refno) {
				global $db;

								$this->sql = "update seg_misc_ops set														
														 modify_id = '$user_id'
														 where encounter_nr = '$enc_nr'";
									return($this->Transact($this->sql));			
		}

		function saveAltDiagDesc_ICP($enc_nr, $code, $desc, $user_id) {
				global $db;
		...
		//ended by daryl
		//added by ken 4/10/2014 based on sir Dommie code :)
		function saveAltDiagCode($enc_nr, $xcode, $code, $user_id) {
				global $db;
				$code = Trim($code);
				// Save alternate code ...
				if($code){
...
//11/15/2013
	function save_Seg_encounter_diagnoses($frombilling='0',$encounter,$code='',$create_id='',$descr='',$type_nr='0', $rvs_date="", $reason=""){
			global $db;
	...
	function AddCode($encounter,$encounter_type,$date,$code,$clinician,$dept,$create_id,$target,$type,$rvs_date="",$frombilling=0){
		$this->useCode($target);
		...
		$data = array('frombilling' => $frombilling,
				     'rvs_date' => $rvs_date,
				     'encounter' => $encounter,
					  'type_nr'      => $type_nr,
					  'code'         => $code,
    ...
    function getOperationsEnc($enc_nr=0){
        ...
        $this->sql="SELECT DISTINCT d.description, d.ops_code, s.rvu, d.op_date, 
                    COUNT(*) AS quantity
                    FROM seg_misc_ops h
    ...
    function addBillingDiagnosis($data){
        ...
        if (empty($descr) || $descr=='' ){
        	$sql = "SELECT description FROM care_ops301_en WHERE code = " . $db->qstr($code);
        $descr = $db->GetOne($sql);
        }
        
        return $this->save_Seg_encounter_diagnoses($frombilling,$encounter,$code,$create_id,$descr,$type_nr,$rvs_date);
	}

    #added by Daryl
	function saveAltDiagCode_seg($enc_nr, $xcode, $code, $user_id) {
				global $db;
				$code = Trim($code);
				// Save alternate code ...
				if($code){
     ...
     function updateIcpAltDesc_seg($data){
    	global $db;
    	$this->sql = $db->Prepare("UPDATE seg_encounter_diagnosis SET 
    		                      	description=?,
    		                      	modify_id=?
    ...
    function if_bill_caserate($enc_nr,$rate_type){
        global $db;

        $this->sql= "SELECT sbc.package_id 
					FROM  seg_billing_caserate AS sbc
					INNER JOIN seg_billing_encounter AS sbe ON sbe.bill_nr = sbc.bill_nr
					WHERE sbe.is_deleted IS NULL 
					AND sbc.`rate_type` = ".$db->qstr($rate_type)."
					AND sbe.encounter_nr =".$db->qstr($enc_nr);
    ...
    function UpdateReferralReason($nr, $updatedReason){
    	global $db;

    	$this->sql = "UPDATE seg_encounter_diagnosis
    					SET reason = ".$db->qstr($updatedReason)."
    					WHERE diagnosis_nr =".$db->qstr($nr);
...
function removeCode_seg($encounter,$code,$target,$create_id){
		$this->useCode($target,$tabs);


		if($target=="icd"){
				$sql = "UPDATE seg_encounter_diagnosis SET is_deleted='1' ".
						"\n WHERE encounter_nr='$encounter' AND code='$code'";
