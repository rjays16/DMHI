<?php
	...
	class SegOR_MiscCharges extends Core{
		...
		function updateMiscCharges($details)
		{
			 ...
			 $no_error = false;
			 if ($refno) { //edit
				 if ($no_error = $this->updateMiscOrder($refno,$is_cash,$discount,$discountid,$charge_date)) {
	 ...
	 function updateMiscOrder($refno,$is_cash,$discount,$discountid,$charge_date)
	 {
			 global $db;
//			 $author = $_SESSION['sess_user_name'];  // Fix for HISSPMC-299
             $author = $_SESSION['sess_temp_userid'];   
				 $this->sql = "UPDATE seg_misc_service SET modify_id=".$db->qstr($author).",
				 											 modify_dt=NOW(), 
				 											 is_cash=".$db->qstr($is_cash).", 
				 											 discount=".$db->qstr($discount).", 
				 											 discountid=".$db->qstr($discountid).",
				 											 chrge_dte=".$db->qstr($charge_date)." 
				 											 WHERE refno = '$refno'";
				 if($db->Execute($this->sql)){
					return true;
				 }
