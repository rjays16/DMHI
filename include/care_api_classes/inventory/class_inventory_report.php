...
//        $db->debug = true;
        $this->sql = 'SELECT sil.packqty, sku.sku_id, sku.expiry_date, sku.unit_id, tr_code, post_uid, post_date, tr_date, tref_no,
                      mvmnt_qty, sil.unit_cost, prev_cost, prev_qty, (mvmnt_qty * packqty) movement, (sil.unit_cost * ABS(mvmnt_qty)) cost
...
//            printR($result, true);
            ...
            foreach($result as $k => $val) {
                ...
                $data[] = array(
                    'tr_date' => $val['tr_date'],
                    'tref_no' => $val['tref_no'],
                    'trdesc' => $stockCard->getFormattedTransactionCode($val['tref_no'], $itemCode, $val['tr_code'], $isSource),
                    'in_qty' => $movement > 0 ? abs($movement) : 0,
                    'in_cost' => $movement > 0 ? number_format($val['cost'],2) : 0,
                    'out_qty' => $movement < 0 ? abs($movement) : 0,
                    'out_cost' => $movement < 0 ? number_format($val['cost'],2) : 0,
                    'netqty' => $totalQty,
                    'avg_cost' =>  number_format($avgCost,2),
                    'totalcost' => number_format($currCost,2),
                );
            }
        }
//        die;
