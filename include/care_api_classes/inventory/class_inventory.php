...
//        $this->serial_detail_array[3] = $supplier_id;
	...
	function addInventory($qty, $unit, $expiry_date=NULL, $serial_no=NULL, $trns_dte=NULL, $unit_cost=-1, $lotNo = '') {
		if (!is_object($unit)) {			
            ...
            $unitid = $unit->unit_id;
            $packqty = (!$unit->is_unit_per_pc) ? $this->getQtyPerBigUnit($this->item_code) : 1;
		}
	...
	function remInventory($qty, $unit, $expiry_date=NULL, $serial_no=NULL, $trns_dte=NULL, $bGetItems = FALSE) {
        ...
        if ($rsskuids) {
            ...
            while ($runqty > 0) {
                ...
                if ($index < sizeof($skus)) {
                    $row = $skus[$index];
                    $index += 1;
                    $skuid = $row['sku_id'];
                    $curqty = $this->getCurrQty($skuid, $trns_dte, $runqty);
                    //printR($skuid . ':' . $curqty . ':'. $index);
//                  current qty is 0
                    if ($curqty <= 0 && $index < sizeof($skus)) {
                        continue;
                    }
                } else {
                    $curqty = $runqty;
