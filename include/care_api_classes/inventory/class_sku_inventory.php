...
class SKUInventory extends Core {    
    ...
    /**
     * Delete Ledger Row when deleting issuance
     * @param $trefNo
     * @return bool
     */
    public function deleteByTrefNo($trefNo) {
        global $db;
        $this->sql = 'DELETE FROM seg_inventory_ledger
                    WHERE tref_no = ? AND tr_code in ("ISS")';
        $success = $db->execute($this->sql, $trefNo);
        if($success) {
            return true;
        }
        return false;
    ...
    public function getSKUId($itemcode, $areacode, $expirydate = '', $serialno = '', $unitid = 0, $unitCost = 0, $lotNo = '') {
        ...
        $unitid = (!empty($unitid)) ? $unitid : DEFAULT_UNIT;
                
        $this->sql = "SELECT 
                        sku_id 
                     FROM 
                        ".SKU_TABLE." skc 
                     WHERE skc.`item_code` = '{$itemcode}' 
                       AND skc.`area_code` = '{$areacode}' 
                       AND skc.`expiry_date` = '{$expirydate}' 
                       AND skc.`serial_no` = '{$serialno}'
                       AND skc.`unit_id` = {$unitid}
                       AND skc.`unit_cost` = {$unitCost}
                       AND skc.`lot_no` = '{$lotNo}'
                     ORDER BY sku_id DESC LIMIT 1";
    ...
    public function getSKUIds($itemcode, $areacode = '', $expirydate = '', $serialno = '', $unitid = 0) {        
        ...
        $skuids = '';
        $sfilter = "item_code = '{$itemcode}'";        
        if (!empty($areacode)) {
            $sfilter .= " AND ";
            $sfilter .= "area_code = '{$areacode}'";
        }        
        if (!empty($expirydate)) {
            $sfilter .= " AND ";
            $sfilter .= "expiry_date = '{$expirydate}'";
        }
        if (!empty($serialno)) {
            $sfilter .= " AND ";
            $sfilter .= "serial_no = '{$serialno}'";            
        }        
        if (!empty($unitid)) {
            $sfilter .= " AND ";
            $sfilter .= "unit_id = '{$unitid}'";            
        }
        
        $this->sql = "SELECT 
                        GROUP_CONCAT(CONCAT('\'', sku_id, '\'')) skuids 
                      FROM
                        ".SKU_TABLE." sku 
                      WHERE {$sfilter}
    ...
    public function getSKUIdswInventory($itemcode, $refdate = '', $areacode = '', $expirydate = '', $serialno = '', $unitid = 0, $lotNo = '', $unitCost = 0) {
        ...
        if (!empty($areacode)) {
            $sfilter .= " AND ";
            $sfilter .= "sku.area_code = '{$areacode}'";
        }        
        if (!empty($expirydate)) {
            $sfilter .= " AND ";
            $sfilter .= "sku.expiry_date = '{$expirydate}'";
        }
        if (!empty($serialno)) {
            $sfilter .= " AND ";
            $sfilter .= "sku.serial_no = '{$serialno}'";
        }        
        if (!empty($unitid)) {
            $sfilter .= " AND ";
            $sfilter .= "sku.unit_id = '{$unitid}'";
        }

        if (!empty($lotNo)) {
            $sfilter .= " AND ";
            $sfilter .= "sku.lot_no = '{$lotNo}'";
        }

        if (!empty($unitCost)) {
            $sfilter .= " AND ";
            $sfilter .= "sku.unit_cost = {$unitCost}";
        }
        ...
        $this->sql = "SELECT 
                      ...
                      FROM
                        seg_sku_catalog sku INNER JOIN ( SELECT 
                          ...
                          INNER JOIN seg_unit un ON un.unit_id = sku.unit_id
                          WHERE {$sfilter}
                      ORDER BY un.is_unit_per_pc DESC, sku.expiry_date";
     ...
     */
    public function getExpiryDateswithQtyinTrans($refno, $itemcode, $areacode) {
        global $db;
//                $db->debug = true;
        ...
        $this->sql = "SELECT 
                        sku.lot_no, sku.expiry_date, sku.serial_no, sku.unit_id, t.qty, sku.unit_cost
                      FROM
                        seg_sku_catalog sku INNER JOIN ( SELECT 
                                                            sel.`sku_id`, SUM(ABS(mvmnt_qty) * packqty) qty 
                                                            FROM
                                                            seg_inventory_ledger sel 
                                                            WHERE tref_no = '{$refno}'
    ...
    public function getItemQty($itemcode, $refdate = '', $areacode = '', $expirydate = '', $serialno = '', $unitid = 0, $brecent = false) {
        ...
        $qty = doubleval(0);
        if (empty($refdate)) $refdate = date("Y-m-d");
        $skuids = $this->getSKUIds($itemcode, $areacode, $expirydate, $serialno, $unitid);
        ...
        if ($result) {            
            if ($result->RecordCount()) {
                ...
                if ($row) {
                    ...
                    $qty = is_null($row['qty']) ? doubleval(0) : doubleval($row['qty']);
                }
            }
        }
...
//        $db->debug = true;
    ...
    private function getQtyPerBigUnit($item_code) {
        ...
        $this->sql = "SELECT qty_per_pack FROM seg_item_extended WHERE item_code = '$item_code'";         
        $qty = 1;   // Default qty per pack 
        $this->result = $db->Execute($this->sql);
        if ($this->result) {
            if($this->result->RecordCount()) {
                $row = $this->result->FetchRow();
                $qty = is_null($row['qty_per_pack']) ? 1 :  $row['qty_per_pack'];
            }            
        } 
...
//        $db->debug = true;
       $this->sql = "SELECT sel.*
                    ...
                    LIMIT 1";
        $result = $db->Execute($this->sql, array($areacode, $itmcode));
...
//        $db->debug = true;
        $this->sql = "INSERT INTO ".INV_TABLE." (tref_no, tr_code, tr_date, sku_id, unit_cost, mvmnt_qty, prev_qty, prev_cost, packqty)
                         VALUES ({$db->qstr($refno)}, {$db->qstr($trcode)}, DATE('{$trdate}'), {$db->qstr($skuid)}, {$ucost}, {$qty}, {$prevqty}, {$prevcost}, {$packqty})";
