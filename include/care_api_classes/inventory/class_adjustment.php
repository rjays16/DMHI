...
class SegAdjustment extends Core{
    ...
    */    
    ...
    function getEarliestExpiryofIteminArea($item_code, $area_code) {
        ...
        $soption = '';
        $this->sql = "SELECT DISTINCT expiry_date FROM seg_eod_inventory WHERE item_code='$item_code' AND area_code='$area_code' ORDER BY expiry_date ASC limit 1";
        $this->result = $db->Execute($this->sql);
        if($this->result){
            //$soption = '<option value = \'\'>-- Select --</option>'; 
            while($row = $this->result->FetchRow())
            {
                $soption =$row['expiry_date'];    
            }
        }
    ...
    */    
    function getSerialofIteminArea($item_code, $area_code) {
        ...
        $soption = '';
        $this->sql = "SELECT DISTINCT serial_no FROM seg_eod_inventory WHERE item_code='$item_code' AND area_code='$area_code' AND serial_no!=''";
        $this->result = $db->Execute($this->sql);
    ...
    */     
    ...
    public function getAdjustments($params)
    {
        ...
        $this->sql = "SELECT b.adjust_date,
						(select d.area_name from seg_areas as d where d.area_code = b.area_code) as area_name,c.bestellnum,c.artikelname,a.adj_qty,e.unit_name,a.unit_cost,f.adj_reason_name
    ...
    #added by janken 1/12/2014
    public function updateUnitCostAdjustment($refno, $item, $uCost){
        global $db;
        ...
        $this->sql = "UPDATE seg_inventory_adjustment_details
                        SET unit_cost = $uCost
                        WHERE refno = $refno
                            AND item_code = $item";
