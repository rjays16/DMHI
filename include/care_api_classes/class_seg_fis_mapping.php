...
var $tb_discount = 'seg_discount';
        ...
        function GetLabService($name, $offset, $maxRows, $accountTransaction, $is_setup){
            global $db;

            switch (strtolower($is_setup) ) {
                case 'yes':
                    $setup_sql = " sam.id IS NOT NULL ";
                    $cond_suffix = " AND ";
                    break;

                 case 'no':
                    $setup_sql = " sam.id IS NULL ";
                    $cond_suffix = " AND ";
                    break;
            ...
            if($name){
                $name = "%".$name."%";
                $where = $cond_suffix." sls.`name` LIKE ".$db->qstr($name)."
                            AND sls.`status` NOT IN ('void', 'deleted')";
            }else{
                $where = $cond_suffix." sls.`status` NOT IN ('void', 'deleted') ";
            }
        ...
        function GetRadioService($name, $offset, $maxRows, $accountTransaction, $is_setup){
            global $db;
            ...
            if($name){
                $name = "%".$name."%";
                $where = $cond_suffix." srs.`name` LIKE ".$db->qstr($name)."
                            AND srs.`status` NOT IN ('void', 'deleted')";
            }else{
                $where = $cond_suffix." srs.`status` NOT IN ('void', 'deleted') ";
            }
        ...
        function GetPharmaItems($name, $offset, $maxRows, $accountTransaction, $is_setup){
            global $db;
            ...
            if($name){
                 $name = "%".$name."%";
                 $where = $cond_suffix." cppm.`artikelname` LIKE ".$db->qstr($name)."
                            AND cppm.`status` NOT IN ('void', 'deleted')";
            }else{
                $where = $cond_suffix." cppm.`status` NOT IN ('void', 'deleted') ";
            }

            $this->sql = "SELECT SQL_CALC_FOUND_ROWS
                            cppm.`bestellnum` AS item_code,
                            if(cppm.`artikelname`, cppm.`artikelname`, cppm.`generic`) AS item_name,
                            IF(sam.`id`, 'setup', 'not yet setup') AS setup,
                            stc.`code_area`
        ...
        function GetMiscItems($name, $offset, $maxRows, $accountTransaction, $is_setup){
            global $db;

             switch (strtolower($is_setup) ) {
                case 'yes':
                    $setup_sql = "WHERE sam.id IS NOT NULL  ";
                    $cond_suffix = " AND ";
                    break;

                 case 'no':
                    $setup_sql = "WHERE sam.id IS NULL  ";
                    $cond_suffix = " AND ";
                    break;
            ...
            if($name){
                $name = '%'.$name.'%';
                $where .= $cond_suffix." sos.`name` LIKE ".$db->qstr($name);
            }
        ...
        function GetItemName($itemcode, $area){
            ...
            if($area == "LD"){
                ...
                $select = "if(artikelname, artikelname, generic)";
            }else{
        ...
        function UpdateFismappingWCC($map, $id){
            ...
            $this->sql = "UPDATE ".$this->tb_account_map."
                            ...
                            credit_id = ".$db->qstr($map['account_credit']).",
                            create_dt = ".$db->qstr(date('Y-m-d H:i:s')).",
                            create_id = ".$db->qstr($HTTP_SESSION_VARS['sess_user_name'])."
                            WHERE id = ".$db->qstr($id);
        ...
        function Getinsurances($name, $offset, $maxRows, $transaction_code, $is_setup){
            global $db;
            ...
            switch (strtolower($is_setup) ) {
                case 'yes':
                    $where .= $cond_suffix." sam.id IS NOT NULL";
                    break;

                 case 'no':
                    $where .= $cond_suffix." sam.id IS NULL";
                    break;
        ...
        function GetCompany($name, $offset, $maxRows, $transaction_code, $is_setup){
            global $db;
        ...
        function GetDiscount($name, $offset, $maxRows, $transaction_code, $is_setup){
            global $db;
        ...
        function GetDeposit($name, $offset, $maxRows, $transaction_code, $is_setup){
            global $db;
            ...
            if($name){
                $name = '%'.$name.'%';
                $where = "WHERE dp.`depositname` LIKE ".$db->qstr($name);
                $cond_suffix = " AND";
