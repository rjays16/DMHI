...
//edited by poliam 03/11/2014
	function CreateDiscount($id, $desc, $discount, $area_used, $bill_areas, $encoder_id, $AfterBill) {
		global $db;
		$id = $db->qstr($id);
		$desc = $db->qstr($desc);
...
//end by poliam 03/11/2014
	...
	*/
	...
	function getDiscount($sid) {
		...
		function Validate_Discount($sid,$val) {
		global $db;
		
		$sid = $db->qstr($sid);

		if ($val == "SC"){
$cond1 = "and discountid = 'SC'";
		}else{
$cond1 = "and discountid IN (".$db->qstr($val).") ";
		}

		$this->sql = "SELECT discountid FROM seg_billingapplied_discount a WHERE a.`encounter_nr` = $sid $cond1";
		if($this->result = $db->Execute($this->sql)) {
			while ($row = $this->result->FetchRow()) {		
					$ntype .= $row['discountid']."/";
				}
			return $ntype;
		}
		else{ return FALSE; }		
	...
	#added by janken 11/05/2014 for deleting the hci coverage when adding sc
	function DeleteHCI($enc){
		global $db;

		$this->sql = "DELETE FROM seg_additional_insurance WHERE encounter_nr = ".$db->qstr($enc);
