...
#	exit();
	...
	function saveOtherInfo(&$data){
		global $HTTP_SESSION_VARS, $dbtype,$db;


		$this->_useDeathCertificate();
		$this->data_array=$data;

$maternal = $db->qstr($data['maternal_condition_tmp']);
$corpse =  $db->qstr($data['corpse_disposal_tmp']);
$corpse_OT_ = $db->qstr($data['corpse_disposal_others']);
$is_late_reg_ = $data['is_late_reg'];
$death_hours = $data['hours'];
$death_days = $data['death_days'];
$death_minutes = $data['minutes'];
$death_sec = $data['sec'];
$death_age = $data['death_age'];
$death_month = $data['death_months'];



$late_cemetery = $db->qstr($data['cemetery_name_address']);

$late_is_attended = $db->qstr($data['is_attended_tmp']);

if ($is_late_reg_ == 1){
$is_late_reg = $is_late_reg_;
}else{
$is_late_reg = 0;
}

if($corpse_OT_ ){
	$corpse_cond = ",corpse_others =" . $corpse_OT_;
}
			$pid_format = " pid='".$data['pid']."' ";

			$this->sql="UPDATE $this->coretable SET maternal_condition=$maternal, 
													corpse_disposal = $corpse,
												
													is_late_reg = $is_late_reg,
													cemetery_name_address=$late_cemetery,
													late_is_attended = $late_is_attended,
													death_hour = $death_hours,
													death_min = $death_minutes,
													death_year = $death_age,
													death_month = $death_month,
													death_day = $death_days,
													death_sec = $death_sec
