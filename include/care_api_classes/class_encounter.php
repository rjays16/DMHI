...
#------ added by Mark on April 17, 2007
	...
	*/
	...
	var $tb_enc_remarks='seg_encounter_remarks';#added by daryl
	...
	*/
	...
	/**added by daryl
	* Field names of seg_encounter_remarks table
	* @var array
	*/
	var $fld_remarks=array(
								'encounter_nr',
								'remarks',
								'modify_id',
							   'modify_time',
...
//			$this->sql = "SELECT encounter_nr FROM $this->tb_enc WHERE encounter_nr BETWEEN ".$db->qstr($ic_nr)." AND ".$db->qstr($ref_nr)." AND encounter_type=".$db->qstr($enc_class_nr)." ORDER BY encounter_nr DESC";
        ...
        elseif($enc_class_nr==12){
            $this->sql = "SELECT encounter_nr FROM $this->tb_enc where encounter_type=12 ORDER BY encounter_nr DESC ";
        }
		else{
	...
	*/
	function getLastestEncounter($pid='0',$mod=0){
		...
		$this->sql="SELECT * FROM $this->tb_enc WHERE pid='".$pid."' AND is_discharged <> '1' ORDER BY encounter_date DESC";
	...
	function getEncounter($pid='0', $iswalkin){
		...
		if ($iswalkin){
			...
			$this->sql ="SELECT cp.pid, enc.encounter_nr,
							...
							cp.spouse_name, cp.guardian_name, cp.contact_no,
	...
	**/
	...
	$this->sql ="SELECT enc.`referrer_institution`,enc.`referrer_notes`, cp.senior_ID, ps.job_function_title, ps.nr AS personnelID, enc_ins.hcare_id,ins.firm_id,enc.current_ward_nr, cp.pid, enc.encounter_nr,
							enc.current_room_nr, enc.current_dept_nr, enc.current_att_dr_nr, enc.consulting_dr_nr, enc.encounter_date,
							cp.name_last, cp.name_first, cp.suffix, cp.name_2, cp.name_3, cp.name_middle, cp.civil_status,
	...
	*/
	...
	global $db;
		...
		$this->sql ="SELECT i.hcare_id ,e.*
					 ...
					 WHERE e.encounter_nr='$encounter_nr'  AND hcare_id ='18'" ;
	...
	#added by daryl 
	#10/09/2014
	#query for patient without insurance
	function getPatientEncounternoInsurance($encounter_nr=0){
		global $db;
...
#-------------------------------------------
	...
	*/
	function loadEncounterData($enc_nr=''){
		...
		$this->sql="SELECT e.*, p.senior_ID, p.pid, p.title,p.name_last, p.name_first,p.suffix, p.date_birth, p.sex,
...
#exit();
	...
	*/
	...
	global $db;
		...
		$this->sql ="SELECT sed.encounter_nr, sed.disp_code, d.disp_desc, d.area_used, sed.hosp_name, sed.hosp_add
...
*/
	...
	*/
	...
	function getIfDoaPatient($enc){
		global $db;
		return $db->getOne("SELECT is_DOA FROM care_encounter WHERE is_DOA = '1' AND encounter_nr = ".$db->qstr($enc)." ");
	}
    ...
    */
	...
	function SearchDiagnosisList($encounter_nr='', $maxcount=100,$offset=0, $frombilling=0){
        ...
        if ($frombilling==1) {
        	...
        	#edited by daryl
				$this->sql = "SELECT SQL_CALC_FOUND_ROWS d.*, icd.diagnosis_code, icd.description, e.is_confidential, d.description as alt_desc
                      FROM seg_encounter_diagnosis  AS d
                      INNER JOIN care_icd10_en AS icd ON icd.diagnosis_code=d.code
                      INNER JOIN care_encounter AS e ON e.encounter_nr=d.encounter_nr
                      WHERE d.encounter_nr='$encounter_nr'
                      AND e.status NOT IN ($this->dead_stat)
                      AND d.is_deleted = 0
                      ORDER BY d.diagnosis_nr, d.create_time DESC";
        }
...
//					  ORDER BY d.date DESC";
    ...
    */
	...
	function countEncounterRequests($encounter_nr){
		...
		function getEncounterRequests($encounter_nr, $offset=0){
				...
				$this->sql = "SELECT l_serv.refno, create_dt AS req_date, 1 AS req_type, u.name AS req_by
											 ...
											 AND l_serv.status NOT IN ('deleted','hidden','inactive','void')
										UNION
										SELECT r_serv.refno, create_dt AS req_date, 2 AS req_type, u.name AS req_by
												FROM seg_radio_serv AS r_serv
												LEFT JOIN care_users AS u ON u.login_id = r_serv.create_id
											 WHERE encounter_nr='$encounter_nr'
											 AND r_serv.status NOT IN ('deleted','hidden','inactive','void')
										UNION
										SELECT p.refno, orderdate AS req_date, 3 AS req_type, u.name AS req_by
										...
										SELECT p.refno, orderdate AS req_date, 4 AS req_type, u.name AS req_by
											 ...
											 WHERE encounter_nr='$encounter_nr' AND pharma_area<>'IP'
										UNION
										SELECT p.refno, chrge_dte AS req_date, 5 AS req_type, u.name AS req_by
												FROM seg_misc_chrg AS p
												LEFT JOIN care_users AS u ON u.login_id = p.create_id
											 WHERE encounter_nr='$encounter_nr'
										UNION
										SELECT p.refno, chrge_dte AS req_date, 6 AS req_type, u.name AS req_by
		...
		function getRequestDetails($refno, $type){
				...
				switch($type){
						...
						case 5: $this->sql = "SELECT name AS item FROM seg_other_services AS s
																		LEFT JOIN seg_misc_chrg_details AS sd ON sd.service_code = s.alt_service_code
																		WHERE sd.refno='$refno'";
										break;
						#added by ken 7/30/2014 for displaying miscellaneous request
						case 6: $this->sql = "SELECT name AS item FROM seg_other_services AS s
																		LEFT JOIN seg_misc_service_details AS sd ON sd.service_code = s.alt_service_code
																		WHERE sd.refno='$refno'";
										break;			
						#added by CHA, May 11,2010
						case 7: $this->sql = "SELECT CONCAT('(', e.number_of_usage, ') ', p.artikelname) AS item FROM seg_equipment_order_items AS e
	...
	function hasFinalBilling($encounter_nr){
		...
		$this->sql ="SELECT bill_frmdte, is_final FROM seg_billing_encounter WHERE encounter_nr='$encounter_nr'
...
#added by ken 4/4/2014
function GetPaidBillDate($pid, $enc){
	global $db;
	$pid = $db->qstr($pid);

	$this->sql = "SELECT or_date FROM seg_pay AS sp
					INNER JOIN seg_pay_request AS spr ON spr.`or_no` = sp.`or_no` AND spr.`ref_source` = 'FB'
					WHERE sp.`cancelled_by` = '' AND sp.`encounter_nr` = ".$db->qstr($enc);
	...
	if($result['or_date'])
		...
		$this->sql2 = "SELECT bill_dte AS or_date FROM seg_billing_encounter
							WHERE request_flag = 'paid' AND doctor_flag = 'paid'
								AND is_deleted IS NULL 
								AND is_final = '1' 
								AND encounter_nr = ".$db->qstr($enc);
		$res = $db->GetRow($this->sql2);
		if($res)
			return $res;
		else
			return FALSE;
...
//added by daryl
	function getPatientEncounterRemarks($encounter_nr=0){
	global $db;

		$this->sql ="SELECT red.encounter_nr, red.remarks
						 FROM $this->tb_enc_remarks as red
                   WHERE red.encounter_nr='$encounter_nr'
						 ORDER BY red.create_time DESC LIMIT 1";
		#echo "<br>res = ".$this->sql;
		if ($this->result=$db->Execute($this->sql)){
			if ($this->result->RecordCount())
	...
	function GetadmitDate($enc){
		global $db;

		$this->sql = "SELECT sbe.`encounter_date` as admit_date, sbe.`er_opd_diagnosis` as diagnosis
					  FROM care_encounter sbe
					  WHERE
					  encounter_nr = ".$db->qstr($enc);
		$this->result = $db->Execute($this->sql);
		$result = $db->GetRow($this->sql);

	if($result['admit_date'])
		return $result;
	else{
		return false;
	}
	...
	function useRemarks(){
		$this->coretable=$this->tb_enc_remarks;
		$this->ref_array=$this->fld_remarks;
	}

	function saveEncounterRemarks(&$data){
		if(!is_array($data)) return FALSE;
		$this->useRemarks();
		$this->buffer_array=NULL;
		return $this->insertDataFromInternalArray();
		...
		function updateEncounterRemarks($item_nr=''){
		if(empty($item_nr)) return FALSE;
		$this->where=" encounter_nr='$item_nr'";
		$this->useRemarks();
		$this->buffer_array=NULL;
...
function UpdateAdmittionDiagnosis($enc, $Admitiondiagnosis){
		global $db;

		$this->sql = "UPDATE care_encounter 
						SET er_opd_diagnosis = ".$db->qstr($Admitiondiagnosis)."
...
function IfPhicExceed($enc, $check, $pid){
	global $db;

		if($check == 1){
			$this->sql_select = "SELECT encounter_nr, 
										pid 
								FROM seg_exceed_phic
								WHERE encounter_nr = ".$db->qstr($enc)."
								AND pid = ".$db->qstr($pid);
			$this->res = $db->Execute($this->sql);
			$res = $db->GetRow($this->res);
			if($res){
				return true;
			}else{
				$this->sql = "INSERT INTO seg_exceed_phic
							(encounter_nr, pid)
							VALUES 
							(".$db->qstr($enc).", ".$db->qstr($pid).")";
			}
			
		}else if($check == 2){
			$this->sql = "DELETE FROM seg_exceed_phic
							WHERE encounter_nr =".$db->qstr($enc)."
							AND pid =".$db->qstr($pid);
		}
...
#added by daryl
function isTempOrFBill($enc){
	global $db;
	$enc = $db->qstr($enc);
	return $db->getRow("SELECT sbe.`encounter_nr`, ce.current_att_dr_nr , ce.current_dept_nr,
						fn_get_department_name(ce.current_dept_nr) AS dept_name,
						 fn_get_personell_lastname_first2(ce.current_att_dr_nr) as dr_name
						FROM seg_billing_encounter AS sbe
						INNER JOIN care_encounter ce ON sbe.encounter_nr = ce.encounter_nr
						   WHERE sbe.`encounter_nr` = $enc
