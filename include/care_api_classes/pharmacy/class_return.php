	...
	function GetReturnables( $filters ) {
		...
		$this->sql =
			"SELECT SQL_CALC_FOUND_ROWS oi.bestellnum,p.artikelname,p.generic,p.packing,\n".
				"(SELECT IFNULL(SUM(IFNULL(r.quantity,0)),0) 
					FROM seg_pharma_return_items r\n".
					"INNER JOIN seg_pharma_returns `spr`
  					ON r.return_nr = spr.return_nr
					WHERE r.bestellnum=oi.bestellnum
					 AND spr.encounter_nr =".$db->qstr($encounter_nr).
