...
class SegOrder extends Core {
	...
	function addOrders($refno, $orderArray, $itemsArray) {
        ...
        if($buf=$db->Execute($this->sql,$orderArray)) {
        	//cooment out auto serve
        	//$this->AutoServer($refno);
            return true;
		} else { return false; }
	}
	//added by poliam
...
//        $db->debug = true;
		...
		$this->sql = "SELECT a.bestellnum,a.artikelname,\n".
			"o.request_flag,oo.charge_type,oo.is_cash,\n".
...
//edited by poliam
		// fix properly display list of orders in pharmacy
		$query = "SELECT {calcRows}\n".
					...
					"o.is_cash,
					o.charge_type,
					a.area_name AS `area_full`,
					(SELECT COUNT(*) 
				  	FROM seg_pharma_order_items AS oi 
				  	WHERE o.refno = oi.refno) AS `count_total_items`,
				  	(SELECT COUNT(*) 
				  	FROM seg_pharma_order_items AS oi 
				  	WHERE o.refno = oi.refno 
				    AND oi.serve_status = 'S') AS `count_served_items`,
					o.is_urgent,\n".
					"(SELECT GROUP_CONCAT(CONCAT(IFNULL(oi.request_flag,''),'\\t',
						IFNULL(oi.serve_status,'N'),'\\t',prod.artikelname) SEPARATOR '\\n')\n".
					"FROM seg_pharma_order_items AS oi\n".
					"INNER JOIN care_pharma_products_main AS prod ON prod.bestellnum=oi.bestellnum\n".
					"WHERE o.refno = oi.refno) AS `items`\n".
			"FROM $this->coretable AS o\n".
	...
	//				break;
		...
		if (!$withDateFilters) {
			...
			$phFilters[] = "DATE(orderdate)>(DATE(NOW())-INTERVAL 5 DAY)";
		}
...
#added by daryl
	function getcompany_order($refno) {
		global $db;
		$refno = $db->qstr($refno);
		$this->sql="SELECT stc.company_desc AS company\n".
			"FROM $this->coretable AS o\n".
			"LEFT JOIN $this->person_tb AS p ON p.pid=o.pid\n".
			"INNER JOIN seg_type_company AS stc ON p.company_name=stc.company_id\n".
			"WHERE o.refno=$refno";
		if($this->result=$db->Execute($this->sql)) {
			return $this->result;
		} else { return false; }
	...
	function getPrice($id, $refno, $trans){
		...
		if($trans == 'charge'){
			$this->sql = "SELECT cppm.`price_charge` AS price 
							FROM care_pharma_products_main AS cppm 
							where cppm.`bestellnum` = ".$id;  
		}
		else{
			$this->sql = "SELECT cppm.`price_cash` AS price 
							FROM care_pharma_products_main AS cppm 
							where cppm.`bestellnum` = ".$id;
		}
