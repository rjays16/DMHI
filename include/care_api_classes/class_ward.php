...
#echo"class_ward.php : countBeds :: this->sql ='".$this->sql."' <br> \n";
	...
	*/
	function createWaitingInpatientList($ward_nr=0,$option){
		...
		*/
		...
		$this->sql="SELECT e.encounter_nr, e.encounter_class_nr, e.current_ward_nr, p.pid, p.name_last, p.name_first,p.suffix, p.date_birth, p.sex,w.ward_id
				    FROM $this->tb_enc AS e
					LEFT JOIN $this->tb_person AS p ON e.pid=p.pid
					LEFT JOIN $this->tb_ward AS w ON e.current_ward_nr=w.nr
	...
	*/
	...
	global $db;
		...
		$this->sql ="SELECT * FROM $this->tb_roomtype WHERE nr <> '99' ORDER BY name";
		if ($this->result=$db->Execute($this->sql)) {
				 $this->count=$this->result->RecordCount();
				 return $this->result;
			} else{
...
function RoomNrExists($wardNr=0, $room_nr=0, $bol=1){
	...
	//added by kenneth 12/05/2013
	function getRoomList($room=''){
		global $db;
		if(!empty($room)){
		$this->sql = "SELECT s.name, s.room_type AS group_id
					FROM care_type_room AS s
					WHERE s.status != '1' AND s.room_type = '$room'";
				}
		else{
			$this->sql = "SELECT s.name, s.room_type AS group_id
			FROM care_type_room AS s
