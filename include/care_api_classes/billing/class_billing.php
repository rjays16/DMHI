...
function getSummaryTransmittal($trans_no,$case_type) {
		global $db;
		
		$case_type_filter = "";
		if($case_type != 'all'){
			$case_type = $db->qstr($case_type);
			$case_type_filter = "WHERE scrp.`case_type` = $case_type ";
		}
					  ...
					  (
					    ...
					    INNER JOIN 
					      ...
					      FROM
					        ...
					        INNER JOIN seg_billing_caserate AS sbca 
	...
	function GetManualPhicCoverage($enc, $area){
		global $db;
			...
			if ($result->RecordCount()) {
				if ($row = $result->FetchRow()) {
					if($area == 'pharma'){
						...
						$amount = $row['lab_amount'];
					}else if ($area == 'rad'){
						$amount = $row['rad_amount'];
					}
...
function GetManualPhicCoverageAll($enc){
		global $db;

		$this->sql = "SELECT max_coverage
						FROM seg_limit_coverage
						WHERE encounter_nr ='$enc'
						AND is_deleted = '0'";
