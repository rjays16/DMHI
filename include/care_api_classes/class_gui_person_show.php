...
class GuiPersonShow {
	...
	*/
	...
	function getAge($dateString) {
			  ...
			  $yearDob = substr($dateString, 6, 4);
			  $monthDob = substr($dateString, 0, 2);
			  $dateDob = substr($dateString, 3, 2);
			  ...
			  $age = array('years' => $yearAge, 'months' => $monthAge, 'days' => $dateAge);
...
#			$this->smarty->assign('sRegTime',convertTimeToLocal(@formatDate2Local($date_reg,$date_format,0,1)));   # burn commented: April 17, 2007
			...
			$this->smarty->assign('sSuffix',$this->createTR($LDSuffix,strtoupper($suffix),1,TRUE));
			// var_dump($sCross);die;
			...
			if($date_birth){
				...
				if (!($age = $this->person_obj->getAge($sBdayBuffer))){
					...
					$sBdayBuffer = 'Not Available';
						...
						$this->smarty->assign('segAge','<span class="vi_data">'.$this->getAge($sBdayBuffer).' </span>'); //updated by mai 06-23-2014
				}
			}
...
#			$this->smarty->assign('sBdayInput','<div class="vi_data">'.@formatDate2Local($date_birth,$date_format).'</div>');   # burn commented: March 26, 2007
        ...
        $this->smarty->assign('sIsNewborn',$showBirth);
			...
			*/
			...
			$company = $person_obj->getcompanyname($pid);
			if($company){
				$this->smarty->assign('sCompanyName',$company);
			}
			//ended by daryl
