...
#echo "class_radiology.php :getNewRID : this->sql = '".$this->sql."' <br> \n";
		...
		*/
	function createNewRID($pid='', $rid_new=''){
		...
		if ($rid = $this->RIDExists($pid)){
			if($rid != $rid_new){
				return $this->UpdateRID($rid_new, $pid);}
			else
				return $rid;
		}
...
#				return TRUE;
	...
	//added by ken 4/29/2014 for updating RID number
	function UpdateRID($rid, $pid){
		global $db;

		$rid = $db->qstr($rid);

		if($check_rid = $this->CheckRID($rid))
			return 0;
		
		$this->sql = "UPDATE seg_radio_id SET rid = ".$rid."
						WHERE pid = ".$pid;
	...
	function CheckRID($rid){
		...
		$this->sql = "SELECT rid, pid FROM seg_radio_id
						WHERE rid = ".$rid;
...
#echo "<br>class_radiology.php : saveRadioRequestInfoFromArrayNEW : data : <br> "; print_r($data); echo " <br><br> \n";
		...
		$arrayItems = array();
		foreach ($service_code as $key => $value){
			$tempArray = array($value,$clinical_info[$key],$pnet[$key],$pcash[$key],$pcharge[$key],$is_in_house[$key],$request_doctor[$key]);
			array_push($arrayItems,$tempArray);
		}
...
#		"request_doctor", "request_date", "encoder",	"status", "priority", "history",	"create_id", "create_dt"
		...
		*/
		...
		if ($is_cash){
			$index = "refno, service_code, clinical_info, price_cash, price_cash_orig, price_charge, is_in_house,
							request_doctor, request_date, encoder,	status, history, create_id, create_dt,parent_batch_nr,parent_refno,approved_by_head,remarks,headID,headpasswd,request_flag";
			$values = "'$refno', ?, ?, ?, ?, ?, ?, ?".
							", '$request_date', '".$HTTP_SESSION_VARS['sess_user_name'].
...
#exit();
	...
	function getScheduledRadioRequestInfo($batch_nr='',$limit=FALSE, $sub_dept_nr='', $date='',$walk=''){
		...
		$this->sql="SELECT sr_sked.schedule_no, sri.rid, IF ((request_flag IS NOT NULL),1,0) AS hasPaid,
							...
							IF(ISNULL(r_serv.pid),
								r_serv.ordername, fn_get_pid_name(r_serv.pid)) AS patient_name,
							cp.name_first, cp.name_middle, cp.name_last, cp.suffix
	...
	*/
	...
	function _searchBasicInfoRadioPending($search_type='',$key,$sub_dept_nr='',$ORDER_BY_OPTION='',$limit=FALSE,$len=30,$so=0){
        ...
        $this->sql="SELECT " .
                ...
                "p.name_last, p.name_first,p.suffix, p.date_birth, p.sex, r.is_served " .
...
#			echo "class_radiology.php : _searchBasicInfoRadioPending : FALSE 02 <br>";
		...
		*/
		...
		function searchRadioPatients($search_type='',$key,$sub_dept_nr='',$limit=FALSE,$len=30,$so=0){
				...
				$this->sql="SELECT DISTINCT sri.rid, p.name_last, p.name_first, p.suffix, e.encounter_type, dept.name_formal as name_short, r_serv.pid
												FROM seg_radio_serv as r_serv
														INNER JOIN seg_radio_id AS sri ON sri.pid=r_serv.pid
														INNER JOIN care_person AS p ON p.pid = r_serv.pid
														INNER JOIN care_test_request_radio AS r ON r.refno=r_serv.refno
        ...
        if (!empty($sub_dept_nr)) {
            ...
            "END)";
				...
				$this->sql="SELECT SQL_CALC_FOUND_ROWS r.request_flag AS charge_name,
									...
									p.name_last, p.name_first,p.suffix,p.name_middle, p.date_birth, p.sex, p.pid,
...
#--------------------------end by mark--------------------------------------------------------
	...
	//added by kenneth 12/07/2013
	function addRadioItemRate($src,$grp,$code,$room,$cash,$charge){
		global $db;
		
		$this->sql="INSERT INTO seg_item_rate(source,group_code,item_code,room_type,cash,charge) 
							VALUES('$src','$grp','".strtoupper($code)."','$room','$cash','$charge')";
	...
	//added kenneth 12/07/2013
	function updateRadioItemRate($excode,$src,$grp,$code,$room,$cash,$charge){
		global $db;
		$charlist="\0..\37";
		$excode=addcslashes($excode,$charlist);
		$this->sql="UPDATE seg_item_rate 
					SET source='$src', group_code='$grp', item_code='$code',room_type='$room',cash='$cash', charge='$charge'
					WHERE ((room_type='$room')AND (item_code='$excode') OR (room_type='$room') AND (item_code='".urlencode($excode)."'))";
...
function SearchService2($source_req='LD', $is_charge2comp=0, $compID='',$dept_nr,$is_cash=1,$discountid='',$discount=0, $is_senior=0, $is_walkin=0, $sc_walkin_discount=0,$codenum, $searchkey='',$multiple=0,$maxcount=100,$offset=0,$area='',$sect=''){
	...
	global $db, $sql_LIKE, $root_path, $date_format;
		...
		$this->sql= "SELECT DISTINCT SQL_CALC_FOUND_ROWS rs.`request_flag`  AS charge_name,e.is_maygohome, r.encounter_nr, r.is_cash, r.request_date, r.request_time, rs.parent_refno,rs.parent_batch_nr,IF ((rs.request_flag IS NOT NULL),1,0) AS hasPaid, " .
                "rs.batch_nr AS refnum,rs.refno AS batchnum, rs.modify_id AS encoder, rs.create_id AS encoder2, sk.*, sk.status AS skstatus, ri.rid, rs.service_code, s.name AS serv_name, " .
                "s.group_code, d.id, d.name_formal, d.name_short AS dept_short_name, " .
                "r.pid, r.refno, p.name_first, e.encounter_type, " .
                "p.name_middle, p.name_last,p.suffix, r.ordername, rs.is_served, rs.served_date " .
                ($fields ? ','. implode(",", $fields) : '') . ' ' .
	...
	function countSearchImpressions($searchkey='',$maxcount=100,$offset=0) {
		...
		$this->sql = "SELECT f.*
						FROM seg_radio_impression_code AS f
	...
	function countSearchFindings($searchkey='',$maxcount=100,$offset=0) {
		...
		$this->sql = "SELECT i.*
						FROM seg_radio_findings_code AS i
						WHERE (i.codename LIKE '%".$keyword."%' OR i.description LIKE '%".$keyword."%')
						ORDER BY i.id ASC";
...
//				"VALUES('".$code."', '".$name."','".$impression."', '', CONCAT('Create: ',NOW(),' [$userid]\\n'),'$userid',NOW(),'$userid',NOW())";
	...
	#---------------
		...
		function getAllRadioDoctor($role,$nr){
				...
				$this->sql = "SELECT a.personell_nr,CONCAT(ifnull(p.name_last,''),', ',ifnull(p.name_first,''),' ',ifnull(concat(substr(p.name_middle,1,1),'.'),'')) AS dr_name, ps.doctor_level,
												...
												AND (a.location_nr=164 OR a.location_nr IN (216, 158, 165, 166, 167, 235) )
    ...
    function ServedRadioRequest($batch_nr, $refno, $service_code, $is_served, $date_served, $rad_tech){
     ...
     function checkServedRadioRequest($batch_nr, $refno, $code){
     	global $db;

     	$sql = "SELECT rr.`price_charge`, rs.`encounter_nr`, rs.`pid`, rs.`request_date`, rs.`is_cash`, rr.`refno`
     				FROM care_test_request_radio AS rr
     					INNER JOIN seg_radio_serv AS rs
     						ON rr.`refno` = rs.`refno`
     				WHERE rr.`is_served` = '1'
     					AND rr.`batch_nr` = '$batch_nr'
     					AND rr.`refno` = '$refno'";
        ...
        if (!empty($sub_dept_nr)) {
            ...
            "END)";
				...
				$this->sql="SELECT SQL_CALC_FOUND_ROWS r.request_flag AS charge_name,
									...
									dept.id AS sub_dept_id, dept.name_formal AS sub_dept_name,
									r_services.service_code, r_services.name AS service_desp,
									p.name_last, p.name_first, '' as 'suffix',p.name_middle, p.date_birth, p.sex, p.pid,
									p.date_birth, IF(fn_calculate_age(NOW(),p.date_birth),fn_get_age(NOW(),p.date_birth),age) AS age,
									r_id.rid, e.current_ward_nr, e.current_room_nr, e.current_dept_nr, e.encounter_type,
            r.parent_batch_nr, r.parent_refno, r.is_served " .
            ($fields ? ','. implode(",", $fields) : '') . ' ' .
 ...
 function SearchSelect_walkin($searchkey='',$sub_dept_nr=0,$maxcount=100,$offset=0){
	global $db, $sql_LIKE, $root_path, $date_format;
		...
		$this->sql= "SELECT DISTINCT SQL_CALC_FOUND_ROWS rs.`request_flag`  AS charge_name, e.is_maygohome, r.encounter_nr, r.is_cash, r.request_date, r.request_time, rs.parent_refno,rs.parent_batch_nr,IF ((rs.request_flag IS NOT NULL),1,0) AS hasPaid, " .
                ...
                "s.group_code, d.id, d.name_formal, d.name_short AS dept_short_name, " .
                "r.pid, r.refno, p.name_first, e.encounter_type, " .
                "p.name_middle, p.name_last,('') as suffix, r.ordername, rs.is_served, rs.served_date " .
                ($fields ? ','. implode(",", $fields) : '') . ' ' .
            "FROM {$this->tb_test_request_radio} AS rs " .
                "LEFT JOIN seg_radio_schedule AS sk ON rs.batch_nr=sk.batch_nr " .
                "INNER JOIN seg_radio_serv AS r ON r.refno=rs.refno " .
...
#edited by daryl
	function searchBasicInfoRadioPending_walkin($search_type='',$key,$sub_dept_nr='',$ORDER_BY_OPTION='',$limit=FALSE,$len=30,$so=0){
        ...
        $this->sql="SELECT " .
                ...
                "r_services.name, r_serv_group.group_code , " .
                "r_serv_group.department_nr AS sub_dept_nr,dept.name_short, dept.id AS sub_dept_id, dept.name_formal AS sub_dept_name, " .
                "p.name_last, p.name_first,('') AS suffix, p.date_birth, r.is_served " .
                ($fields ? ','. implode(",", $fields) : '') . ' ' .
            "FROM seg_radio_serv AS r_serv " .
                "INNER JOIN care_test_request_radio AS r ON r.refno=r_serv.refno " .
                "LEFT JOIN care_encounter AS e ON e.encounter_nr=r_serv.encounter_nr " .
...
#			echo "class_radiology.php : _searchBasicInfoRadioDone : FALSE 02 <br>";
	...
	function GetHolidays($date){
		...
		$this->sql = "SELECT holiday_date
					FROM seg_holidays
					WHERE holiday_date = ". $db->qstr($date);
	...
	// //added by daryl 07/01/2014
function checkifSCs($refno){
		global $db;

		$this->sql = "SELECT    SUM(  CASE    WHEN (str.`price_cash` <> str.`price_cash_orig`) 
						OR   (str.`price_charge` <> str.`price_cash_orig`) 
     							   THEN 1    	
     							    ELSE 0     END  ) AS ifSC
	 				FROM  care_test_request_radio  str
					WHERE str.`refno` = ".$db->qstr($refno)."";

		$ifSC = $db->Execute($this->sql);
		return $ifSC;
}

//added by janken 1/21/2015 for changing the price in cash and charge
function getPrice($id, $refno, $trans){
	global $db;
	$refno = $db->qstr($refno);
	$id = $db->qstr($id);

	if($trans == 'charge'){
		$this->sql = "SELECT IFNULL(sor.`charge_price`, srs.`price_charge`) AS price 
						FROM seg_radio_serv AS sr 
						  INNER JOIN care_test_request_radio AS ctrr 
						    ON sr.`refno` = ctrr.`refno` 
						  INNER JOIN seg_radio_services AS srs 
						    ON ctrr.`service_code` = srs.`service_code` 
						  INNER JOIN care_encounter AS ce 
						    ON ce.`encounter_nr` = sr.`encounter_nr` 
						  INNER JOIN care_room AS cr 
						    ON ce.`current_room_nr` = cr.`room_nr` 
						    AND ce.`current_ward_nr` = cr.`ward_nr` 
						  INNER JOIN seg_other_rates AS sor 
						    ON cr.`type_nr` = sor.`room_type` 
						    AND sor.`service_code` = ctrr.`service_code` 
						WHERE sr.`refno` =  ".$refno." AND ctrr.`service_code` = ".$id;  
	}
	else{
		$this->sql = "SELECT IFNULL(sor.`cash_price`, srs.`price_cash`) AS price 
						FROM seg_radio_serv AS sr 
						  INNER JOIN care_test_request_radio AS ctrr 
						    ON sr.`refno` = ctrr.`refno` 
						  INNER JOIN seg_radio_services AS srs 
...
function checkDone($refno){
	...
	$this->sql = "SELECT ctrr.`refno`, ctrr.`service_code`, ctrr.`is_served`
					FROM care_test_request_radio AS ctrr
					WHERE ctrr.`is_served` = '1' AND ctrr.`refno` = ".$refno;
