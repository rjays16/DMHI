...
#echo "class_drg.php : getDiagnosisCodes : this->sql = '".$this->sql."'<br> \n";
	...
	*/
	...
	function getProcedureCodes($enc_nr=0,$enc_type=0){
		...
		$this->sql="SELECT sed.`description` AS diag_desc, p.*, TRIM(c.code) AS code, TRIM(c.description) AS therapy, ".
...
".
						...
						"INNER JOIN 
						  ...
						  ON sed.`encounter_nr` = p.`encounter_nr`
						  AND p.`code` = sed.`code`
...
#echo "getProcedureCodes : this->sql = '".$this->sql."' <br> \n";
	...
	*/
	...
	//added by daryl
	//08/31/2014
	//added seg_encounter_diagnosis table and description parameter
	function getProcedure_Codes($enc_nr=0,$enc_type=0){
	    global $db;
		...
		$this->sql="SELECT sed.`description`, p.*, TRIM(c.code) AS code, TRIM(c.description) AS therapy, ".
						" p.type_nr AS type, m.description AS parent_desc ".
"
	, (SELECT id FROM care_department AS dept WHERE dept.nr = p.responsible_dept_nr) AS responsible_dept_id
	, (SELECT name_formal FROM care_department AS dept WHERE dept.nr = p.responsible_dept_nr) AS responsible_dept_name
...
".
						" FROM $this->tb_procedure AS p ".
						" LEFT JOIN $this->tb_proc_codes AS c ON p.code=c.code".
						" LEFT JOIN seg_encounter_diagnosis AS sed ON sed.`code` = c.`code`".
						" LEFT JOIN $this->tb_proc_codes AS m ON p.code_parent=m.code ".
						" WHERE p.encounter_nr='$this->enc_nr' AND p.status NOT IN($this->dead_stat) $cond".
						" ORDER BY p.type_nr DESC, p.date ASC";
#echo "getProcedureCodes : this->sql = '".$this->sql."' <br> \n";
		...
		function getDiagnosisCodes_seg($enc_nr,$code){
		global $db;

		$this->sql="SELECT * FROM seg_encounter_diagnosis sed 
					WHERE sed.`encounter_nr` = '$enc_nr' 
					AND sed.`code` = '$code'
					And sed.`is_deleted` <> '1' ";
	...
	function get_DiagnosisCodes($enc_nr){
		global $db;

		$this->sql = "SELECT 
							  sed.`code` AS diag_code,
							...
							FROM
							  ...
							  INNER JOIN care_encounter e 
							    ON e.`encounter_nr` = sed.`encounter_nr` 
							...
							FROM
							  seg_encounter_diagnosis sed 
							  INNER JOIN care_icd10_en cen 
							    ON cen.diagnosis_code = sed.`code` 
							  INNER JOIN care_encounter e 
							    ON e.`encounter_nr` = sed.`encounter_nr` 
							  INNER JOIN care_encounter_diagnosis ced 
							    ON ced.code = cen.diagnosis_code 
							WHERE sed.`encounter_nr` = '$enc_nr' 
							  AND sed.`is_deleted` = '0' 
							  AND e.status NOT IN (
							    'deleted',
