	...
	*/
	...
	function searchProducts($codename, $generic, $classification, $prodclass, $offset=0, $rowcount=10, $sort_order='artikelname') {
		...
		if ($codename) {
			...
			$where[] = "p.bestellnum=" . $db->qstr($codename). " OR p.artikelname REGEXP ". $db->qstr("[[:<:]]" . $codename);
		}
		if ($generic) {
		...
		if ($classification) {
			...
			$where[] = "EXISTS(SELECT * FROM seg_pharma_products_classification WHERE bestellnum=p.bestellnum AND class_code=" . $db->qstr($classification) . ")";
		}
		if ($prodclass) {
		...
		foreach ( $filters as $filter=>$value ) {
			switch (strtolower($filter)) {
				...
				case 'prodclass':
					//modified by cha, october 14, 2010
					//$where[] = "ptype.prod_class=".$db->qstr($prodclass);
					$where[] = "ptype.prod_class=".$db->qstr($value);
					break;
				case 'offset':
					$offset = $value;
					break;
	...
	function getProductInfo($nr) {
		...
		function getExtendedProductInfo($item_no) {
				...
				$this->sql = "SELECT * FROM seg_item_extended WHERE item_code='$item_no'";
				if($buf=$db->Execute($this->sql)) {
						if($buf->RecordCount()) {
								$row = $buf->FetchRow();
								return $row;
	...
	//edited by poliam
	//remove  ptype.prod_class AS `prod_class` and where ptype IN ('M', 'S');
	//to identify the real Meds and supplies
...
//        if ($stype) $where[] = "prod_class = '$stype'";
		...
		function getDefBigUnitID(&$big_unitid, &$big_unitname) {
				...
				$strSQL = "select unit_id, unit_name ".
									"   from seg_unit ".
				...
				if ($result = $db->Execute($strSQL)) {
						if ($result->RecordCount()) {
								$row = $result->FetchRow();
								$big_unitid = $row['unit_id'];
								$big_unitname = $row['unit_name'];
						}
				}
		}
		...
		function getDefSmallUnitID(&$small_unitid, &$small_unitname) {
				...
				if ($result = $db->Execute($strSQL)) {
						if ($result->RecordCount()) {
								$row = $result->FetchRow();
								$small_unitid = $row['unit_id'];
								$small_unitname = $row['unit_name'];
						}
				}
		}
