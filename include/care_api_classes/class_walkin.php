...
class SegWalkin extends Core {
  ...
  function getWalkinByName($ln, $fn, $mn) {
    ...
    $fn = $db->qstr($fn);
    $mn = $db->qstr($mn);
		$this->sql = "SELECT pid,name_last,name_first,name_middle,date_birth,address,sex,age FROM seg_walkin WHERE name_last=$ln AND name_first=$fn AND name_middle=$mn";
	...
	function searchWalkin($filters)
      ...
      function getAllInfoArray($pid='') {
        ...
        */
        ...
        $this->sql= "SELECT p.*,
                            (SELECT admission_dt FROM care_encounter AS enc 
                            WHERE (enc.pid='$this->pid')
                            AND enc.encounter_status <> 'cancelled' 
                            AND enc.status NOT IN ('deleted','void') 
                            ORDER BY enc.encounter_nr DESC LIMIT 1) AS admission_dt,
                            (SELECT encounter_date FROM care_encounter AS enc 
                            WHERE (enc.pid='$this->pid') 
                            AND enc.encounter_status <> 'cancelled' 
                            AND enc.status NOT IN ('deleted','void') 
                            ORDER BY enc.encounter_nr DESC LIMIT 1) AS encounter_date
