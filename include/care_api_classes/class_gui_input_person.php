			...
			if(!$error) {
				...
				if ($update) {
					...
					$sql="UPDATE $dbtable SET
							 ...
							 name_2=upper('$name_2'),
							 name_3=upper('$name_3'),
							 name_middle=upper('$name_middle'),
							 name_maiden=upper('$name_maiden'),
							 name_others=upper('$name_others'),
							 date_birth='".formatDate2STD($date_birth,$date_format)."',
                             birth_time = '$birth_time',
							 place_birth=upper('".$place_birth."'),
							 ...
							 nat_id_nr='$nat_id_nr',
							 religion='$religion',
							 mother_fname=upper('".$mother_fname."'),
							 mother_maidenname=upper('".$mother_maidenname."'),
							 mother_mname=upper('".$mother_mname."'),
							 mother_lname=upper('".$mother_lname."'),
							 father_fname=upper('".$father_fname."'),
							 father_mname=upper('".$father_mname."'),
							 father_lname=upper('".$father_lname."'),
...
#echo "class_gui_input_person.php : insert mode : pinsure_obj->sql='".$pinsure_obj->sql."' <br> \n";
					...
					if(!$error_person_exists||$mode=='forcesave'){
						...
						#edited by ken for separation of pid in personell and patient 2/25/2014
						$ref_nr=$GLOBAL_CONFIG['person_id_nr_init'];
						$HTTP_POST_VARS['pid']=$person_obj->getNewPIDNr($ref_nr+$GLOBAL_CONFIG['person_id_nr_init_adder'],$personell);
						...
						$HTTP_POST_VARS['contact_no'] = $ContactNo;
						$HTTP_POST_VARS['company_name'] = $CompanyName;
						#----------------------
						...
						if($person_obj->insertDataFromInternalArray()){
							...
							if(file_exists($this->displayfile)){
                                ...
                                //$HTTP_POST_VARS['admitted_baby'] == 0
                                if ((($ptype=='newborn')&&($fromtemp))&&($HTTP_POST_VARS['admitted_baby']==0)){
...
<script type="text/javascript" src="js/reg-person-gui.js?t=<?=time()?>"></script> <!-- added by daryl: -->
...
//			var dob = document.aufnahmeform.date_birth.value;
		...
		*/
		...
		/*added by mai 06-20-2014*/
		function getAge_(dateString) {
			  var now = new Date();
...
#echo "class_gui_input_person.php : pid = '".$pid."';   profileType = '".$profileType."' <br> \n";
	...
	*/
	...
	<?
		...
		#edited by VAN
		$this->smarty->assign('segAge','<input name="age" id="age" type="text" size="21" maxlength=5 value="'.$age.'" onBlur="trimString(this);checkCivilStat();checkIfSenior();"> old'); /*updated by mai 06-23-2014*/
#echo "senior_ID = ".$senior_ID;
		...
		if ($_GET['pid']){
			...
			// $this->smarty->assign('LDCompanyName',"Company"); 
			// $this->smarty->assign('sCompanyName','<select id="CompanyName" name="CompanyName"></select>');
