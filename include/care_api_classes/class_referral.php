...
class Referral extends Notes {
    ...
    */
    ...
    function SaveReason($hospital, $reason, $enc, $nr){
        global $db;

        
        $this->sql = "SELECT * FROM seg_referral_diag_and_procedure  WHERE is_deleted <> '1' AND enc_nr = ".$db->qstr($enc)." AND icd_code = ".$db->qstr($nr);
        
        if($this->result = $db->Execute($this->sql))     
            if ($this->result->RowCount() != 0) {
                $this->sql = "UPDATE seg_referral_diag_and_procedure SET accredit_nr = '".$hospital."', reason = '".$reason."' WHERE is_deleted <> '1' AND enc_nr = ".$db->qstr($enc)." AND icd_code = ".$db->qstr($nr);
                if($this->result = $db->Execute($this->sql))
                        return true;
                    else
                        return false;
             }else{
                $this->sql = "INSERT INTO seg_referral_diag_and_procedure(accredit_nr,reason,enc_nr,icd_code) ".
                  "VALUES ('".$hospital."','".$reason."','".$enc."', '".$nr."')";
                    if($this->result = $db->Execute($this->sql))
                        return true;
                    else
                        return false;
    ...
    function Getreferral($enc, $nr){
        ...
        $this->sql = "SELECT reason,
                            accredit_nr
                    FROM seg_referral_diag_and_procedure
                    WHERE is_deleted <> '1'
                    AND enc_nr = ".$db->qstr($enc)."
                    AND icd_code = ".$db->qstr($nr);
        if($this->result = $db->Execute($this->sql)){
            return $this->result;
        }else{
            return false;
...
function getreferralDetails($enc){
        global $db;
