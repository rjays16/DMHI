...
Class Lab_Results extends Core {
		...
		*/
		...
		function add_lab_resultdata ($refno, $group_id=0, $date, $med_tech, $pathologist, $is_confidential, $service_code='')
		{
				...
				$this->sql = "INSERT INTO seg_lab_resultdata(refno, group_id, service_code, service_date, pathologist_pid, med_tech_pid, history, modify_id, modify_dt, create_id, create_dt, status, is_confidential) VALUES('$refno', '$group_id', '$service_code', '$date', '$pathologist', '$med_tech', ".$history.", '".$_SESSION['sess_user_name']."', NOW(), '".$_SESSION['sess_user_name']."', NOW(), '', $is_confidential);";
				//$error = $db->ErrorMsg();
	...
	//added by ken 5/7/2014 for getting the service name in result
		...
		function checkResult($refno, $code){
			...
			$this->sql = "SELECT fn_get_person_name(rd.`pathologist_pid`) AS pathologist, rd.`is_confidential`, 
								fn_get_person_name(rd.`med_tech_pid`) AS medtech, rd.`service_date`, res.*, rp.*, 
								rpg.`name` AS param_grp_name, rpa.`param_id`, rf.`form_id`, rf.`name` AS form_name
							FROM seg_lab_resultdata AS rd
							...
							LEFT JOIN seg_lab_result_paramgroups AS rpg
								ON rpg.`param_group_id` = rp.`param_group_id`
							WHERE rd.`refno` = '$refno' 
								AND rd.`service_code` = '$code'
								AND rd.`status` != 'deleted'
								AND res.`status` != 'deleted'
								  ORDER BY form_id, rpg.`name` ";
