...
//added by daryl
		function getIcd10Info_seg($icdcode=''){
		global $db;

		$this->sql="SELECT * FROM seg_case_rate_packages WHERE code='$icdcode'";
		#echo "<br>sql = ".$this->sql;
		if($this->res['mcode']=$db->Execute($this->sql)){
			if($this->res['mcode']->RecordCount()){
				return $this->res['mcode'];
...
//added by daryl
//08/30/2014
  function getICD_Selected($sfilter = '') {
        global $db;

				$sfilter = trim($sfilter);
        // $this->sql = "select code, description
        //                  from seg_case_rate_packages
        //                  where code like '$sfilter%' 
        //                  AND case_type='m'
        //                   order by code";   
  $this->sql = "SELECT diagnosis_code, description
                         FROM care_icd10_en
                         WHERE diagnosis_code LIKE '$sfilter%'  
                         AND description <> ''
                UNION
               	SELECT scrp.code AS diagnosis_code, scrp.description
              			 FROM seg_case_rate_packages AS scrp
              			 WHERE scrp.code LIKE '$sfilter%'
              			   AND description <> ''
                            AND case_type = 'm'
                           ORDER BY diagnosis_code";   
   ...
   function getSelected_ICDDesc($sfilter = '') {
        global $db;
				#edited by VAN
				$char_array = array(".","'","{","}","[","]","^","(",")","-","`",",","|");
				$sfilter = str_replace($char_array,"",$sfilter);
      ...
      //   $this->sql = "SELECT code, description
      //                    from seg_case_rate_packages
						// 						 where replace(description,',','') regexp '.*$sfilter.*' 
						// AND case_type='m'
      //                    order by description";
		$this->sql = "SELECT diagnosis_code, description
                         from care_icd10_en
